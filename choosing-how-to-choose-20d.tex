...
{\color{orange}
	This example is closely related to another phenomenon: Dutch books, or paradoxes of sequential choice, can be constructed against agents on the basis of such examples. For instance, we might consider how the agent will choose in $D_1$ and $D_2$, individually, and then combine these choices and observe that the result is dominated---1B-and-2A dominates 1A-and-2B. One response is simply to reject the package principle. However, there is another version of the examples in which they are presented diachronically: first evaluate $D_1$, then $D_2$. And again we can note that 1B- But then the evaluation of $D_2$ surely depends on what has been selected in $D_1$. 
Such examples can be used to construct instances of undermining evaluations of strategies when we assume that your confidence over the various decisions you'll face is uniform, so that the result is equivalent (probabilistically) to just adding up the results of each of the decisions. 
}
...
{\color{blue}
	...
	Instead, the defender of E-Admissibility will highlight that it sees value in coordinating how you resolve incomparability and randomisation, or anything that amounts to that, just won't do. 
