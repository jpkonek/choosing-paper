...
If there is a partition $\X$ of $\IP$ such that for all $x\in\X$ there is some $\exists B_x\subseteq\Decs$ satisfying
\begin{enumerate}
\item $(\forall D\in B_x)\EAd_\IP(D)\setminus\left(\cup_{p\in x}\EU_{p}(D)\right)\not=\varnothing$
...
For any $x\in\X$ and any $D\in B_x$, let $\s(D)$ be any element of $\EAd_\IP(D)\setminus\left(\cup_{p\in x}\EU_{p}(D)\right)$. For any $D\in\Decs\setminus\left(\cup_{x\in\X}B_x\right)$, let $\s(D)$ be any element of $\EAd_\IP(D)$.
