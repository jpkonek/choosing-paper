...
%\end{colored}
\section{Maximality \\(\cref{thm:max-nu,thm:max-nu-reg-nec,thm:max-existsimpermissible[dep]})}
...
We now have two versions of Wald's Complete Class Theorem: the first we proved along the way to proving the second. First: if $\Uwald(\O)$ is closed and convex, then $o$ is Bayes iff $o$ is not strictly dominated. Second: if $\Uwald$ is Bayes-continuous over $\O$ and $\O$ is Bayes-existing relative to $\Uwald$, then again $o$ is Bayes iff $o$ is not strictly dominated.
...
\item $\Uwald:\Nu\times\Omega\to[l,u]$ defined by $\Uwald(\nu)(\omega):=\Exp_{\mu^*}[\U(\nu)(\omega)]$.
%\item $\Uwald(\n)(\omega):=\Exp_{\mu^*}[\U(\nu)(\omega)]$. 
\\That is, $\Exp_{D\sim \mu^*}[\U(\nu)(\omega,D)]=\Exp_{D\sim \mu^*}[\Exp_{a\sim \nu_D}\U(a)(\omega)]$.
%\item $\Uwald(\n)(\omega):=\Exp_{\mu^*}[\U(\n, -)(\omega)]=\Exp_{\mu^*}[\Exp_{\n_{-}}[ \U(\cdot)(\omega)]$. 
\\The utility of a probabilistic picking strategy $\n$ at $\omega$ is the expectation of the utility you'll obtain at $\omega$ by picking in accordance with $\n$.
\end{itemize}
...
\end{lemma}
\begin{proof}
	If $\Nu$ is EU-complete, then for each $p$, there is some $\n$ in $\Nu$ that picks for $\EU_p$. And, by \cref{thm:EU appendix corollary}, since $\n$ picks for $\EU_p$, $\n$ is Bayes for $p$. %\todooldinfo{where's the ref??} 
\end{proof}
...
\begin{lemma}\label{thm:cct-appln:cts}
	...
	then $\Uwald$ is Bayes continuous on $\Nu$, where $\Uwald(\nu)$. 
...
%	We will show that for any $\n^{p^*}$ which is Bayes for $p^*$ and $\n^{p_n}$ which are Bayes for $p_n$, we have the utility profiles of $\n^{p_n}$ converge to the utility profile of $\nu^{p^*}$. 

Suppose $\nu^{p_n}$ is Bayes for $p_n$ and $\nu^{p^*}$ is Bayes for $p^*$. 
We will show that for each $\omega\in\Omega$,  $\Uwald(\n^{p_n})(\omega)\longrightarrow\Uwald(\nu^{p^*})(\omega)$; that is, $\Exp_{\mu^*}[\U(\nu^{p_n})(\omega)]\longrightarrow\Exp_{\mu^*}[\U(\nu^{p^*})(\omega)]$. 

Fix any $\omega^*\in\Omega$. 

Let:
\begin{itemize}
	\item $\X^* = \Set{D \in \D \given \n^{p^*}_D(\EU_{p^*}(D)) = 1 }$
	\item $\X^n = \Set{D \in \D \given \n^{p_n}_D(\EU_{p^n}(D)) = 1 }$
	\item $\Y^* = \Set{D \in \D \given \EU_{p^*}(D) \text{ is a singleton}}$
\end{itemize}

We have ${\mu^*}(\X^*) =1$ by assumption that $\nu^{p^*}$ is Bayes for $p^*$; and similarly, for each $n$, $\mu^*(\X^n)=1$ as $\nu^{p_n}$ is assumed to be Bayes for $p_n$.

We also know that ${\mu^*}(\Y^*) = 1$, since we have assumed that ${\mu^*}$ requires almost everywhere decisiveness. 

And so, since ${\mu^*}$ is countably additive,
$${\mu^*}\left (\X^* \cap \bigcap^\infty_{n=1} \X^n \cap \Y^* \right ) = 1.$$


Now take any $D^*\in \X^* \cap \bigcap^\infty_{n=1} \X^n \cap \Y^* $

$\EU_{p^*}(D^*)$ is a singleton, so let $\EU_{p^*}(D^*)=\{a^*\}$. As $\nu^{p^*}_{D^*}(\{a^*\})=1$, observe that  $\U(\nu^{p^*})(\omega^*,D^*)=\U(a^*)(\omega^*)$. 

Expected utility is continuous as a function of probabilities; \todo{This must require ctbly addy? Anything else?}i.e., for all $a\in\A$, $\Exp_{p_n}[\U(a)]\longrightarrow \Exp_{p^*}[\U(a)]$. Thus, as $D^*$ is finite, for some $N\in\mathbb{N}\setminus\{0\}$ and for all $n>N$, $\EU_{p_n}(D^*)=\{a^*\}$. 

And so, for each $n>N$, $\nu^{p_n}_{D^*}(\{a^*\})=1$ and  so $\U(\nu^{p_n})(\omega^*,D^*)=\U(a^*)(\omega^*)=\U(\nu^{p^*})(\omega^*,D^*)$. 

This gives us that $\U(\nu^{p_n})(\omega^*,D^*)\longrightarrow\U(\nu^{p^*})(\omega^*,D^*)$. 

As this holds for all $D\in \X^* \cap \bigcap^\infty_{n=1} \X^n \cap \Y^*$, 
we have that on a set of $\mu^*$-measure one, 
which is $\mu^*$-almost everywhere, as utilities are bounded, a Dominated Convergence Theorem gives us that
$\Exp_{\mu^*}[\U(\nu^{p_n})(\omega^*)]\longrightarrow\Exp_{\mu^*}\U(\nu^{p^*})(\omega^*)]$. That is, $\Uwald(\nu^{p_n})(\omega^*)\longrightarrow\Uwald(\nu^{p^*})(\omega^*)$.
\end{proof}
\end{colored}
...
\end{proof}
\cref{thm:max-nu} follows from this.% \todo{Let's double checkt hat that's true.}
