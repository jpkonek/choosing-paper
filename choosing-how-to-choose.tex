...
{\color{violet}\todoinfo{CCM playing with rewriting. Actually 3 versions/attempts. }
			\todoinfo{v2 }
...
\end{array}
$$
The empirically observed preferences are these: $\text{1A} \succ \text{1B}$ and $\text{2A} \prec \text{2B}$. 
Let's suppose that our agent, with her selected decision theory, endorses such preferences.
...
Now consider a decision problem with two options, a risky bet and a safe option. 
For example, these are options for what stocks she can invest in. 
Let's say the risky one depends on some particular outcome, say whether this particular company goes bust; and let's suppose that the agent thinks that the odds of this company going bust are $1:10$. 
She may thus be faced with this decision problem:
$$
\begin{array}{r|cc}
...
\end{array}
$$	
Suppose, for now, that she prefers Safe over Risky.

Now, suppose she is unsure which decision will be in play. 
...
Or perhaps she has to choose a proxy to act on her behalf, knowing what they'll pick in each decision problem, but unsure which one will be at stake. For example, she is about to go into an operation and will be under anaesthetic when a decision might need to be made. 


The form that these options have is what we will call a \emph{picking strategy}. It is a function $\s$ which selects one of the available acts from each (relevant) decision problem. For example, it will specify what it will pick in $D^{\mathrm{Allais}}_{\mathrm{local}}$\; Safe or Risky. 
This is another kind of decision problem, a higher-order decision, and she can make use of her decision theory to determine what to do. 

Suppose that she is unsure whether she will be facing $D^{\mathrm{Allais}}_{\mathrm{local}}$, or will receive \pounds 1m for sure. And let's suppose that the credence that she'll be offered the \pounds 1m option is $89\%$, with $11\%$ chance that $D^{\mathrm{Allais}}_{\mathrm{local}}$ will be the decision in play. 

Here, the only relevant consideration in choosing amongst picking strategies (instructions to leave / proxy to select) is what they will pick when this is the decision in play. There are thus two options: $\s_{\mathrm{Safe}}$ or $\s_{\mathrm{Risky}}$, which select Safe or Risky, respectively. 

We are treating this just like another choice, another decision problem, evaluating it by its fruits. She is thus facing the following decision:
	$$
\begin{array}{c|ccc}
	\multirow{2}{*}{\text{Version 1}}	&\multicolumn{2}{c}{\text{choice offered}}&\text{choice not offered}\\
	& \text{goes bust} &  \text{not bust}  \\
...
\end{array}
$$
This is extensionally equivalent to the choice of 1A vs 1B in $D^{\mathrm{Allais}}_1$. 
So, assuming that her decision preferences just depend on the structure of credences in possibilities and the outcomes in each possibility, she should also judge $\s_{\mathrm{Safe}}$ to be preferable to $\s_{\mathrm{Risky}}$. 

That is, she should want to instruct her stockbroker to take the Safe option when facing this scenario, and select a proxy who will pick the Safe course of action. 

Now, suppose instead, that whilst she still has the same credence that she'll be offered $D^{\mathrm{Allais}}_{\mathrm{local}}$, she thinks that if it is not on offer, the alternative case is that she gets nothing. 
...
\end{array}
$$
This is now extensionally equivalent to the choice of 2A vs 2B in $D^{\mathrm{Allais}}_2$, and we have supposed that she preferred 2B over 2A, matching the observed preferences. In this scenario, then, she will evaluate the picking strategy $\s_{\text{Risky}}$ to be preferable to $\s_{\text{Safe}}$, even though, when faced with  $D^{\mathrm{Allais}}_{\mathrm{local}}$ herself, she prefers Safe.
...
If the decision theory she is using endorses the Allais preferences, then 
...
Every probability in $\IP$ rules out at least one of the E-Admissible options as impermissible. For example, any $p(\textit{Black})>\nicefrac{7}{30}$ rules 1A as excluded, i.e., not in $\EU_p$, but there's some positive chance that $\sigma_{D_1}$ picks 1A, by the assumption that it is regular for $\EAd_\IP$. 
Thus, $\sigma$ does not pick for $\EU_p$. It also does not even $\mu$-surely pick for $\EU_p$ if we assume that each $\mu$ assigns positive probability to both $D_1$ and $D_2$. It is thus not E-Admissible. 
...
Since $\Gamma$-Maximin is a more restrictive theory than Maximality, for a strategy to be $\Gamma$-Maximin there must be some $p\in\IP$ for which it $\mu^*$ surely pick for \todo{eg here should we replace?? I did so... changed order of quantifier}$\EU_p$, although such strategies may nonetheless be impermissible according to $\Gamma$-Maximin, as we saw, for example, in the Ellsberg case (\cref{sect:gamma}) where the only strategy compatible with $\Gamma$-Maximin was judged impermissible by the theory itself. 
	\begin{proposition}\label{thm:gamma-nu}Suppose $\N$ is $\EU$-complete. Suppose that 
		$\IB$ has the form $\{p\times \mu^*\given p\in \IP\}$ and $\mu^*$ requires almost everywhere decisiveness. Then if $\sigma \in \Maximin_\IB(\N)$, $ \sigma \textit{ $\mu^*$-surely picks for $\EU_p$, for some probability $p$.}$
