...
Suppose $\Nu$ is $\EU$-complete. Suppose that 
		...
		Suppose further that for all $p\in\IP$, $\mu^*\bigl\{D\in\Decs\given \Gamma_\IP(D)\not \subseteq\EU_{p}(D)\bigr\}>0$. Then if $\n$ is regular for $\Maximin_\IP$ then $\n\notin \Maximin_\IB(\Nu)$.
...
Suppose $\Nu$ is $\EU$-complete. Suppose that 
		...
		Suppose further that for every probability $p$, $\mu^*\{D\given \Maximin_\IP(D)\subseteq\EU_p(D)\}<1$. 
		Then if $\n$ is regular for $\Maximin_\IP$ then $\n\notin \Maximin_\IB(\Nu)$.
