...
{\color{violet}
%
Here are the four gambles over which the Allais preferences are defined, with the payout of the actions depending on, lets say, the outcome of a lottery with 100 tickets. \todo{gamble vs action??}
%$$
%\begin{array}{r|cccc}
...
\end{array}
$$
The empirically observed preferences are preferring 1A to 1B in $D^{\mathrm{Allais}}_1$ and 2B to 2A in $D^{\mathrm{Allais}}_2$.
% $\text{1A} \succ \text{1B}$ and $\text{2A} \prec \text{2B}$. 
Let's suppose that our agent has a decision theory which endorses these preferences, given parameters such as her credences and utilities.
...
The choice between picking functions is another kind of decision problem, a higher-order decision. She can make use of her decision theory to determine which picking function to choose, depending on her credences over which decision problem she'll be faced with, and the utility that the picking strategy leads to in each possibility. We will evaluate picking strategies by their fruits: by the utility of what they select when faced with the specified decision problem and given the state of the world.\todo{add an assumption dec-state indep?} 
...
%This is, \emph{prima facie}, a bad-making feature of a decision theory. 
It is self-undermining, in the sense that given this particular precise probability distribution over the possible decision problems she might face ($11\%$ $D^{\mathrm{Allais}}_{\mathrm{local}}$; $89\%$ sure-\pounds1m), if she applies her decision theory to the question of which picking strategy to use, it rules out as impermissible its own recommended course of action. 




This was all based on the assumption not only that her decision theory endorsed the Allais preferences, but also that it recommended Risky over Safe in $D^{\mathrm{Allais}}_{\mathrm{local}}$. If instead she preferred Safe over Risky in $D^{\mathrm{Allais}}_{\mathrm{local}}$, we can instead consider a different decision problem: Suppose that whilst she still thinks that her credence that she'll be offered $D^{\mathrm{Allais}}_{\mathrm{local}}$ is $11\%$, she thinks that if she is not given it, then she is given nothing. 
Now, she is facing the following decision problem over which picking strategy to select. 
$$
\begin{array}{c|ccc}
	\multirow{2}{*}{\text{Version 2}}	&\multicolumn{2}{c}{\text{choice offered}}&\text{choice not offered}\\
...
What if, however, her decision theory has neither a strict preference for Safe over Risky or Risky over Safe in $D^{\mathrm{Allais}}_{\mathrm{local}}$. Neither way of picking is deemed impermissible. Both picking strategies are compatible with her decision theory. Whilst which picking strategy she deems optimal changes depending on version 1 or version 2, in neither case does she rule out all the picking strategies which are compatible with her recommendations, she just rules out one of them. Whilst this is some sort of underminingness, it is a much weaker one that what we had previously, where it rules out as impermissible all picking strategies compatible with its recommendations (there is only one). \todo{what do we actually say about this?? EAdmiss??}
...
So then, in this decision, she will prefer Risky-sweetened to Safe:
$$
\begin{array}{r|cc}
 D^{\mathrm{Allais}}_{\mathrm{local-sweetened}} & \text{goes bust} &  \text{not bust}  \Bstrut\\\hline
	p & \sfrac{1}{11} & \sfrac{10}{11}\Bstrut\\\hline\hline\Tstrut
	\text{Safe} &\text{\pounds 1m}&\text{\pounds 1m}                      \\
	\text{Risky-sweetened} & \text{\pounds 1}&\text{\pounds 5m+\pounds 1}
\end{array}
$$ For some small enough sweetening, it is plausible that she will still prefer 1A to 1B-sweetened (this is an Archimadeanicity principle):
$$
\begin{array}{r|cccc}
...
{\todo{this needs fixing for above. But haven't we said it all above anyway? What's this section adding? }Second: Relatedly, for someone who uses the decision theory, it gives contradictory advice at different points. Recall the case above in which you were initially uncertain whether you'd face $D_1$ or $D_2$.
