...
The set $\S$ of all deterministic picking strategies discussed in the earlier part of the paper is EU-complete, and so we obtain the result that we hinted at in \cref{sect:Max}, namely, that if we have a $\IB$ that satisfies the \todo{???}, then it is only $\EU_p$ strategies that are in $\Maximality_\IB(\S)$.
...
\end{align}
\todo{Reference monotone convergence theorem from simple functions? Better to find a reference though... } 
