...
\begin{colored}{violet}
	...
	We will first construct a $\sigma$-algebra on $\A$, which will be defined relative to a utility function $\U:\A\to\Re^\Omega\cong\Re^n$. We consider the standard Borel $\sigma$-algebra on $\Re^n$, and assume that $\U(\A)$ is a Borel set. We then equip $\A$ with the pull-back of this $\sigma$-algebra, $\mathcal{B}(\A)=\Set{\U^{-1}(B)\given B\in\mathcal{B}(\Re^n)}$. This implies that any measurable function cannot distinguish acts with the same utility profiles.    By assumption that $\U(\A)$ is Borel, this is a standard Borel space. 
