...
\begin{colored}{violet}
	...
	Throughout the paper, we have ignored the question of measurability. We have considered various probability measures, $p$ a probability on $\Omega$, $\mu$ a probability on $\Decs$, or $\pb$ a probability on $\Omega\times\Decs$. To make this precise, we should fix the $\sigma$-algebras on each of these spaces.
...
%	We then require any probability $p$$\mu$ on $\Decs$ to be a probability measure on $(\Decs,\Sigma)$; and $\pb$ to be a probability measure on $(\Omega\times\Decs,\wp({\Omega})\otimes \Sigma)$
	...
	All the results in this paper are restricted to measurable picking strategies $\s:\Decs\to\A$, or measurable probabilistic picking strategies $\prpickstrat:\Decs\to\Delta(\A)$ so that the induced payoffs, $\U(\s),\U(\prpickstrat):\Omega\times\Decs\to\Re$, are measurable random variables with respect to $(\Omega\times\Decs,\wp({\Omega})\otimes \Sigma)$, recalling that $\U(\prpickstrat)(\omega,D):=\Exp_{\prpickstrat_D}[\U(\cdot)(\omega)]$.
...
For $b$, a probability measure on $(\Omega\times\Decs,\wp({\Omega})\otimes \Sigma)$, we assumed we had $b(\cdot | D)$, probability functions on $\Omega$, defined for $b_\Decs$-almost every $D$. The important feature that our results rely on is that for bounded (or integrable) measurable random variables $X:\Omega\times\Decs\to\Re$, (in particular, for our $\U(\s)$ or $\U(\prpickstrat)$), $\Exp_b[X]=\Exp_{b_\Decs}[\Exp_{b(\cdot|D)}[X]]$.

When $\Decs$ is supported on a finite subset of $\Decs$, we can just define $b(\cdot | D)$ by the ratio formula for any $D$ with $b_\Decs(D)>0$. 

In the more general setting, we need to make use of further machinery. Under our imposed $\sigma$-algebra, $(\Omega\times\Decs,\wp({\Omega})\otimes \Sigma)$ is a standard Borel space, and so by the Regular Conditional Probability theorem guarantee the existence of $b(\cdot|D)$ for $b_\Decs$ almost every $D$ which is a probability measure on $\Omega$ and such that for every bounded (or integrable) random variable $X$, the law of total expectation holds: $\Exp_b[X]=\Exp_{b_\Decs}[\Exp_{b(\cdot|D)}[X]]$.
