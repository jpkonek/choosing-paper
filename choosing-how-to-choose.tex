...
and, more generally, 
$$\prpickstrat \in \EU_{b}(\PrPickStrategies) \Leftrightarrow \prpickstrat \text{ $b_\Decs$-surely picks for $\EU_{b(\cdot|-)}$.}$$
\end{proposition}
...
If $\prpickstrat$ $\mu$-surely picks for $\EU_p$ for some $p\times \mu\in \IB$, then $\prpickstrat\in\EAd_\IB(\PrPickStrategies)$. More generally, if for some $b\in\IB$, $\prpickstrat$ $b_\Decs$-surely picks for $\EU_{b(\cdot|-)}$, then $\prpickstrat\in\EAd_\IB(\PrPickStrategies)$. 

Moreover, these are the only members of $\EAd_\IB$, at least assuming that $\PrPickStrategies$ is $\EU$-complete: 
$$\prpickstrat \in \EAd_\IB(\PrPickStrategies) \Leftrightarrow \text{for some $b$ in $\IB$, $\prpickstrat$ $b_\Decs$-surely picks for $\EU_{b(\cdot|-)}$.}$$
...
\begin{proposition}\label{thm:max-nu-suff}
		...
		More generally, if for some $b\in \IB$, $\prpickstrat$ $b_\Decs$-surely picks for $\EU_{b(\cdot|-)}$, then $\prpickstrat\in\Maximality_\IB(\PrPickStrategies)$. 
\end{proposition}
...
%	Using this, we get a result paralleling that for precise picking strategies: to avoid being ruled out as impermissible, one's strategy needs to $\mu$-surely be an $\EU_p$ strategy for some $p\times\mu\in\IB$. (Or equivalently for the conditional version.)
	Suppose $\imprecpickstrat$ is a set of picking strategies, and there is no $p \times \mu$ in $\IB$ such that all strategies in $\imprecpickstrat$ $\mu$-surely pick for $\EU_p$; then each probability $b=p\times \mu$ evaluates the precise picking strategy $\{\prpickstrat^p\}$, where $\prpickstrat^p$ picks for $\EU_p$, to be better, in this sense: for every $\prpickstrat\in\imprecpickstrat$, $\Exp_p[\U(\prpickstrat)]\leq \Exp_p[\U(\prpickstrat^p)]$, and there is some $\prpickstrat\in\imprecpickstrat$ with $\Exp_p[\U(\prpickstrat)]<\Exp_p[\U(\prpickstrat^p)]$. 
%	 
...
%	 	and thus, $b=p\times\mu$ evaluates the precise picking strategy $\{\prpickstrat^p\}$ to be better, in this sense; so $\imprecpickstrat$ is deemed impermissible by this extension of E-Admissibility.  
...
For $b$, a probability measure on $(\Omega\times\Decs,\mathcal{F}\otimes \Sigma)$, we assumed we had appropriate conditional probability functions, $b(\cdot | D)$, on $\Omega$, defined for $b_\Decs$-almost every $D$. The important feature that our results rely on is that for bounded measurable random variables $X:\Omega\times\Decs\to\Re$, the law of total expectation holds: $\Exp_b[X]=\Exp_{b_\Decs}[\Exp_{b(\cdot|D)}[X]]$. We will in particular apply this to our $\U(\s)$ or $\U(\prpickstrat)$, which are bounded, by assumption that $\U$ is bounded.\todo{We could probably instead remove it as a *background* assumption?? Just as we restrict to measurable $\s$ or $\prpickstrat$, we could restrict to bounded or integrable strategies?? (though ``integrable'' is relative to a measure...) Anyway, I don't think we shoudl change this, but just to note it. }
...
\begin{theorem}\label{thm:EU appendix} 
	\hspace*{1mm}
	\begin{enumerate}[label=\normalfont(\roman*), ref=\roman*]
		\item  If $\prpickstrat$ $b_\Decs$-surely picks for $\EU_{b(\cdot|-)}$ then, for any $\prpickstrat'$, $\Exp_b[\U(\prpickstrat)]\geq\Exp_b[\U(\prpickstrat')]$
		\item  If $\prpickstrat$ $b_\Decs$-surely picks for $\EU_{b(\cdot|-)}$ and $\prpickstrat'$ does not, then $\Exp_b[\U(\prpickstrat)]>\Exp_b[\U(\prpickstrat')]$.
	\end{enumerate}
\end{theorem}
\begin{proof}
...
Thus $\Exp_{b(\cdot | D^*)}[\U(\prpickstrat)]=\Exp_{\nu_{D^*}}[\Exp_{b(\cdot | D^*)}[\U]]$ is maximised when $\prpickstrat_{D^*}(\EU_{b(\cdot|-)}(D^*))=1$. 

And so $\Exp_{b}[\U(\prpickstrat)]=\Exp_{b_\Decs}[\Exp_{b(\cdot | D)}[\U(\prpickstrat)]]$ is maximised when $\prpickstrat_{D^*}(\EU_{b(\cdot|-)}(D))=1$ for $b_\Decs$-almost every $D$. 
...
%
%Thus, $\Exp_{b}[\U(\prpickstrat)]=\Exp_{b_\Decs}[\Exp_{b(\cdot | D)}[\U(\prpickstrat)]]$ is maximised just when for $b_\Decs$ almost every $D$,  $\prpickstrat_{D^*}(\EU_{b(\cdot|-)}(D))=1$, which is to say that $\prpickstrat$ $b_\Decs$-surely picks for $\EU_{b(\cdot|-)}$. 
%
%
...
%$
%obtains its maximum at $\Exp_{b_\Decs}[t_b(D)]$ when $\prpickstrat_D(\EU_{b(\cdot|-)}(D))=1$ for $b_\D$-almost all $D$, i.e., when $\prpickstrat$ $b_\Decs$-surely picks for $\EU_{b(\cdot|-)}$.
\end{proof}
...
\end{definition}
	...
	A picking strategy $\s\in\S$ is then said to $b_\Decs$-surely pick for $\EU_{b(\cdot|-)}$ when \[b\{\seq{\omega,D}\given\omega\in\Omega,\, \s(D)\in \EU_{b(\cdot|-)}(D)\}=1\] i.e., for the marginal $b_\D$, $b_\D\{D\given \s(D)\in\EU_{b(\cdot|-)}(D)\}=1$. A probabilistic picking strategy $\prpickstrat$ is said to $b_\Decs$-surely pick for $\EU_{b(\cdot|-)}$ when \[b\{\seq{\omega,D}\given\prpickstrat_D(\EU_{b(\cdot|-)}(D))=1\}=1\]
...
\begin{proof}
	...
	{\allowdisplaybreaks
		\begin{align}
			\Exp_b[\U(\prpickstrat)]&=\sum_{\omega\in\Omega}\int_{\D} \U(\prpickstrat)(\omega,D) f(\omega,D) \frac{1}{|\Omega|}\diff\lambda(D)\\
			...
			&=\int_{\D'}\Exp_{b(\cdot|D)}[\U(\prpickstrat)(\omega,D)]\;f_\D(D)\frac{1}{|\Omega|}\diff\lambda(D)\\
			&=\sum_{\omega\in\Omega}\int_{\D'}\Exp_{b(\cdot|D)}[\U(\prpickstrat)(\omega,D)]\;f(\omega,D)\frac{1}{|\Omega|}\diff\lambda(D)\\
	...
	Thus, $\Exp_b[\U(\s)]\leq \Exp_b [t_b]$ with equality iff $\Exp_{b(\cdot|D)}[\U(\prpickstrat)(\omega,D)]=t_b(\omega,D)$ almost everywhere, which is true iff $b\{\left<\omega,D\right> \in \Omega\times\D \given \s(D)\in \EU_{b(\cdot|-)}(D)\}=1$, i.e., iff $\s$ $b_\Decs$-surely picks for $\EU_{b(\cdot|-)}$. Both (i) and (ii) follow immediately from this.
\end{proof}
...
This is because $\Exp_b[\U(\prpickstrat)]=\Exp_{b_\D}\Exp_{b(\cdot|D)}\U(\prpickstrat)$, where $b_\D$ is the marginal. Each $\Exp_{b(\cdot|D)}\U(\prpickstrat)$ is maximised when $\prpickstrat_D(\EU_{b(\cdot|D)}(D))=1$, at least, whenever this is well-defined. So, similarly, $\Exp_b[\U(\prpickstrat)]$  is maximised when $b_\D\{D\given \prpickstrat_D(\EU_{b(\cdot|D)}(D))=1\}=1$.
\end{comment}
...
\begin{lemma}\label{thm:ead to eu appendix}
	$a\in\EAd_\IP(D)$ iff there is some $p\in\IP$ such that $a\in\EU_p(D)$.  
	Thus, any $\s$ which picks for $\EU_p$ for some $p\in\IP$ also picks for $\EAd_\IP$; and similarly for $\prpickstrat$.

More generally, 
	$a\in\EAd_\IB(\cdot|-)(D)$ iff there is some $b\in\IB$ such that $a\in\EU_b(D)$.  
Thus, any $\s$ which picks for $\EU_{b(\cdot|-)}$ for some $b\in\IB$ also picks for $\EAd_\IB(\cdot|-)$; and similarly for $\prpickstrat$. 

Also, 
	$\s\in\EAd_\IB(\S)$ iff there is some $b\in\IB$ such that $\s\in\EU_b(\S)$. 
	and 
		$\prpickstrat\in\EAd_\IB(\PrPickStrategies)$ iff there is some $b\in\IB$ such that $\prpickstrat\in\EU_b(\PrPickStrategies)$ 
\end{lemma}
