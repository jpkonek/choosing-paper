...
Now we want to consider how E-Admissibility judges picking strategies.  
This depends on your uncertainty over which decision problem you'll face. 
...
By \cref{thm:eu-self-rec}, for any $b\in\IB$ which has the form $p\times\mu$, if $\s$ picks for $\EU_p$ then it is in $\EU_{b}(\S)$, and thus, is in $\EAd_\IB(\S)$. Also, when $p\in\IP$ any $\s$ which picks for $\EU_p$ also picks for $\EAd_\IP$. We thus typically have some strategy which both picks for $\EAd_\IP$ and is in $\EAd_\IB$, unlike for $\Gamma$-Maximin. 
 \begin{proposition}\label{thm:ead-suff-indep}
	...
	If there exists some $p\in\IP$ with some $\mu$ such that $p\times\mu\in\IB$, then there are some strategies which pick for $\EAd_\IP$ and are in $\EAd_\IB(\S)$. 
	
	If for every $p\in\IP$ there is some $\mu$ such that $p\times\mu\in\IB$, then for every $D\in\Decs$, if $a\in\EAd_\IB(\S)$ there is some $\s$ such that $\s(D)=a$ and $\s\in\EAd_\IB(\S)$. 
...
\begin{colored}{violet}
	...
	If there exists some $p\in\IP$ with some $\mu$ such that $p\times\mu\in\IB$, then there are some strategies which pick for $\Maximality_\IP$ and are in $\Maximality_\IB(\S)$. 
	
	If for every $p\in\IP$ there is some $\mu$ such that $p\times\mu\in\IB$, then for every $D\in\Decs$, if $a\in\Maximality_\IB(\S)$ there is some $\s$ such that $\s(D)=a$ and $\s\in\Maximality_\IB(\S)$. 
...
\begin{definition}
	...
	\begin{proposition}\label{thm:ead-equiv[dep]}
		...
		Thus, there is some $\s$ which picks for  $\EAd_{\IB(\cdot|-)}$ and is in $\EAd_\IB(\Strategies)$, namely any $\s$ which $b_\Decs$-surely picks for $\EU_{b(\cdot|-)}$.
	\end{proposition}
\end{colored}
	Thus whilst some of its picking strategies are not ruled out, some will be, at least if $\EAd_{\IB(\cdot|-)}$ is not a restriction of some precise choice function. 
...
Let $f_\omega$ be a measure on $\Decs$ given by $f_\omega(E)=b(\{\omega\}\times E)$ for $E\subseteq \Decs$ measurable. Then $b_\Decs=\sum_{\omega\in\Omega}f_\omega$. As 
...
We have $b_\omega(E)=\int_{E}b(\omega |D) b_\D(\diff D)$ for $E\subseteq \Decs$. 
...
\begin{align}
	...
	&=\sum_\omega\int_\D X(\omega,D) b_\omega (\diff D)\\
		&=\sum_\omega\int_\D X(\omega,D) b(\omega |D) b_\D(\diff D)&&\text{integration by parts?? - check!}\\
