...
We can give some sufficient conditions for strategies that pick for $\EAd_\IP$ but are rejected by $\EAd_\IB$: 
...
If there is a selection of events $E_q\subseteq\Decs$ for each $q\in\IP$ (some of which may be empty) which are pairwise disjoint and such that for all $p\times\mu\in\IB$, 
\[
...
The set $\S$ of all deterministic picking strategies discussed in the earlier part of the paper is EU-complete, and so we obtain the result that we hinted at in \cref{sect:Max}, namely, that if we have a $\IB$ that satisfies the , then it is only $\EU_p$ strategies that are in $\Maximality_\IB(\S)$.
...
%\end{tabularx}
	...
	In fact, if there are some $a,a'$ which have the same utility profile, $\U(a)=\U(a')$, then $d(a,a')=0$, so it is actually a pseudo-metric. Any measurable notions will treat such acts equivalently and it is Borel isomorphic to the structure identifying any two such acts.
	
	We can then use the Hausdorff metric on $\Decs$ to again obtain our associated Borel $\sigma$-algebra on $\Decs$.
	  \[
	d(D, D') = \max\Bigl\{
...
As each $b(A|D)$ is well defined on $b_\Decs$ almost every $D$ and there are only finitely many $A\subseteq\Omega$, $b(\cdot|D)$ is still well-defined on $b_\Decs$ almost every $D$. It is also probabilistic by the linearity of Radon Nikodym derivatives.
