...
%\end{tabularx}
	...
	We can then use the Hausdorff (pseudo-)metric on $\Decs$ to again obtain our associated Borel $\sigma$-algebra on $\Decs$.\footnote{	Note that this is identical to the Vietoris, or hit-and-miss, topology which is generated by the subbasis consisting of $\{D\in \Decs\given D\cap U\neq \emptyset\}$ and $\{D \in\Decs\given D\subseteq U\}$ for $U$ open subset of $\A$. }\todo{I moved the equiv to Vietoris into ftnte as no longer using it in the pf}
	  \[
	d(D, D') = \max\Bigl\{
	...
	In fact, if we assume that $\U(\A)$ is a Borel subset of $\Re^n$, then both $\A$ and $\Decs$ are standard Borel.\todo{delete the below pf as we don't use it??}
	 \begin{colored}{red}
...
These need to have $b(A\times E)=\int_{D\in E}b(A|D)b_\Decs(\diff D)$ for all $A$ measurable $\subseteq\Omega$ and $E$ measurable $\subseteq\Decs$, so we have the law of total expectation for bounded measurable random variables, $\Exp_b[X]=\Exp_{b_\Decs}[\Exp_{b(\cdot|D)}[X]]$. We will then apply this to our $\U(\s)$ or $\U(\nu)$. 

\begin{colored}{red}
	The important feature that our results rely on is that for bounded measurable random variables $X:\Omega\times\Decs\to\Re$, the law of total expectation holds: $\Exp_b[X]=\Exp_{b_\Decs}[\Exp_{b(\cdot|D)}[X]]$. We will in particular apply this to our $\U(\s)$ or $\U(\n)$, which are bounded, by assumption that $\U$ is bounded.
\end{colored}
\todo{We could probably instead remove it as a *background* assumption?? Just as we restrict to measurable $\s$ or $\n$, we could restrict to bounded or integrable strategies?? (though ``integrable'' is relative to a measure...) Anyway, I don't think we shoudl change this, but just to note it. }
...
Let $\Omega_b:=\Set{\omega\given b(\{\omega\}\times\Decs)>0}$, and note that $b(\Omega_b\times\Decs)=1$. For any $\omega\in \Omega_b$, let $\beta_\omega$ be the measure on $\Decs$ given by $\beta_\omega(E)=b(\{\omega\}\times E)$ for $E\subseteq \Decs$ measurable. 
...
\subsection{Underminingness of E-Admissibility \\(\cref{thm:ead-existsimpermissible[indep],thm:ead-existsimpermissible[dep],thm:ead-nu-reg-nec})}
...
In these cases, we have distinct decisions and probabilities such that every probability function disagrees with the recommendations of $\EU_{p^*_i}$ on $D_i$, and the $D_i$ have positive measure according to every $\mu$. More generally:
...
\begin{theorem}
	Suppose $\IB$ is non-singleton and there is a selection of disjoint $E_b\subseteq\Decs$ for each $b\in\IB$ (some possibly empty), such that for all $b\in\IB$, $$b_\Decs\left(\bigcup_{b'\in\IB}\{D\in E_{b'}\given \EU_{b(\cdot|-)}(D)\cap \EU_{b'(\cdot|-)}(D)=\emptyset\}\right)>0.$$
...
\todo{is there a way of combining the below two? What's special about the spreads of probabilities here?}
...
Suppose $V_1$ and $V_2$ are disjoint non-empty subsets of $[l,h]^\Omega$ such that for all $g\in \Re^\Omega$ there is some $\lambda_1$ such that $\lambda_1 g\in V_1$ and $\lambda_2>0$ such that $\lambda_2 g\in V_2$. 
...
If $\U(a)\in \{g\in V_i\given \Exp_p[g]>0>\Exp_{p^*_i}[g]\}$, then $\EU_p(D)\cap\EU_{p^*_i}(D)=\emptyset$. 

Thus, by making use of our assumption on $\mu$, $$\mu(\{\{a,0\}\given \U(a)\in V_i\text{ and }\EU_p(\{a,0\})\cap\EU_{p^*_i}(\{a,0\})=\emptyset\})>0.$$


%So, for every $(p,\mu)\in\IB$, $\mu\{\{a,0\}\given \U(a)\in\{g\in V_i\given \Exp_p[g]>0>\Exp_{p^*_i}[g]\} \}>0$, by our assumption.
%
%
%Also, if $D=\{a,0\}$ with $\U(a)\in V_i$ and $\Exp_p[\U(a)]>0>\Exp_{p^*_i}[\U(a)]\}$, then $\EU_p(D)\cap\EU_{p^*_i}(D)=\emptyset$. 
%
%So also $$\mu(\{\{a,0\}\given \U(a)\in V_i\text{ and }\EU_p(\{a,0\})\cap\EU_{p^*_i}(\{a,0\})=\emptyset\})>0.$$

This then satisfies the conditions of \cref{thm:appendix:ead:existsimpermissibe:conditiontwoprobs[indep]}, putting $E_i=\{\{a,0\}\given a\in V_i\}$.
\end{proof}
...
\begin{proof}
	...
	Take any $p_1,p_2$ distinct members of $\IP$. This lemma then lets us check the conditions of 	
