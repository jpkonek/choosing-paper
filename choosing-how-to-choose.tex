...
\end{align}
where $\sigma(D)$ is the $\sigma$-field generated by $D$, $\Exp_{b}[X | \sigma(D)]$ is defined as the orthogonal projection of $X$ onto the linear subspace of $\sigma(D)$-measurable random variables, and $\Exp_{b}[X | D]$ is the value of this random variable at $D$. (This extends by continuity to $X\in L^1(b)$.)
