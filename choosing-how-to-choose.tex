...
\begin{proposition}\label{thm:ead-existsimpermissible[dep]:spreadegtakeorleave}%\todo{why am I now using g? We were using X as rv, althoguh that was for rv on OmegaxDecs}
	...
	Suppose $\IP$ is non-singleton and for every $p \times \mu\in\IB$, for every open non-empty subset $U \subseteq\A$, $\mu$ assigns strictly positive measure to $\{\{a,0\}\given a\in U\}$.

Then there is $\s$ that picks for $\EAd_\IP$ but there is no $p\times\mu\in \IB$ such that $\s$ $\mu$-surely pick for $\EU_p$.
\end{proposition}
\begin{proof}
...
\begin{proof}
	...
	By assumption on $V$, there is some $\lambda >0$ with $\lambda X\in V$. Also $\Exp_p[\lambda X]>0>\Exp_{q}[\lambda X]$. 
...
\begin{proof}
	...
	\begin{proof}
		...
		As in \cref{thm:ead-existsimpermissible[indep]:CCM-1:nondegentakeleave}, we can find some $X^*$ such that $\Exp_{p}[X^*]>0>\Exp_{q}[X^*]$, and, moreover, can choose a scalar small enough so that $\U(a_0)+\lambda X^*\in[l,h]^n$ and thus that $a_0+\lambda X^*\in\A$ (abusing notation). 
		...
		As $\U(a_0)+\lambda X^*\in V$ by choice of $\lambda$, $D^*\subseteq V$, so $D^*\in E$, as required. 
