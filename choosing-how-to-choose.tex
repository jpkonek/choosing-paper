...
Second: Relatedly, for someone who uses the decision theory, it gives contradictory advice at different points. Recall the case above in which you were initially uncertain whether you'd face $D^\mathrm{Allais}_\mathrm{local}$ or get nothing for sure, and your decision theory would choose Safe over Risky when faced with $D^\mathrm{Allais}_\mathrm{local}$.
In that state of uncertainty, you prefer the picking strategy $\s_\mathrm{Risky}$, since the choice between $\s_\mathrm{Safe}$ and $\s_\mathrm{Risky}$ is extensionally equivalent to the choice between 2A and 2B in $D^\mathrm{Allais}_2$, and you prefer 2B to 2A there. And yet, when you face $D^\mathrm{Allais}_\mathrm{local}$, you prefer to use $\s_\mathrm{Safe}$, for it picks what your decision theory says you should pick. And so which should you actually use? Which of these perspectives is privileged? Standing facing the decision problem $D^\mathrm{Allais}_\mathrm{local}$, you might ask yourself: my decision theory tells me to choose Safe and not to choose Risky, but it also tells me that, if I could have chosen how I would choose, it would have told me to choose Risky and not to choose Safe---which should I do?
...
%\begin{colored}{red}
%\todoinfo{ORIGINAL}
Another way the defender of these theories could argue against our claim of self-underminingness: they can argue that we have essentially assumed Bayesian conditionalization and argue against conditionalization. 
