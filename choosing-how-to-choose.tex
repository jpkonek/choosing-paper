...
This requires both that one's credal state is imprecise and that each $\mu$ sees a requirement to coordinate (and, moreover, that these requirements are coordinated).
...
\begin{proposition}\label{thm:ead-existsimpermissibel[indep]}
	If there exist some (distinct) $q_1,q_2\in\IP$ and disjoint $E_1,E_2\subseteq\Decs$ such that for all $(p,\mu)\in\IB$ there is at least one of $q_1$ or $q_2$ such that
	\[
...
We can also show it if each $\mu$ is sufficiently spread. For example, if each $\mu$ assigns strictly positive measure to every collection of take-it-or-leave-it decisions generated by a non-empty open subset of $\A$, that is $\mu(\{\{a,0\}\given a\in V\})>0$ for $V$ open non-empty subset of $\A$; or if each $\mu$ assigns strictly positive measure to every non-empty open set of decisions. 
...
\end{colored}
	Whilst some of its picking strategies are not ruled out, some will be, at least if $\EAd_{\IB(\cdot|-)}$ is not a restriction of some precise choice function, $\EU_{b(\cdot|-)}$.
		\begin{proposition}\label{thm:ead-existsimpermissibel[dep]}
...
an analogue of \cref{thm:ead-existsimpermissibel[dep]} unless we impose additional particular restrictions on $\IB$ (\cref{sect:nu:Max}).
...
\subsubsection{E-Admissibility}
\Cref{thm:ead-equiv[dep],thm:ead-existence[dep],thm:ead-existence[indep],thm:ead-existsimpermissibel[indep],thm:ead-existsimpermissibel[indep]} also generalise to the probabilistic picking strategy setting. 
...
\begin{proof}
	...
	For \cref{itm:eu-2}, take $\nu_b$ which does pick for $\EU_{b(\cdot|-)}$. \Cref{thm:EU appendix} implies that $\Exp_b[\U(\nu^b)]>\Exp_b[\U(\nu)]$, so $\nu\notin\EU_b(\Nu)$. 
\end{proof}
...
\subsection{Underminingness of E-Admissibility \\(\cref{thm:ead-existsimpermissibel[indep],thm:ead-existence[dep],thm:ead-nu-reg-nec})}

We will now move to \cref{thm:ead-existsimpermissibel[indep],thm:ead-existence[dep],thm:ead-nu-reg-nec}. For these results, we need to moreover give conditions under which there exists a strategy which picks for E-Admissibility and does not looks like an expected utility strategy. 
...
\begin{proposition}\label{thm:appendix:ead:existsimpermissibe:conditiontwoprobs[indep]}
...
%there is some $\{p^*_i\given i\in I\}\subseteq\IP$ (which may be uncountable) such that for all $(p,\mu)\in\IB$, there is some $J\subseteq I$ such that $\mu(\bigcup_{i\in J}\mu\{D\in E_i\given \EU_p(D)\cap\EU_{p^*_i}(D)=\emptyset\})>0$. The proof is exactly the same: putting $\s(D)\in\EU_{p^*_i}(D)$ for $D\in E_i$ and verifying that this condition ensures that it does not $\mu$-surely pick for $\EU_p$. 

\begin{theorem}
	There is a selection of disjoint $E_p\subseteq\Decs$ for each $p\in\IP$, such that for all $(p_0,\mu_0)\in\IB$, $$\mu_0\left(\bigcup_{p\in\IP}\{D\in E_p\given \EU_p(D)\cap\EU_{p_0}(D)=\emptyset\}\right)>0.$$
\end{theorem}
...
So, for every $(p,\mu)\in\IB$, $\mu\{\{a,0\}\given \U(a)\in\{g\in V_i\given \Exp_p[g]>0>\Exp_{p^*_i}[g]\} \}$
\end{proof}
...
\begin{proof}
	...
	\cref{thm:appendix:ead:existsimpermissibe:conditiontwoprobs[indep]}
\end{proof}




So we have proved \cref{thm:eu-self-rec,thm:eu-uniquely-optimal,thm:eu-nu-nec-suff,thm:eu-dep,thm:ead-equiv[indep],thm:ead-existence[dep],thm:ead-existsimpermissibel[indep],thm:ead-nu-nec-suff,thm:ead-nu-reg-nec,thm:ead-existence[indep],thm:max-suff,thm:max-suff[dep],thm:max-nu-suff,thm:ead-equiv[dep]}
...
%		
	...
	For any 
	 ...
	 \cref{thm:ead-existsimpermissibel[indep]:CCM-1}.	
\end{proof}



\begin{theorem}[CCM]\label{thm:ead-existsimpermissibel[indep]:reg}
	Completely independent. 
