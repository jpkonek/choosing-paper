...
\begin{enumerate}
\item $(\forall p\times\mu_p\in\IB)(\exists E_p\subseteq\Decs)\mu_p(E_p)>0$, 
\item $(\forall p,q\in\IP: p\not=q)E_p\cap E_q=\varnothing$
\item $(\forall p\times\mu_p\in\IB)(\forall X\in E_p)(\exists f_X\in\EAd_\IP(X))(\exists g_X\in X)$
\[
(\forall E^\prime\subseteq E_p)\left[\posi(\left\{f_X-g_X\given X\in E^\prime\right\})\cap \mathbb{R}^n_{<0}=\varnothing\;\Rightarrow\;\mu_p(E^\prime)=0\right]
\]
...
For any $p\times\mu_p\in\IB$ and $X\in E_p$, let $\s(x)=f_X$. For $X\notin \cup_{p\in\IP}E_p$, let $\s(X)$ be anything in $\EAd_\IP(X)$.  So $\s$ picks for $\EAd_\IP$. 


	Choose any $p\times\mu_p\in\IB$. Suppose for contradiction that $\s$ $\mu_p$-surely pick for $\EU_p$. Then $f_X\in\EU_p(X)$ $\mu_p$-a.e. on $E_p$, or equivalently, $f_X\in\EU_p(X)$ on $E^\prime$ for some $E^\prime\subseteq E_p$ with $\mu_p(E_p)=\mu_p(E^\prime)>0$. In that case,
\[
\posi(\left\{f_X-h\given X\in E^\prime, h\in X\right\})\cap \mathbb{R}^n_{<0}=\varnothing
\]
since	 $p(f_X-h)\geq0$ for all $X\in E^\prime, h\in X$. But 
\[
\left\{f_X-g_X\given X\in E^\prime\right\}\subseteq\left\{f_X-h\given X\in E^\prime, h\in X\right\}.
\]
So we must have 
\[
\posi(\left\{f_X-g_X\given X\in E^\prime\right\})\cap \mathbb{R}^n_{<0}=\varnothing
\]
But then by assumption $\mu_p(E^\prime)=0$ which is a contradiction.
