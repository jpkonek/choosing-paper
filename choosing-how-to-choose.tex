...
\subsubsection{E-Admissibility and EU equivalences}
...
\end{proof}
\cref{thm:ead-suff-indep} follows from this, using the fact that $\S$ is $\EU$-complete so we can always select such an $\s$. Same for the second part of \cref{thm:ead-equiv[dep]}
...
\subsubsection{Underminingness of E-Admissibility}

\begin{proposition}\label{thm:ead to eu appendix}
	$a\in\EAd_\IP(D)$ iff there is some $p\in\IP$ such that $a\in\EU_p(D)$.  
	
	So, for any $p\in\IP$, $\EU_p(D)\subseteq\EAd_\IP(D)$, and thus, any $\s$ which picks for $\EU_p$ for some $p\in\IP$ also picks for $\EAd_\IP$; and similarly for $\n$.

More generally, 
	$a\in\EAd_{\IB(\cdot|-)}(D)$ iff there is some $b\in\IB$ such that $a\in\EU_{b(\cdot|-)}(D)$.  
Thus, any $\s$ which picks for $\EU_{b(\cdot|-)}$ for some $b\in\IB$ also picks for $\EAd_\IB(\cdot|-)$; and similarly for $\n$. 

Also, 
	$\s\in\EAd_\IB(\S)$ iff there is some $b\in\IB$ such that $\s\in\EU_b(\S)$. 
	and 
		$\n\in\EAd_\IB(\Nu)$ iff there is some $b\in\IB$ such that $\n\in\EU_b(\Nu)$ 
\end{proposition}
