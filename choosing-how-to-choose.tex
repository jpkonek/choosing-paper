...
%	 first consider the product sigma algebra $\mathcal{B}(\A)^k$ on $\A^k$, giving $k$-tuples. We quotient this by permutations to obtain a 
%	we consider it as the countable disjoint union of $k$-tuples in $\Re^n$, modulo permutations. 
	Put $\Sigma$ as the $\sigma$-algebra generated by $\Set{D\in \Decs\given D\cap A\neq\emptyset}$ and $\Set{D\in \Decs\given D\subseteq A}$, for $A\in\mathcal{B}(\A)$. This is the finite-set Vietoris, or hit-and-miss topology, and it is a standard Borel space. 
%	Put $\Sigma$ as the smallest $\sigma$-algebra containing each $\{D\in\Decs\given \#(D\cap A)=k\}$ for $A\in\mathcal{B}(\A)$ and $k\in\mathbb{N}\setminus\{0\}$.\footnote{Equivalently, it can be seen as the Vietoris or hit-and-miss topology, generated by $\Set{D\in \Decs\given D\cap A\neq\emptyset}$ and $\Set{D\in \Decs\given D\subseteq A}$, for $A\in\mathcal{B}(\A)$.}
...
For  $\Omega\times\Decs$ we simply use $\mathcal{F}\otimes \Sigma$
