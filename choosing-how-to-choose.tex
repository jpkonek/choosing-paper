...
Suppose there is a selection of pairwise disjoint events $E_q\subseteq\Decs$, one for each $q\in\IP$ (some of which may be empty), such that for all $(p,\mu)\in\IB$, 
	\[
...
\begin{proposition}\label{thm:ead-existsimpermissible[dep]}
	Suppose there is a selection of events $E_q\subseteq\Decs$, one for each $q\in\IP$ (some of which may be empty), which are pairwise disjoint and such that for all $(p,\mu)\in\IB$, 
	\[
	\mu\left(\bigcup_{q\in\IP}\{D\in E_q\given \EU_p(D)\cap\EU_q(D)=\emptyset\}\right)>0
	\]
