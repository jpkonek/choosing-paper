...
\end{lemma}
\begin{proof}
	$\phi:\Decs\twoheadrightarrow\A$ by $\phi(D)=D$ is a weakly measurable correspondence: for $U$ open $\subseteq\A$, $\{D\given U\cap D\neq\emptyset\}$ is open in $\Decs$ \citep[3.91]{aliprantis2006infinite}.
	
	$\Exp_p[\U(a)]=\sum_{\omega\in\Omega}p(\omega)\U(a)(\omega)$ is a continuous function of $a$. Conceiving of it as a function from $\A\times\Decs\to\Re$ which doesn't depend on $D$, we also see that it is trivially measurable as a function of $D$. 
	
	So this follows from the Measurable Maximum Theorem \citep[Theorem 18.19]{aliprantis2006infinite}. 
\end{proof}
...
\begin{proof}
	...
	Then the existence of a measurable picking function then follows from the Measurable Maximum Theorem \citep[Theorem 18.19]{aliprantis2006infinite}. 
\end{proof}
...
And we know that  $\mu^*(E\cap S)=1$.
So, by a Dominated Convergence Theorem, as utilities are bounded, we have that
