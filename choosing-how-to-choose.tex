...
In fact, using this proposed notion of $p$ expecting an imprecise picking strategy to do better than another imprecise picking strategy, we get that the only imprecise picking strategies which are acceptable according to Maximality if one's credence over which decision she'll be faced with is given by a precise $\mu^*$ which requires almost everywhere decisiveness, are those each of whose members $\mu^*$ surely picks for some $\EU_p$.
Maximality thus avoids the charge of evaluating the imprecise picking strategy as impermissible. There can be imprecise picking strategies that are Maximal and which do not all pick for a single $\EU_p$, unlike for E-Admissibility, so long as they all pick for some $\EU_p$, at least if ones credence over decisions is given in such a way. 
\todoinfo{I rewrote this to include our more general theorem!!}
