...
\begin{proof}
	...
	Then put $\s(D)\in G(q,D,\epsilon_q)$ for $D\in E_i$, and $\s(D)\in \c(D)$ for $D\notin\bigcup_{i=1,\ldots,k} E_i$. 
	
	Observe that $\s(D)$ picks for $\c$ by definition. 
	
	Take any $p\in\Prob$. 
	
	There is some $i$ such that $p\in U_{q_i}$. This will entail $\s(D)\notin\EU_p(D)$ by this sublemma:
		\begin{sublemma}
	...
	\begin{proof}
		...
		\begin{align}
			...
			&\geq \epsilon_q -\|q-p\|\|\U(a^*)-\U(b)\|_\infty\\
			&\geq  \epsilon_q -\nicefrac{\epsilon_q}{2M}\times M=\nicefrac{\epsilon_q}{2}>0\qedhere
		\end{align}
	\end{proof}
