...
\begin{corollary}[CCM version of \cref{thm:ead-existsimpermissibel[indep]:RP-1}]\label{thm:ead-existsimpermissible[indep]:CCM-1:nondegentakeleave}
	...
	Then there is $\s$ which picks for $\EAd_\IP$ but there is no $p\times\mu\in \IB$ such that $\s$ $\mu$-surely pick for $\EU_p$.
\end{corollary}
\begin{proof}WLOG suppose $l<0<u$ and that $|l|,|u|>1$.  Otherwise work with transformed utilities. 
	
	Let $V_1:=\{a\in\A\given \sup\U(a)>1\}$ and $V_2:=\{a\in\A\given \sup\U(a)<1\}$.	Put $E_1:=\{\{a,0\}\given a\in V_1\}$ and $E_2:=\{\{a,0\}\given a \in V_2\}$. 
	
	
	Let $\mathrm{Disagree}(p,q):=\{a\in \A\given \Exp_p[\U(a)]>0\text{ and }\Exp_q[\U(a)]<0\}\cup\{a\in \A\given \Exp_p[\U(a)]<0\text{ and }\Exp_q[\U(a)]>0\}$. Note that it is open and contains $\lambda a$ for any $a\in\A$, at least so long as $\lambda a\in\A$. Also observe that $\EU_{q}(\{a,0\})\cap \EU_{p}(\{a,0\})=\emptyset$ iff $a\in \mathrm{Disagree}(p,q)$.
	
	Now take any $p_1,p_2$ distinct in $\IP$. 
	
	Suppose $q\neq p_1$.
	Take $\omega^*$ such that $q(\omega^*)\neq p_1(\omega^*)$. Let $r=\frac{q(\omega^*)+p_i(\omega^*)}{2}>0$ and consider the Dutch-book-style bet $a^*(\omega)=\begin{cases}
		1-r&\omega=\omega^*\\
		-r&\omega\neq \omega^*
	\end{cases}$ and observe that $a^*\in  \mathrm{Disagree}(p_1,q)$.
	
	Moreover, by choosing $\lambda>\nicefrac{1}{1-r}$, $\sup\U(\lambda a^*)>1$, and $\lambda a^*$ is still in $\mathrm{Disagree}(p_1,q)$, and thus $V_1\cap \mathrm{Disagree}(p_1,q)\neq\emptyset$. It is thus a non-empty open subset of $\A$ and so \begin{equation}
	...
	For $q=p_2$, it is distinct from $p_2$, so using an analogous argument we find some $a^*\in \mathrm{Disagree}(p_2,q)$, and then putting $\lambda<\nicefrac{1}{1-r}$ we have $\lambda a^*\in V_2\cap \mathrm{Disagree}(p_2,q)$, so it is a non-empty open subset of $\A$. Then observe that $\mu \{D\in E_2\given \EU_{p_2}(D)\cap \EU_p(D)= \emptyset\}>0$, as required. 
...
\end{corollary}
\begin{proof}		
	\begin{sublemma}Suppose $E$ non-empty open $\subseteq\Decs$. For distinct $q_1,q_2$, 
...
%		
		...
		As $E$ is open, for some $\epsilon>0$, $B_\epsilon(D_0)\subseteq E$. 
		
		As in \cref{thm:ead-existsimpermissible[indep]:CCM-1:nondegentakeleave}, there is some $a^*$ such that $a^*\in  \mathrm{Disagree}(q_1,q_2)$, and we can choose $a^*$ so that $\sup\{a^*(\omega)\}<\epsilon$. WLOG assume that $\Exp_{q_1}[\U(a^*)]>0$ and $\Exp_{q_1}[\U(a^*)]<0$. Note then that $\Exp_{q_1}[\U(a_0+a^*)]>\Exp_{q_1}[\U(a_0)]$ and $\Exp_{q_1}[\U(a_0+a^*)]<\Exp_{q_1}[\U(a_0)]$.
		
		Consider $D^*=D_0\cup\{a_0+a^*\}$ and observe that $D^*\in B_\epsilon(D_0)$. 
