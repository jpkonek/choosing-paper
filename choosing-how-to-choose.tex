...
\begin{definition}\label{def:cond}
	...
	$\EU_{\pb(\cdot|-)}$ given by $\EU_{\pb(\cdot|-)}(D)=\EU_{\pb(\cdot|D)}(D)$, when this is well-defined.
...
\end{definition}
When $\pb$ has the form $p\times\mu$, then $\pb_\Decs$ is just $\mu$ and, for every $D$, $\pb(\cdot|D)$ is just $p$. These are thus generalisations of our original notions of the choice function $\EU_p$ and a strategy $\s$ $\mu$-surely picking for $\EU_p$. 
...
We represent your uncertainty over $\Omega \times \D$ with a credal set, $\IB$, given by a set of probability functions, $\pb$, over $\Omega\times \D$. 
...
%	$\s\in\EAd_\IB(\S)$ iff there is some $\pb\in\IB$ such that for all $\s'\in\S$, $\Exp_\pb(\s)\geq\Exp_\pb(\s')$.
 $$\EAd_\IB(\S)=\Set{\s\in\S\given  (\exists \pb \in \IB)(\forall \s' \in \S)(\Exp_\pb[\U(\s)]\geq\Exp_\pb[\U(\s')])}$$
That is, $\s\in\EAd_\IB(\S)$ iff there is some $\pb\in\IB$ such that $\s\in\EU_\pb(\S)$. 
...
%	Thus, any $\s$ which picks for $\EU_p$ for some $p\in\IP$ also picks for $\EAd_\IP$.
%	
%	$\s\in\EAd_\IB(\S)$ iff there is some $\pb\in\IB$ such that $\s\in\EU_\pb(\S)$. 
%\end{lemma}


By \cref{thm:eu-self-rec}, for any $\pb\in\IB$ which has the form $p\times\mu$, if $\s$ picks for $\EU_p$ then it is in $\EU_{\pb}(\S)$, and thus, is in $\EAd_\IB(\S)$. Also, when $p\in\IP$ any $\s$ which picks for $\EU_p$ also picks for $\EAd_\IP$. We thus typically have some strategy which both picks for $\EAd_\IP$ and is in $\EAd_\IB$, unlike for $\Gamma$-Maximin. 
...
\begin{proposition}\label{thm:ead-equiv[indep]} Suppose $\D$ and $\Omega$ are completely independent in $\IB$ (so that every $\pb\in\IB$ has the form $p\times\mu$). In that case, $\s\in\EAd_\IB(\Strategies)$ iff there is some $p\times \mu\in\IB$ where $\s$ $\mu$-surely picks for $\EU_p$. 
...
If every $\pb\in\IB$ has the form $p\times\mu$ with each $\mu$ giving positive probability to facing both of the decisions in the Ellsberg case, then it also does not $\mu$-surely pick for $\EU_p$ for any $p\times\mu\in\IB$; and thus, is not in $\EAd_\IB(\S)$, despite picking for $\EAd_\IP$. 
...
{Suppose $\D$ and $\Omega$ are completely independent in $\IB$ (so that every $\pb\in\IB$ has the form $p\times\mu$).
...
\begin{colored}
	...
	$$\Maximality_\IB(\S)=\Set{\s\in\S\given (\forall \s'\in\S)(\exists \pb\in\IB)(\Exp_\pb[\U(\s)]\geq\Exp_\pb[\U(\s')])}$$
\end{colored}
...
\begin{definition}
	...
	\end{definition}
	That is, $a\in\EAd_{\IB(\cdot|-)}(D)$ iff there is some $\pb\in\IB$ such that $a\in\EU_{\pb(\cdot|-)}(D)$. 
	
	Also $\s\in\EAd_\IB(\Strategies)$ iff there is some $\pb\in\IB$ such that $\s\in\EU_\pb(\Strategies)$. 
	...
	\begin{proposition}\label{thm:ead-equiv[dep]}
		...
		Thus, there is some $\s$ which picks for  $\EAd_{\IB(\cdot|-)}$ and is in $\EAd_\IB(\Strategies)$, namely any $\s$ which $\pb_\Decs$-surely picks for $\EU_{\pb(\cdot|-)}$.
	\end{proposition}
\end{colored}
...
%	Equivalently, it can be seen as the countable disjoint union of $k$-tuples in $\Re^n$, modulo permutations, with the obvious inherited $\sigma$-algebra.\todo{more care}
	...
	We then require $p$ a probability on $(\Omega,\mathcal{F})$, $\mu$ a probability on $(\Decs,\Sigma)$, and $\pb$ a probability on $(\Omega\times\Decs,\mathcal{F}\otimes \Sigma)$.
...
\section{Alternative direct proof...}
...
More generally, 
	$a\in\EAd_\IB(\cdot|-)(D)$ iff there is some $\pb\in\IB$ such that $a\in\EU_\pb(D)$.  
Thus, any $\s$ which picks for $\EU_{\pb(\cdot|-)}$ for some $\pb\in\IB$ also picks for $\EAd_\IB(\cdot|-)$; and similarly for $\prpickstrat$. 

Also, 
	$\s\in\EAd_\IB(\S)$ iff there is some $\pb\in\IB$ such that $\s\in\EU_\pb(\S)$. 
	and 
		$\prpickstrat\in\EAd_\IB(\PrPickStrategies)$ iff there is some $\pb\in\IB$ such that $\prpickstrat\in\EU_\pb(\PrPickStrategies)$ 
\end{lemma}
