...
$\EU_p$, although such strategies may nonetheless be impermissible according to $\Gamma$-Maximin, as we saw, for example, in the Ellsberg case (\cref{sect:gamma}) where the only strategy compatible with $\Gamma$-Maximin was judged impermissible by the theory itself. 
	\begin{proposition}\label{thm:gamma-nu}Suppose $\Nu$ is $\EU$-complete. Suppose that 
		...
		Then if $\n$ is regular for $\Maximin_\IP$ then $\n\notin \Maximin_\IB(\Nu)$.\todo{NO PROOF OF THIS YET. SAME AS EAD AND MAX. MAYBE JUST RUN THOSE PROOFS FOR GENERAL CHOICE FUNCTIONS AND THEN APPLY IT HERE.}
...
When $b=p\times\mu$, then $b_\Decs=\mu$ and $b(\cdot|D)=p$ for all $D$; so $\Exp_{p\times\mu}[X]=\Exp_{\mu}[\Exp_{p}[X]]$.
...
\begin{proof}
	...
	\begin{proof}
		...
		Observe
		\begin{align}
			\{D\given \c(D)\not\subseteq\EU_p(D)\}&=\{D\given\exists b\in\c(D)\text{ and }t_q(D)>\Exp_q(D) \}\\
			&=\bigcup_{k\in\mathbb{N}}\{D\given\exists b\in \c(D)\text{ and }t_q(D)>\Exp_q(D)+\nicefrac{1}{k} \}\\
	...
	We have assumed that utility is bounded. Put $M>0$ such that $\|\U(a)\|_\infty\leq M$ for all $a$, that is $M\geq\max\{|l|,|h|\}$ if $\U:\A\to[l,h]^\Omega$. 	Let $$U_q:=\{p\in\Prob\given \|q-p\|_1<\frac{\epsilon_q}{4M}\}.$$
	...
	\end{sublemma}
	\begin{proof}
		This uses the atomlessness.
		
		Put $\alpha=\min\{\mu^*(C_1),\ldots,\mu^*(C_k)\}$. 
		
		Choose $E_1\subseteq C_1$ with $\mu^*(E_1)= \frac{\alpha}{k}$. 
		
		Then $\mu^*(C_2\setminus E_1)\geq\alpha-\frac{\alpha}{k}\geq\frac{\alpha}{k}$ so we can choose $E_2\subseteq C_2\setminus E_1$ with $\mu^*(E_2)=\frac{\alpha}{k}$. 
		
		Then $\mu^*(C_3\setminus(E_1\cup E_2))\geq\alpha-2\frac{\alpha}{k}\geq \frac{\alpha}{k}$ so we can choose $E_3\subseteq C_3\setminus (E_1\cup E_2)$ with $\mu^*(E_3)=\frac{\alpha}{k}$. 
		
		And so on, with the final stage ensuring $\mu^*(C_k\setminus(E_1\cup\ldots\cup E_{k-1}))\geq\alpha-(k-1)\frac{\alpha}{k}\geq \frac{\alpha}{k}$ so we can choose $E_k\subseteq C_k\setminus (E_1\cup \ldots\cup E_{k-1})$ with $\mu^*(E_k)=\frac{\alpha}{k}$.  
	\end{proof}
	...
	\begin{proof} As $b\in G(q,D,\epsilon_q)$,  $t_q(D)>\Exp_q[\U(b)]+\epsilon_q$, where $t_q(D)=\sup_{a \in D}\Exp_q[\U(a)]$. Take $a^*\in \EU_q(D)$ so that $\Exp_q[\U(a^*)]=t_q(D)$ so that $\Exp_q[\U(a^*)]-\Exp_q[\U(b)]\geq \epsilon_q$. 
		...
		This shows that $b\notin\EU_p(D)$. 
	\end{proof}
	Since we have chosen $\s(D)\in G(q_i,D,\epsilon_{q_i})$ for $D\in E_i$, then 
