...
\begin{colored}
	...
	Throughout this paper, we'll show that this idea results in a challenge for a host of theories that diverge from expected utility theory. The strength of the results that we show vary in the different cases. We provide a summary here.
	
	Decision theories that accommodate risk, by rationalising the Allais preferences, lead to ways of being uncertain over which decision problem you'll face such that they deem their own recommendations impermissible. Such theories therefore undermine themselves in a particularly strong sense. However, the initial result is limited to particular ways of being uncertain over which decision problem you'll face. We strengthen the result a little by showing the same follows for some natural ways of being uncertain over a much wider range of decisions, but we do not have a general result that holds for a whole host of ways of being uncertain. It is thus still open to a defender of this theory to argue that they are not uncertain over which decision they will face in such a way, or at least should not be. 
	
	Some other decision theories that differ from expected utility theory are those that accommodate ambiguity or imprecision. One of these, $\Gamma$-maximin is self-undermining in the same strong sense just described, at least for certain ways of being uncertain about which decision problem you'll face.
	
	Two other prominent decision theories for imprecision are E-Admissibility and Maximality. 	
	These theories deem some actions impermissible but may leave many possible actions incomparable, neither deeming one preferable to the other or treating them as indifferent. There are thus typically many different ways of acting which are compatible with the recommendations of the theory. 
	...
	For E-Admissibility, it deems impermissible any picking strategy which does not correspond to an expected utility picking strategy. Since picking in accordance with expected utility theory is also picking in accordance with E-Admissibility, it is not self-undermining in the strong sense we've considered so far: it does not rule out all ways of picking compatible with itself. However, it does rule some out.
	 ...
	 We propose that each probability function judges the imprecise picking strategy as sub-optimal,\todo{this sentence}
 	...
 	Again, it looks very much like a permissivist version of expected utility theory. \todo{maybe weaken this claim}
...
\begin{definition}\label{def:EU-complete}
	A set of probabilistic picking strategies, $\Nu$, is \emph{$\EU$-complete} if, for every probability $p$ over $\Omega$, there is some $\n$ in $\Nu$ such that $\n$ picks for $\EU_p$.
	%	
...
%\end{proposition}
	...
	\begin{proposition}\label{thm:ead-nu-reg-nec}
		Suppose that $\Nu$ is $\EU$-complete. \todo{I don't think we need EU-complete!}
...
Alternatively one could simply use the disintegration or Regular Conditional Probability theorem. 
...
\begin{corollary}
	\begin{enumerate}
		...
		\item \label{itm:eu-2}Suppose $\Nu$ is $\EU$-complete. If $\nu$ does not $b_\Decs$-surely pick for $\EU_{b(\cdot|-)}$ then $\nu\notin\EU_b(\Nu)$.
	\end{enumerate}
\end{corollary}
\begin{proof}
	\cref{itm:eu-1} is immediate from \cref{thm:EU appendix}. 
	
	For \cref{itm:eu-2}, take $\nu^*$ which does pick for $\EU_{b(\cdot|-)}$, possible by $\EU$-completeness, and \cref{thm:EU appendix} implies that $\Exp_b[\U(\nu^*)]>\Exp_b[\U(\nu)]$, so $\nu\notin\EU_b(\Nu)$. 
\end{proof}

This suffices to prove \cref{thm:eu-self-rec,thm:eu-uniquely-optimal,thm:eu-nu-nec-suff,thm:eu-dep} (noting that $\S$ is $\EU$-complete by definition). 
...
\subsection{E-Admissibility and EU equivalences}
This will then imply many of the results about E-Admissible strategies.
...
\end{corollary}
\begin{proof}
	Immediate from \cref{thm:appendix:ead:strategiesjudge,thm:EU appendix}. In fact, \cref{itm:ead:2} does not require $\EU$-completeness. 
\end{proof}
This gives \cref{thm:ead-equiv[indep],thm:ead-nu-nec-suff} and the first part of \cref{thm:ead-equiv[dep]}. 
...
\begin{corollary}\label{thm:appendix:ead:exist}
	\begin{itemize}
...
\end{corollary}
\begin{proof}
	\Cref{itm:ead:blah1}: From \cref{thm:appendix:ead:pickingeu}, $\nu$ picks for $\EAd_\IP$. By \cref{thm:appendix:ead:eu}, $\nu\in\EAd_\IB(\Nu)$. 
	
	\Cref{itm:ead:blah2}: From \cref{thm:appendix:ead:pickingeu}, $\nu$ picks for $\EAd_{\IB(\cdot|-)}$. By \cref{thm:appendix:ead:eu}, $\nu\in\EAd_\IB(\Nu)$. 
\end{proof}
\cref{thm:ead-suff-indep} follows from this, using the fact that $\S$ is $\EU$-complete so we can always select such an $\s$. So does the second part of \cref{thm:ead-equiv[dep]}



We have thus proved \cref{thm:ead-equiv[indep],thm:ead-nu-nec-suff,thm:ead-suff-indep,thm:ead-equiv[dep]}. 
...
As Maximality is a more permissive theory than E-Admissibility, \cref{thm:appendix:ead:exists,thm:appendix:ead:eu} also imply our results that there exist some Maximality strategies which are themselves judged as Maximal: \cref{thm:max-suff,thm:max-suff[dep],thm:max-nu-suff}

\subsection{Underminingness of E-Admissibility}

We will now move to \cref{thm:ead-existsimpermissibel[indep],thm:ead-existence[dep],thm:ead-nu-reg-nec}. For these results, we need to moreover give conditions under which no strategy looks like an expected utility strategy. 

For regular picking strategies this is simple: it has to not look like an $\EU_p$ strategy. 
...
\end{colored}
\section{Maximality}
To prove \cref{thm:max-nu,thm:max-nu-reg-nec}, we turn to two versions of Abraham Wald's (\citeyear{wald1947cct}) Complete Class Theorem. \todo{what are the two versions referred to?}
...
\begin{corollary}\label{thm:cct-applns:corr}
	Suppose that $\Nu$ is EU-complete, $\mu^*$ requires almost everywhere decisiveness and is countably additive. 
...
\end{proof}
\cref{thm:max-nu,thm:max-nu-reg-nec,thm:gamma-nu} follow from this. \todo{Let's double checkt hat that's true.}
...
\begin{corollary}Completely independent. 
		...
		Assume that for all  $g\in[l,h]^\Omega$, there is some $a\in\A$ such that $\U(a)=g$. 
...
\end{corollary}
\begin{proof}
	Let $V_1,V_2$ be disjoint non-empty open subsets of $[l,h]^\Omega$. Let $E_i:=\{D\in\D\given D\subseteq V_i\}$. This is open. 
...
\textbf{It is nonempty:}
	...
	As in \cref{thm:ead-existsimpermissible[indep]:CCM-1:nondegentakeleave}, we can find some $g^*\in\mathrm{Disagree}(q1,q2)$, and, moreover, can choose a scalar small enoough so that . 
...
\begin{proposition}[JK]\label{thm:ead-existsimpgen[indep]:JK}
...
\begin{enumerate}
\item $(\forall p\in\IP)(\exists E_p\subseteq E)\mu_p(E_p)>0$; moreover
\item $(\forall F\subseteq E_p)\left[\posi(\left\{f_X-g_X\given X\in E_p\right\}\cup\D_p)\cap\mathbb{R}^n_{<0}=\varnothing\;\Rightarrow\;\mu_p(F)=0\right]$
\end{enumerate}
...
\[
(\forall F\subseteq E_p)\left[\posi(\left\{f_X-g_X\given X\in E_p\right\}\cup\D_p)\cap\mathbb{R}^n_{<0}=\varnothing\;\Rightarrow\;\mu_p(F)=0\right]
\]
...
\[
\posi(\left\{f_X-h\given X\in F, h\in X\right\}\cup\D_p)\cap\mathbb{R}^n_{<0}=\varnothing
\]
since	 $E_p(f_X-h)\geq0$ for all $X\in F, h\in X$ and $E_p(h^\prime)\geq0$ for all $h^\prime\in\D_p$. But 
\[
\left\{f_X-g_X\given X\in F\right\}\subseteq\left\{f_X-h\given X\in F, h\in X\right\}.
\]
So we must have 
\[
\posi(\left\{f_X-g_X\given X\in E_p\right\}\cup\D_p)\cap\mathbb{R}^n_{<0}=\varnothing
\]
This implies $\mu_p(F)=0$ which contradicts $\mu_p(E_p)=\mu_p(F)>0$.
...
Suppose further that have a family  $\{E_i\given i\in I\}$ of mutually disjoint $E_i\subseteq\Decs$ satisfying
\begin{enumerate}
\item $(\forall p\in\IP)(\exists i\in I)\mu_p(E_i)>0$, 
\item $(\forall X\in \cup_{i\in I} E_i)(\exists f_X\in\EAd_\IP(X))(\exists g_X\in X)$
\[
(\forall F\subseteq \cup_{i\in I} E_i)\left[\posi(\left\{f_X-g_X\given X\in F\right\})\cap \mathbb{R}^n_{<0}=\varnothing\;\Rightarrow\;(\forall p\in\IP)\mu_p(F)=0\right]
\]
where $\posi(A)=\left\{\sum_{k=1}^m\lambda_k h_k \given m\in\mathbb{N},\;h_1,\hdots,h_m\in A,\;\lambda_1,\hdots\lambda_m>0\right\}$ for any $A\subseteq\mathbb{R}^n$.
\end{enumerate}
...
For any $X\in \cup_{i\in I} E_i$, let $\s(x)=f_X$. For $X\notin \cup_{p\in\IP}E_p$, let $\s(X)$ be anything in $\EAd_\IP(X)$.  So $\s$ picks for $\EAd_\IP$. 


	Choose any $p\times\mu_p\in\IB$. By (1), $\mu_p(E_i)>0$ for some  $i\in I$. Suppose for contradiction that $\s$ $\mu_p$-surely pick for $\EU_p$. Then $f_X\in\EU_p(X)$ $\mu_p$-a.e. on $E_i$, or equivalently, $f_X\in\EU_p(X)$ on $F$ for some $F\subseteq E_i$ with $\mu_p(E_i)=\mu_p(F)>0$. In that case,
\[
\posi(\left\{f_X-h\given X\in F, h\in X\right\})\cap \mathbb{R}^n_{<0}=\varnothing
\]
since	 $p(f_X-h)\geq0$ for all $X\in F, h\in X$. But 
\[
\left\{f_X-g_X\given X\in F\right\}\subseteq\left\{f_X-h\given X\in F, h\in X\right\}.
\]
So we must have 
\[
\posi(\left\{f_X-g_X\given X\in F\right\})\cap \mathbb{R}^n_{<0}=\varnothing
\]
But then by assumption $\mu_p(F)=0$ which contradicts (2).
