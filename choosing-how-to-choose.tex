...
\end{align}
where $\Exp_{b}[X | D]$ is defined as the orthogonal projection of $X$ onto the linear subspace of $\sigma(D)$-measurable random variables and $\sigma(D)$ is the $\sigma$-field generated by $D$. (This extends by continuity to $X\in L^1(b)$.)
...
\end{align}
Where no confusion arises, we write $\Exp_{b_\Decs}[\Exp_{b(\cdot|D)}[X]]$ as shorthand for $\Exp_{b_\Decs}[\Exp_{b(\cdot|D)}[X(\cdot, D)]]$. Hence $\Exp_b[X]=\Exp_{b_\Decs}[\Exp_{b(\cdot|D)}[X]]$.
