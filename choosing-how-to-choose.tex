...
\usepackage{wrapfig}
\usepackage{mathpazo}
%\usepackage{mathbbol} %% conflict with mathpazo
\usepackage{lscape}
\usepackage{stmaryrd}
...
%	Consider a decision theory, $\s$ which recommend taking the safe option over the risky option in $D^*$. 
	...
	\begin{array}{c|ccc}
		...
		& \text{goes bust} &  \text{not bust}  \\
		\hline
		P & \sfrac{1}{100} &  \sfrac{10}{100} &  \sfrac{89}{100} \\
		\hline
		\hline
...
%		P &  \sfrac{1}{11} \times \sfrac{11}{100} & \sfrac{10}{11}\times\sfrac{11}{100} & \sfrac{89}{100} \\
	...
	$$
	\begin{array}{r|cccc}
		& \text{ticket 1} &\text{ticket 2-11}& \text{ticket 12-100}\\
		\hline
		P & \sfrac{1}{100} & \sfrac{10}{100} & \sfrac{89}{100} \\
		\hline
		\hline
...
$$
\begin{array}{r|cc}
	\text{Decision problem } D^{\mathrm{Allais}}_{\mathrm{local}} & \text{goes bust} &  \text{not bust}  \\
	\hline
	p & \sfrac{1}{11} & \sfrac{10}{11} \\
	\hline
	\hline
...
\[
\begin{array}{r|ccc}
	& \text{Ticket 1} & \text{Tickets 2--11} & \text{Tickets 12--100} \\
	\hline
	p & \sfrac{1}{100} & \sfrac{10}{100} & \sfrac{89}{100} \\
	\hline\hline
...
Now, instead consider this decision problem. There are two actions that the agent could take, take a risky bet or the safe course of action. Let's say the outcome of the risky bet depends on whether this particular company goes bust. These actions might be, for example, options for stocks she can invest in. Let's suppose that the agent thinks that the odds of this company going bust are $1:10$. \todo{I'm happy to instead keep the example as one with lottery tickets, now with 11 tickets... And then whether she's offered it is determined by a toss of a biased coin??}
She is thus be faced with this decision problem:
$$
\begin{array}{r|cc}
	\text{Decision problem } D^{\mathrm{Allais}}_{\mathrm{local}} & \text{goes bust} &  \text{not bust}  \\
...
\end{array}
$$	
Perhaps her decision theory prefers Risky over Safe.

We will now turn to a higher-order decision that the agent might face: a choice amongst various so-called picking strategies. She is unsure which decision problem she will face, and is choosing a strategy for what to pick in each possible decision problem. 
%She has some credence over various decision problems that she might be faced with.

Perhaps she is leaving instructions for what her stockbroker should do, unsure what will be on offer. 
Or perhaps she has to choose a proxy to act on her behalf, knowing what they'll pick in each decision problem, but unsure which decision problem will be in play. 
%As she is unavailable to make the decision at the later time. 
...
\end{array}
$$
This is extensionally equivalent to the choice of 1A vs AB in $D^{\mathrm{Allais}}_2$. Assuming her decision theory is structural, in the sense that its recommendations don't depend on the content of the outcomes but just the list of credences of various utility outcomes, then she should also judge $\s_{\mathrm{Safe}}$ to be preferable to $\s_{\mathrm{Risky}}$, even though, when faced with  $D^{\mathrm{Allais}}_{\mathrm{local}}$ herself, she prefers Risky.
%This is now extensionally equivalent to the choice of 2A vs 2B in $D^{\mathrm{Allais}}_2$, and we have supposed that she preferred 2B over 2A, matching the observed preferences. In this scenario, then, she will evaluate the picking strategy $\s_{\mathrm{Risky}}$ to be preferable to $\s_{\mathrm{Safe}}$, even though, when faced with  $D^{\mathrm{Allais}}_{\mathrm{local}}$ herself, she prefers Safe.

Given the choice of what to write as instructions for her stockbroker, she prefers to instruct them to act differently to how she would act were she facing the decision herself. Given the choice over which proxy to nominate, she thinks it is better to nominate one that she knows won't choose in the way she would were she in the situation herself. 
Given the choice of whether she should tie herself to the mast, and pre-commit to a particular course of action, she is willing to pay money to get that rope.\todo{isn't this making it like the others???}

If our agent, using her decision theory, endorses the Allais preferences and recommends Risky over Safe in $D^{\mathrm{Allais}}_{\mathrm{local}}$, then she will prefer to write down a picking strategy which does not match her own. 
That is, her adopted decision theory recommends a picking strategy which is not compatible with its own recommendations. Despite the fact that her decision theory recommends picking Risky, it thinks it would be better to use a decision theory which recommends Safe in $D^{\mathrm{Allais}}_{\mathrm{local}}$.
...
%This is, \emph{prima facie}, a bad-making feature of a decision theory. 
It is self-undermining, in the sense that in this particular scenario, with the particular precise probability distribution over the possible decision problems she might face ($11\%$ for $D^{\mathrm{Allais}}_{\mathrm{local}}$; $89\%$ for sure-\pounds1m, a trivial decision problem), if she applies her decision theory to the question of which picking strategy to use, it rules out as impermissible, what she thinks she should do in that scenario.


And there is no funny business going on here. Learning that you face decision problem $D^{\mathrm{Allais}}_{\mathrm{local}}$ does not, we are supposing, provide any information about whether the company will go bust. We assumed that the decision problem is independent of the state of the world. We are also assuming act-state independence: picking one option or another in $D^{\mathrm{Allais}}_{\mathrm{local}}$ does not tell you anything about which state of the world you're in.

This was all based on the asumption not only that her decision theory endorsed the Allais preferences, but also that it recommended Risky over Safe in $D^{\mathrm{Allais}}_{\mathrm{local}}$. If instead she preferred Safe over Risky in $D^{\mathrm{Allais}}_{\mathrm{local}}$, however, consider instead a different decision problem: Suppose that whilst she still thinks that her credence that she'll be offered $D^{\mathrm{Allais}}_{\mathrm{local}}$ is $11\%$, she thinks that if she is not given it, then she is given nothing. 
Now, she is facing the following decision problem over which picking strategy to select. 
$$
\begin{array}{c|ccc}
	\multirow{2}{*}{\text{Version 2}}	&\multicolumn{2}{c}{\text{choice offered}}&\text{choice not offered}\\
...
Thus, if her decision theory is opinionated over what to choose in $D^{\mathrm{Allais}}_{\mathrm{local}}$ and endorses the Allais preferences then it is undermining in the sense that there is some uncertainty over which decision problem she'll face where the decision theory rules its own way of picking as impermissible. 

What if, however, her decision theory has neither a strict preference for Safe over Risky or Risky over Safe in $D^{\mathrm{Allais}}_{\mathrm{local}}$, with both being compatible with the recommendations of her decision theory. Thus both picking strategies are compatible with her decision theory, and whilst which picking strategy she deems optimal changes depending on version 1 or version 2, in neither case does she rule out all the picking strategies which are compatible with her recommendations.


Decision theories do typically leave some decisions unsettled, where her reasons for picking a course of action have run out. They might be indifferent between various actions or, as we will study later in the paper, deem them incomparable.  In acting in the world, an agent does actually need to pick an option, even if her reasons for one or the other have run out. Decision theories often leave open various ways to pick. 
Picking is a matter of settling on what to do, even if her reasons have run out. This might be distinguished from a stronger notion, that of \emph{choosing}, which \citet{ullman1977}, which is selecting an option based on her reasons. \todo{this para needs improving.} 

The decision theories which endorse the Allais preferences are usually motivated by avoidance of risk rather than considerations of ambiguity or imprecision, and they would typically say that in cases where one doesn't have a strict preference either way are where she is indifferent between them, and for any slight sweetening is sufficient to make her choose that option. So then, in 
$$
\begin{array}{r|cc}
 D^{\mathrm{Allais}}_{\mathrm{local-sweetened}} & \text{goes bust} &  \text{not bust}  \\
	\hline
	p & \sfrac{1}{11} & \sfrac{10}{11} \\
	\hline
	\hline
	\text{Safe} &\text{\pounds 1m}&\text{\pounds 1m}                      \\
	\text{Risky-sweetened} & \text{\pounds 1}&\text{\pounds 5m+\pounds 1}
\end{array}
$$ she will prefer Risky-sweetened to Safe; but for some small enough sweetening, it is plausible that she will still prefer 1A to 1B-sweetened
$$
\begin{array}{r|cccc}
	& \text{ticket 1} &\text{ticket 2-11}& \text{ticket 12-100}\\
	\hline
	...
	\hline
	\text{1A} & \text{\pounds 1m} & \text{\pounds 1m}  & \text{\pounds 1m} \\
	\text{1B-sweetened} & \text{\pounds 1} & \text{\pounds 5m+\pounds1}  & \text{\pounds 1m}
\end{array}
$$
whereby we again have the decision theory deeming it impermissible to pick in accordance with its own recommendations, instead recommending using a different decision theory. 

Any failure of von Neumann-Morgenstern's Independence Principle will generate such a case: if your decision theory deems action $a$ to be preferable to action $b$, but when evaluating an action which tosses a biased coin and selects $a$ (or $b$) if the coin lands Heads and $c$ if the coin lands Tails, the decision theory evaluates the $b$ version to be preferable, then similarly we can generate a way of being uncertain over which decision problem you'll be faced with so that the unique picking strategy which is compatible with the decision theory is itself impermissible according to the theory. 

These cases are very closely related to Dutch bookability / sequential incoherence / Value of Information failures / ??? Which entail underminingness? What do we know about when they are guaranteed? Might be worth including here... ! \todo{Sort out!}
...
\todo{Maybe worth seeing if we can reword. }
...
\subsection{Who's fault is it}

What we've used the decision theory to evaluate is picking functions. We then associated picking functions with the decision theory. 
...
We have then said that this shows that the decision theory recommends using a different decision theory. But that is only the right diagnosis if we hold fixed the parameters of credences, utilities and risk-profile, assuming that they don't change once she knows which decision problem she'll be faced with.

The same kind of argument has been used which instead hold fixed that she makes decisions in accordance with Expected Utility Theory and instead uses these results to judge her credences (Schervish/Levinstein). We could allow her credences to vary depending which decision she is faced with, treating the decision and state as dependent. This is essentially Brown's argument for updating credences by conditionalization. By holding fixed a different decision theory, say risk-weighed expected utiltiy theory of Buchak's, alternative credal update strategies are deemed optimal\todo{hum.. the link to my stuff with B is not super clear as that's importantly epistemic land}. Someone WHO?? is considering varying utilities. Good's Value of Evidence cases instead hold fixed that she'll update by conditionalization and make a decision in accordance with expected utility theory, but judges whether she wants to gather the evidence or avoid it. 

\end{colored}
\subsubsection{Change the way of updating credences}
	 \todo{remove or significantly condense this sec?? !!!}
{\color{red}\todoinfo{CCM trying to condense...}
	We have shown that 
...
\todo{This needs rewriting!!! I've made a start...}
\begin{colored}
	{violet} Moreover, the particular way of being uncertain over decisions you'll be faced with was a rather odd one. We specified that the credence you'll face $D^{\mathrm{Allais}}_{\mathrm{local}}$ is $\sfrac{1}{11}$ and your credence that you'll get the constant amount is $\sfrac{89}{100}$. 
	
	Of course this might be the situation you're facing, where you happen to have these credences. Or it could be specifically constructed this way by a nefarious opponent who has set you up: they're going to toss a biased coin to determine what to offer you at the specified probabilities, having chosen these number specifically to demonstrate your inconsistent judgements. 
	
	However, typically one's uncertainty over which decision you'll face is spread over a wide class of possible decision problems. If your particular uncertainty is not one which generates the underminingness, is it really that problematic that were you to be in the particular uncertainty situation your decision theory were undermining, if in your actual situation it judges its own picking strategies as optimal. 
...
$$
\begin{array}{r|ccc}
D^{\mathrm{Ellsberg}}_1 & \emph{Red} & \emph{Black} &\emph{Yellow} \\
\hline
...
\hspace{10mm}
\begin{array}{r|ccc}
D^{\mathrm{Ellsberg}}_2 & \emph{Red} & \emph{Black} &\emph{Yellow} \\
\hline
\IP & \sfrac{1}{3} &x&\sfrac{2}{3}-x \\% \multicolumn{2}{c}{\sfrac{2}{3}}\\
\hline
\hline
...
Observe, then, that $\Exp_{\mu^*}\U(\s_{\text{1A,2B}})(\omega)=5$ for each $\omega$ in $\Set{\emph{Red}, \emph{Black}, \emph{Yellow}}$, but $\Exp_{\mu^*}\U(\s_{\text{1B,2A}})(\omega)=6$ for each $\omega$ in $\Set{\emph{Red}, \emph{Black}, \emph{Yellow}}$. So, for any $p$ with $p\times \mu^*\in \IB$,  $\EU_{p\times\mu^*}\U(\s_{\text{1A,2B}})=5$ and  $\EU_{p\times\mu^*}\U(\s_{\text{1B,2A}})=6$; so $\s_E=\s_{\text{1A,2B}}\notin\Maximin_{\IB}(\S)$. 
	...
	For instance, we might consider how the agent will choose in $D^{\mathrm{Ellsberg}}_1$ and $D^{\mathrm{Ellsberg}}_2$, individually, and then combine these choices and observe that the result is dominated---1B-and-2A dominates 1A-and-2B. One response is simply to reject the package principle. However, there is another version of the examples in which they are presented diachronically: first evaluate $D^{\mathrm{Ellsberg}}_1$, then $D^{\mathrm{Ellsberg}}_2$. And again we can note that 1B-then-2A dominates 1A-then-2B. And we can note that $\Gamma$-Maximin would still have you choose 2A when faced with $D^{\mathrm{Ellsberg}}_2$, even if you know you've already chosen 1B when faced with $D^{\mathrm{Ellsberg}}_1$. But some will deny that sure loss as a result of decisions at different times indicates irrationality.
...
What all of this shows, one might think, is just that E-Admissibility sees value in coordinating how you resolve incomparability. Take a simple example.
	...
	Any $\EU$-maximizer will coordinate their choices in $D^{\mathrm{Ellsberg}}_1$ and $D^{\mathrm{Ellsberg}}_2$ in the following sense: they will (assuming utility linear in GBP) choose $b$ (reject $a_1$) in $D^{\mathrm{Ellsberg}}_1$ just in case they choose $a_2$ (reject $b$) in $D^{\mathrm{Ellsberg}}_2$; likewise, they will choose $a_1$ (reject $b$) in $D^{\mathrm{Ellsberg}}_1$ just in case they choose $b$ (reject $a_2$) in $D^{\mathrm{Ellsberg}}_2$. 
% \todoold{unless $p=0.5$!!!}	
	
	Suppose $\IP = \{p_1, p_2\}$, where $p_1$ expects $b$ to be strictly better than $a_1$, while $p_2$ expects $a_1$ to be strictly better than $b$. In that case, $\EAd_\IP(D^{\mathrm{Ellsberg}}_1)=\{b,a_1\}$ and $\EAd_\IP(D^{\mathrm{Ellsberg}}_2)=\{b,a_2\}$. You find both options in both options \emph{incomparable}, \emph{i.e.}, not rejected, but also not indifferent or equally good. Just as each of $p_1$ and $p_2$ coordinates their choices in $D^{\mathrm{Ellsberg}}_1$ and $D^{\mathrm{Ellsberg}}_2$, so too does E-Admissibility advise you to coordinate how you resolve incomparability by picking. You ought to pick $b$ in $D^{\mathrm{Ellsberg}}_1$ just in case you pick $a_2$ in $D^{\mathrm{Ellsberg}}_2$. Likewise, you ought to pick $a_1$ in $D^{\mathrm{Ellsberg}}_1$ just in case you pick $b$ in $D^{\mathrm{Ellsberg}}_2$. 
	
	
	
	Now, you might doubt that there is \emph{really} any value in this sort of ``modal coordination.'' (Recall, you will actually only face one of $D^{\mathrm{Ellsberg}}_1$ or $D^{\mathrm{Ellsberg}}_2$. You are not coordinating across time.) But the fact that E-Admissibility \emph{sees} value in coordinating how you resolve incomparability does not render it self-undermining.\todoold{I like this paragraph a lot!!!!}
...
%{\color{blue}What these show is that for a strategy to be evaluated as E-Admissible it requires coordination across different decision problems. For example, in the Ellsberg case, is fine for one's strategy to pick either act in $D^{\mathrm{Ellsberg}}_1$ so long as it picks the corresponding act in $D^{\mathrm{Ellsberg}}_2$. It does not permit arbitrary picking strategies which are compatible with 
...
%In either of these, what we have is for each choice function $C$ and decision $D$, a probability function $\sigma^C_D$ over $C(D)$, the options from $D$ which are not ruled out by $C$ in this decision problem. We can then consider $\U(C,D,\omega)=\Exp_{\sigma^C_D} a(\omega)$. 
...
%Either way, what we formally have, is, for each decision problem $D$ and picking strategy $C$, some probability $\sigma$ over the options which are not ruled out by $C$. Then we have $\U(C,D,\omega)$ as $\Exp_{\sigma^C_D}\U(a,D,\omega)$; which we think of as your expected utility of having adopted the choice function $C$. 
...
%		\item A  \emph{(deterministic) $C$ picking strategy} specifies an action compatible with $C$ for each $D$, that is where $\s(D)\in C(D)$
		...
		A \emph{probabilistic picking strategy} $\sigma$, specifies, for each decision problem, $D\in\D$, a probability function $\sigma_D$ over $D$, i.e., over the acts available in the decision problem $D$.
		% $\sigma$ is a function from $\D$ to probability functions over $\A$ which 
		\item For a choice function $\c$,  $\sigma$ \emph{picks for $\c$} iff $\sigma_D(\c(D))=1$ for each $D\in\D$, i.e., it is certain that what it picks will be compatible with $C$'s recommendations.
		\item For a choice function $\c$, $\sigma$ \emph{$\mu$-surely picks for $\c$}, if $\mu\{D\given \sigma_D(\c(D))=1\}=1$.
 	\end{itemize}
\end{definition}
And we add a new one:
\begin{definition}
For a choice function $\c$,  \emph{$\sigma$ is regular for $\c$}, if, for each $D$ in $\D$, $\sigma_D(a) > 0$ iff $a \in \c(D)$.
\end{definition}
Observe that in the special case where $\sigma$ is extremal---that is, when for every $D$ it assigns all its probabilistic weight to an individual member of $D$---then we recover our original notion of a picking strategy. 

%We will apply the choice function to evaluate strategies, or more generally, $\sigma$ too. \todooldinfo{JK to help!! is there a systematic thing to say about extending. See the Max section...???}

For a non-probabilistic picking strategy, $\s$, we simply took its utility to be the utility of the act it requires you to pick: given a state of the world $\omega$ and a decision problem $D$, $\U(\s)(\omega,D):=\U(\s(D))(\omega)$, the utility of the act $\s(D)$ at $\omega$. For $\sigma$, we take its utility to be the \emph{expected} utility of the act it lead you to pick: given a state of the world $\omega$ and a decision problem $D$, $$\U(\sigma)(\omega,D):=\sum_{a \in D} \sigma_D(a)\U(a)(\omega).$$
\begin{comment}
\begin{colored}{violet}
	If we are allowing $D$ to be infinite (although compact), we should in fact take an integral, $\U(\sigma)(\omega,D):=\int_{a \in D} \U(a)(\omega)\,\sigma_D(\diff a).$\todoold{should we explicitly talk about finitely additive integrals?}
%	All our results merely require that $\sigma$ is finitely additive so in fact, we should be taking a Daniell integral, REFS.  $\U(\sigma)(\omega,D):=\int_{a \in D} \U(a)(\omega)\,\sigma_D(\diff a).$
\end{colored}
\end{comment}
% = \Exp_{\sigma_D}[\U(\omega)].$$ %We then have
%$$\EU_{p\times\mu}[\U(\sigma)]=\EU_{p\times\mu}\Exp_{\sigma_D}[\U(\omega)]$$%\todooldinfo{I'm not sure about the right notation}


We have thus far been assuming that decision problems $D$ are non-empty finite sets of acts. If we were to allow $D$ to be infinite (although compact), then we should have $\U(\sigma)(\omega,D):=\int_{D} \U(a)(\omega) \sigma_D(\diff a)$.
...
We will judge whether the decision theory evaluates a probabilistic picking strategy $\sigma$ to be impermissible. This will depend on the range of alternatives available. That is, we will be judging whether $\sigma$ is an impermissible picking strategy from a set of picking strategies, $\N$. 
...
If you think we are just evaluating some choice functions, and each one just comes along with a single randomisation process, then $\N$ will have a particular $\sigma^\c$ for each $\c$, where $\sigma$ picks for $\c$. 
If you instead are just uncertain over how you'll pick when using a choice function $\c$, and assume that this is a matter governed by a precise probability, then again we'll have a $\sigma^\c$ representing your probabilistic uncertainty over how you'll pick once you've selected a choice function $\c$ and are faced with a decision $D$.
...
\begin{definition}\label{def:EU-complete}
	A set of probabilistic picking strategies, $\N$, is \emph{$\EU$-complete} if, for every probability $p$ over $\Omega$, there is some $\sigma$ in $\N$ such that $\sigma$ picks for $\EU_p$.
	%	
	%A set of probabilistic picking strategies is \emph{$\EU$-complete (for $\IB$)} if, for each $p\times \mu\in \IB$, there is $\sigma$ in $\N$ such that $\sigma$ $\mu$-surely picks for $\EU_p$.
\end{definition}
%In fact, for our results we will only require that it is sufficiently $\EU$-complete for $\IB$, meaning that for each $p\times \mu\in \IB$ there is some $\sigma\in\N$ such that $\sigma$ $\mu$-surely picks for $\EU_p$. However, we will continue to use the general $\EU$-completeness for ease of exposition. 
...
$$\sigma \in \EU_{p\times \mu}(\N) \Leftrightarrow \sigma \text{ $\mu$-surely picks for $\EU_p$.}$$
and, more generally, 
$$\sigma \in \EU_{\pb}(\N) \Leftrightarrow \sigma \text{ $\pb$-surely picks for $\EU_{\pb(\cdot|-)}$.}$$
\end{proposition}
...
If $\sigma$ $\mu$-surely picks for $\EU_p$ for some $p\times \mu\in \IB$, then $\sigma\in\EAd_\IB(\N)$. More generally, if for some $\pb\in\IB$, $\sigma$ $\pb$-surely picks for $\EU_{\pb(\cdot|-)}$, then $\sigma\in\EAd_\IB(\N)$. 

Moreover, these are the only members of $\EAd_\IB$, at least assuming that $\N$ is $\EU$-complete: 
$$\sigma \in \EAd_\IB(\N) \Leftrightarrow \text{for some $\pb$ in $\IB$, $\sigma$ $\pb$-surely picks for $\EU_{\pb(\cdot|-)}$.}$$
%If $\IB$ is strongly complete, then: $\sigma \in \EAd_\IB(\N) \Leftrightarrow \sigma \text{ $\mu$-surely picks for $\EU_p$, for some $p\times \mu$ in $\IB$.}
%\end{align}

%If $\N$ is $\EU$-complete, then $$\sigma \in \EAd_\IB(\N) \Leftrightarrow \sigma \text{ $\mu$-surely picks for $\EU_p$, for some $p\times \mu$ in $\IB$.}$$
\end{proposition}
...
%Suppose $\c$ is a choice function and, for every $p\times \mu\in \IB$,  $\mu\{D\given \c(D)\subseteq \EU_p(D)\}< 1$. Then, if $\sigma$ is a regular picking strategy for $\c$, then $\sigma\notin \EAd_\IB(\N)$. \todooldinfo{let's amend this to just being an EAd version! Do we know how this would look???}
%\end{proposition}
...
%	Suppose that for every $p\times \mu\in \IB$, $\mu\{D\given \EAd_\IP(D)\subseteq \EU_p(D)\}\neq 1$. That is, for all $p\times \mu\in \IB$, $\mu\{D\given \text{there is $p'\in\IP$ with }\EU_{p'}(D)\not\subseteq\EU_p(D)\}>0$. 
	...
	Then, if $\sigma$ is a regular picking strategy for $\EAd_{\IB(\cdot|-)}$, then $\sigma\notin \EAd_\IB(\N)$.
	\end{proposition}
	



%The antecedent of the final part of the theorem ensures that $\IP$ is actually imprecise and that you think you'll be faced with a decision where this matters. In which case, then the $\sigma$ will not be an $\EU_p$ picking strategy for any $p\in\IP$, and moreover, will not be $\mu$-surely an $\EU_p$ picking strategy for any $p\times \mu\in \IB$. 
...
%Suppose you're certain you'll face a decision problem $D = \{a_1, a_2\}$. So $\mu(D) = 1$. Suppose $\IP = \{p_1, p_2\}$, where $p_1$ expects $a_1$ to be strictly better than $a_2$, while $p_2$ expects $a_2$ to be strictly better than $a_1$. So E-Admissibility with $\IP$ says that both $a_1$ and $a_2$ are choiceworthy. Then for any picking strategy $\sigma$ such that $\sigma_D$ gives positive probability to $a_1$ and $a_2$, $p_1$ doesn't expect it to be best, and nor does $p_2$. So, by E-Admissibility, that picking strategy is not rationally permissible.


Let's see this at work. Consider again the Ellsberg setup, \cref{sect:Ellsberg}. Recall that $\EAd_\IP(D^{\mathrm{Ellsberg}}_1)=\{\text{1A,1B}\}$ and $\EAd_\IP(D^{\mathrm{Ellsberg}}_2)=\{\text{2A,2B}\}$. 
Every probability in $\IP$ rules out at least one of the E-Admissible options as impermissible. For example, any $p(\emph{Black})>\nicefrac{7}{30}$ rules 1A as excluded, i.e., not in $\EU_p$, but there's some positive chance that $\sigma_{D^{\mathrm{Ellsberg}}_1}$ picks 1A, by the assumption that it is regular for $\EAd_\IP$. 
Thus, $\sigma$ does not pick for $\EU_p$. It also does not even $\mu$-surely pick for $\EU_p$ if we assume that each $\mu$ assigns positive probability to both $D^{\mathrm{Ellsberg}}_1$ and $D^{\mathrm{Ellsberg}}_2$. It is thus not E-Admissible. 

In fact, if $\IP$ is allowed to be non-convex, we get cases where $\sigma$ will be judged as impermissible even when you know what decision problem you'll be faced with.\footnote{These examples are avoided when $\IP$ is convex as then there will be  a probability which is indifferent between the two actions, and thus, $\sigma$ is Bayes for this probability.}\todo{ref: Bayes for p not defined}
Suppose you're certain you'll face a decision problem $D = \{a_1, a_2\}$. So $\mu(D) = 1$. Suppose $\IP = \{p_1, p_2\}$, where $p_1$ expects $a_1$ to be strictly better than $a_2$, while $p_2$ expects $a_2$ to be strictly better than $a_1$. So E-Admissibility with $\IP$ says that neither $a_1$ or $a_2$ are rejected. Then for any regular picking strategy for $\EAd_\IP$, $\sigma_D$ will give positive probability to both $a_1$ and $a_2$. $p_1$ doesn't expect it to be best, and nor does $p_2$. So, by E-Admissibility, $\sigma$ is not rationally permissible.
...
\begin{proposition}\label{thm:max-nu-suff}
		If for some $p\times\mu\in \IB$, $\sigma$ $\mu$-surely picks for $\EU_{p}$, then $\sigma\in\Maximality_\IB(\N)$. 
		
		More generally, if for some $\pb\in \IB$, $\sigma$ $\mu$-surely picks for $\EU_{\pb(\cdot|-)}$, then $\sigma\in\Maximality_\IB(\N)$. 
\end{proposition}
...
Then we have the following result.%
\footnote{{The assumptions on $\mu^*$ are not required if one instead assumes that $\N$ is convex, which is motivated when one considers randomisations as available options. If $D$ was allowed to be infinite, one should also ensure that $\N$ is closed, 
...
\begin{proposition}\label{thm:max-nu}Suppose $\N$ is $\EU$-complete. Suppose that 
	$\IB$ has the form $\{p\times \mu^*\given p\in \IP\}$ and $\mu^*$ requires almost everywhere decisiveness. Then if $\sigma \in \Maximality_\IB(\N)$, then $\sigma \text{ $\mu^*$-surely picks for $\EU_p$, for some probability $p$.}$
...
\end{proposition}
This follows from a version of Wald's Complete Class Theorem. We will be able to show that if is not in $\EU_{p\times\mu^*}$ for any $p$, then there is some alternative $\sigma'$, in fact, an $\sigma'$ which picks for some $\EU_p$, where $\Exp_{\mu^*}\U(\sigma')(\omega)>\Exp_{\mu^*}\U(\sigma)(\omega)$ for all $\omega$, and thus for all $p$, $\Exp_p[\Exp_{\mu^*}\U(\sigma')]>\Exp_p[\Exp_{\mu^*}\U(\sigma)]$, i.e., $\Exp_{p\times \mu^*}\U(\sigma')>\Exp_{p\times\mu^*}\U(\sigma)$, so $\sigma\notin\Maximality_\IB(\N)$. 
...
%	 If $\sigma$ is not Bayes\todooldinfo{here!}
%	If $\sigma$ does not $\mu^*$-surely pick for $\EU_p$ for any probability $p$, then by \cref{thm:eu-nu-nec-suff}, $\sigma\notin \EU_{p\times\mu^*}(\N)$. We can then use a version of Wald's Complete Class Theorem to show that there is some $\sigma'$ where $\Exp_{\mu^*}\U(\sigma')(\omega)>\Exp_{\mu^*}\U(\sigma)(\omega)$ for all $\omega$, and thus for all $p$, $\EU_{p\times\mu^*}\U(\sigma')>\EU_{p\times\mu^*}\U(\sigma)$, so $\sigma\notin\Maximality_\IB(\N)$. 
%	What we can show is that if $\sigma$ is such that there is no probability function with $\Exp_{p}\Exp_{\mu^*}\U(\sigma)$
%	 which shows that non-Bayes options are dominated. We 
%%	Using this, we show that if $\sigma$ is not Bayes for any 
...
\begin{proposition}\label{thm:max-nu-reg-nec}
	...
	Then, if $\sigma$ is regular picking strategy for $\Maximality$ then $\sigma\notin \Maximality_\IB(\N)$, where 	$\IB$ has the form $\{p\times \mu^*\given p\in \IP\}$. 
\end{proposition}
...
%Our core result is that if $\sigma$

%{\color{violet}This isn't unique to Maximality. It also doesn't depend on any strong independence condition. What we actually show is that if $\sigma$ is not Bayes, then there is $\sigma'$ such that $\Exp_{\mu^*}\U(\sigma)(\omega)<\Exp_{\mu^*}\U(\sigma')(\omega)$ for all $\omega$. So if $\c$ is any choice function allowing some non-$\EU_p$ strategies, then any regular $\sigma$ gives some positive probability to these non-$\EU_p$ options; which are thus not Bayes and thus dominated in expectation by some alternative. }
%
% It will hold for all {
%	\color{orange} manner of choice functions. 
%
%	For a choice function $\c$, let $\c_{\mu^*}$ be an extension of $\c$ to $\Omega\times \D$ which treats $\D$ as being governed by the precise probability $\mu^*$ and respects dominance, i.e., if $\Exp_{\mu^*}\U(\sigma)(\omega)<\Exp_{\mu^*}\U(\sigma')(\omega)$ for all $\omega\in\Omega$, then $\sigma\notin \c_{\mu^*}(\N)$. 
%	}
...
%	
%	Then, if $\sigma$ is regular picking strategy for $\c$ then $\sigma\notin \c_{\mu^*}(\N)$.\todoold{check this}
%\end{proposition}
%
...
%
%Then, if $\sigma$ is regular picking strategy for $\Maximin_\IP$ then $\sigma\notin \Maximality_\IB(\N)$, where 	$\IB$ has the form $\{p\times \mu^*\given p\in \IP\}$. 
%\end{proposition}



Since $\Gamma$-Maximin is a more restrictive theory than Maximality, for a strategy to be $\Gamma$-Maximin there must be some $p\in\IP$ for which it $\mu^*$ surely pick for \todo{eg here should we replace?? I did so... changed order of quantifier}$\EU_p$, although such strategies may nonetheless be impermissible according to $\Gamma$-Maximin, as we saw, for example, in the Ellsberg case (\cref{sect:gamma}) where the only strategy compatible with $\Gamma$-Maximin was judged impermissible by the theory itself. 
	\begin{proposition}\label{thm:gamma-nu}Suppose $\N$ is $\EU$-complete. Suppose that 
		$\IB$ has the form $\{p\times \mu^*\given p\in \IP\}$ and $\mu^*$ requires almost everywhere decisiveness. Then if $\sigma \in \Maximin_\IB(\N)$, $ \sigma \emph{ $\mu^*$-surely picks for $\EU_p$, for some probability $p$.}$
		
		If also for every $p\in\IP$, $\mu^*\{D\given \Maximin_\IP(D)\subseteq \EU_p(D)\}<1$, then there is some $\sigma$ which picks for $\Maximin_\IP$ and which is not in $\Maximin_\IB(\N)$ (in fact, any $\sigma$ which picks for $\EU_p$ with $p\in\IP$ will do).  And if $\sigma$ is regular for $\Maximin_\IP$ then $\sigma\notin \Maximin_\IB(\N)$. 
	\end{proposition}
...
\begin{proposition}Suppose that $\N$ is $\EU$-complete and convex.\todooldinfo{Actually we should be careful: if $\N$ is convex but not closed then it is non-compact and our decision theories aren't even defined for it!!!}
	...
	If for every $p\in\IP$, $\mu^*\{D\given \Maximality_\IP(D)\subseteq\EU_p(D)\}< 1$ and $\sigma$ is a regular picking strategy for $\Maximality_\IP$, then $\sigma\notin\Maximality_\IB(\N)$.
%	
%	And, more generally, 
%	If for every $p\in\IP$, $\mu^*\{D\given \c(D)\subseteq\EU_p(D)\}< 1$ and $\sigma$ is a regular picking strategy for $\c$, then $\sigma\notin\c_{\mu^*}(\N)$.
%	
%
\end{proposition}
\end{comment}
...
%	We proposed using probabilistic picking strategies, $\sigma$ in evaluating a decision theory. 
	
	\subsubsection{Imprecise picking strategies}\label{IPpicking}
	So far, we've used decision theories to judge deterministic picking strategies, $\s$, and probabilistic picking strategies, $\sigma$. 
...
%	Using this, we get a result paralleling that for precise picking strategies: to avoid being ruled out as impermissible, one's strategy needs to $\mu$-surely be an $\EU_p$ strategy for some $p\times\mu\in\IB$. (Or equivalently for the conditional version.)
	Suppose $\mathbb{N}$ is a set of picking strategies, and there is no $p \times \mu$ in $\IB$ such that all strategies in $\mathbb{N}$ $\mu$-surely pick for $\EU_p$; then each probability $\pb=p\times \mu$ evaluates the precise picking strategy $\{\sigma^p\}$, where $\sigma^p$ picks for $\EU_p$, to be better, in this sense: for every $\sigma\in\mathbb{N}$, $\Exp_p[\U(\sigma)]\leq \Exp_p[\U(\sigma^p)]$, and there is some $\sigma\in\mathbb{N}$ with $\Exp_p[\U(\sigma)]<\Exp_p[\U(\sigma^p)]$. 
%	 
%	 	It is, however, already strong enough to rule out an imprecise picking strategy for $\EAd_\IP$ which is regular, i.e., where every $\sigma\in\mathbb{N}$ is a regular picking strategy for $\EAd_\IP$. This is because, for each $\sigma\in\mathbb{N}$, $\Exp_{p\times\mu}[\U(\sigma)]\leq\Exp_{p\times\mu}[\U(\sigma^p)]$, where $\sigma^p$ picks for $\EU_p$, and there is some $\
%	 	and thus, $\pb=p\times\mu$ evaluates the precise picking strategy $\{\sigma^p\}$ to be better, in this sense; so $\mathbb{N}$ is deemed impermissible by this extension of E-Admissibility.  
...
%%	In our case, a set of probabilistic picking strategies, $\mathbb{N}$, is not deemed impermissible in this way iff there exists some $\sigma\in\mathbb{N}$ which $\mu$-surely picks for some $\EU_p$, for some $p \times \mu$ in $\IB$. But this is already strong enough to rule out any imprecise picking strategies which only contain regular probabilistic picking strategies. 
%	This is because each probability $p\times\mu$ in $\IB$ will evaluate every such regular $\sigma\in\mathbb{N}$ to be worse than a $\EU_p$ strategy, $\sigma^p$ (by \cref{thm:ead-nu-reg-nec}). So for each $\pb$ in $\IB$, there is an alternative $\mathbb{N}_p=\{\sigma^p\}$, where $\sigma^p$ picks for $\EU_p$ which is preferable. 
%	
...
%%	
%	If $\mathbb{N}$ is a set of picking strategies, and there is no $p \times \mu$ in $\IB$ such that all strategies in $\mathbb{N}$ $\mu$-surely pick for $\EU_p$, then $\mathbb{N}$ is judged  impermissible by this slightly strengthened account. This is because each probability $p\times \mu$ evaluates the precise picking strategy $\{\sigma^p\}$, where $\sigma^p$ picks for $\EU_p$, to weakly dominate it, i.e., every $\sigma\in\mathbb{N}$, $\Exp_p[\U(\sigma)]\leq \Exp_p[\U(\sigma^p)]$, and there is some $\sigma\in\mathbb{N}$ with $\Exp_p[\U(\sigma)]<\Exp_p[\U(\sigma^p)]$. 
%	
...
%%	We would not expect the analogue results to apply for Maximality because our results show for each $\sigma$ there's a $\sigma'$ which is better according to every probability, but there's no guarantee of a single $\sigma'$ which is better than all the $\sigma\in\mathbb{N}$. 
%
...
%%We would not expect to obtain similar challenges for extending Maximality, as we can only show that each $\sigma$ has a $\sigma'$ which is preferable, but there's no guarantee. 
...
%When we assume that one's probability over which decision she'll be faced with is precise, we showed that for every $\sigma$ which is not $\mu^*$-surely an $\EU_p$ strategy we get some $\sigma'$ which every probability $p$ agrees is preferable. But we can't guarantee that given a set of $\sigma$s, $\mathbb{N}$, we get any agreement on which $\sigma'$ are preferable.
...
%	Let's now apply this understanding to our case. Let $\PowN$ be the set of all sets of probabilistic picking strategies, and let $\N$ be one such set of these strategies. Then $\N$ is in $\EAd_\IB(\PowN)$ iff there is $\sigma$ in $\N$ that $\mu$-surely picks for $\EU_p$. After all, in that case, for any alternative $\N'$ and any $\sigma'$ in $\N'$, $\Exp_{p\times \mu}[\sigma] \geq \Exp_{p\times \mu}[\sigma']$. So, by \cref{thm:ead-nu-reg-nec}, if $\N$ contains only regular probabilistic picking strategies that pick for $\EAd_\IP$, then $\N$ is not in $\EAd_\IB(\PowN)$.
...
%
%In this context, we can extend the results about E-Admissibility. Applying our definition, we have that $\N\in\EAd_\IB(\PowN)$ iff there is some $\sigma\in\N$ and $p\times \mu\in \IB$ such that 
%
%
...
%\begin{enumerate}[{\normalfont (i)}]
%\item A set $\N$ of probabilistic picking strategies is in $\EAd_\IB(\PowN)$ iff there is $\langle p, \mu \rangle$ in $\IB$ and $\sigma$ in $\N$ such that $\sigma$ $\mu$-surely picks for $\EU_p$. (This is an extension of \cref{thm:ead-nu-nec-suff}.)
%\item Suppose each $\sigma$ in $\N$ is regular. Then, by \cref{thm:ead-nu-reg-nec} there is no $\sigma$ which is $\N$ is not in $\EAd_\IB(\PowN)$, where $\IB$ is as in that theorem.
%%\item Suppose there is some $\sigma$ which does not $\mu$-surely pick for $\EU_p$ for any $p\times \mu\in \IB$, then 
%%
%%\item Suppose each $\sigma$ in $\N$ is (a) regular and (b) $\mu^*$-surely picks for $\EAd_\IP$; where $\mu^*$ is precise, requires almost everywhere decisiveness, and countably additive, then $\N$ is not in $\EAd_\IB(\PowN)$. 
%%(This is a corollary of \cref{thm:ead-nu-reg-nec}.)
%%\item Suppose that $N$ 
%\end{enumerate}
%
% However, we don't get any result analogous to \cref{thm:max-nu,thm:max-nu-reg-nec}. In those theorems, we showed that, if $\mu^*$ is precise, requires almost everywhere decisiveness, and countably additive, then, for any $\sigma$ that doesn't $\mu^*$-surely pick for $\EU_p$, for some $p$, there is an alternative $\sigma'$ that is better than $\sigma$ in expectation. And this is sufficient to give us the two analogous results for E-Admissibility just stated. But it is not sufficient to give the analogous results for Maximality, because there is no guarantee that it is the same $\sigma'$ that is better in expectation than each $\sigma$ in $\N$.
%
%
...
We noted that all our previous results generalised to this setting and moreover, that the situation became worse for the imprecise decision theories. E-Admissibility now judges as impermissible any regular $\sigma$ which picks for it, as it no longer looks like $\EU_p$ for any $p$, at least given some mild assumptions on the credal set. 
For Maximality, we were able to show something similar, that it deems impermissible any regular $\sigma$ that picks for it, although in this case we needed a much stronger assumption: that one's uncertainty over which decision she'll be faced with is governed by a precise probability, along with a broadness assumption that this requires almost everywhere decisiveness. 
...
%\section{Setup?}
%We will work in the appendix throughout with $\sigma$. Recall the definitions from \cref{def:nu stuff}
...
\begin{theorem}\label{thm:EU appendix}Suppose $p$ is a probability function over $\Omega$ and $\mu$ is a measure over $\D$. Then:
	\begin{enumerate}[{\normalfont (i)}]
		\item  If $\sigma$ $\mu$-surely picks for $\EU_p$ then, for any $\sigma'$, $\EU_{p\times\mu}[\U(\sigma)]\geq\EU_{p\times\mu}[\U(\sigma')]$
		\item If $\sigma$ $\mu$-surely picks for $\EU_p$, and $\sigma'$ does not, then $\EU_{p\times\mu}[\U(\sigma)]>\EU_{p\times\mu}[\U(\sigma')]$.
	\end{enumerate}
\end{theorem}
\begin{proof}
	...
	Since $\sigma_D$ is a probability over $D$, we know that $\Exp_{\sigma(D)}\Exp_p[\U]\leq m_D(p)$ with equality only if $\sigma_D(\EU_p(D))=1$. And note that $\Exp_{\sigma(D)}\Exp_p(a)=\Exp_p[\U(\sigma)(D)]$
%	
%	{\color{cyan}
%		If $P$ is probability over $\Omega\times\D$, 
%		$\Exp_P[\U(\sigma)]=\Exp_{P(\D)}\Exp_{P(\cdot|D)}\U(\sigma)$. This is a standard fact about iterated expectations. We have shown that $\Exp_{P(\cdot|D)}\U(\sigma)\leq m_D(p(\cdot|D))$, with equality iff $\sigma_D(\EU_{P(\cdot|D)}(D))=1$. Thus, $\Exp_P[\U(\sigma)]\leq\Exp_{P(\D)} m_D(p(\cdot|D))$ with equality only if $P(\{D\given\sigma_D(\EU_{P(\cdot|D)}(D))=1 \}=1$, i.e., only if $\sigma$ $P$-surely picks for $\EU_P$.)
%		
%%		$\Exp_P[\U(\sigma)]=\sum_{D\text{ s.t., }P(D)\neq 0} P(D)\sum_\omega \frac{P(D\wedge\omega)}{P(D)}\U(\sigma,D,\omega)$
%		}
		
	Thus, $\EU_{p\times\mu}[\U(\sigma)]\leq \Exp_\mu [t(p)]$ with equality only if $\mu\Set {D \given \sigma_D(\EU_p(D))=1} = 1$, i.e., only if $\sigma$ $\mu$-surely picks for $\EU_p$. 
...
%	
%	Thus, if $\sigma_D(\EU_p(D))=1$, then $\Exp_{\sigma(D)}\Exp_p(a)=m_{p,D}$. 
%	And if $\sigma_D(\EU_p(D))\neq 1$, then $\Exp_{\sigma(D)}\Exp_p(a)<m_{p,D}$.
%	
%	Thus, if $\sigma$ does $\mu$-surely pick for $\EU_p$, i.e., $\mu\{D\given \sigma_D(\EU_p(D))=1\}=1$, then $\Exp_\mu\Exp_{\sigma(D)}\Exp_p(a)=\Exp_\mu m_{p,D}$
%	
%	If $\sigma$ does not $\mu$-surely pick for $\EU_p$, then $\mu\{D\given \sigma_D(\EU_p(D))\neq 1\}>0$, then $\Exp_\mu\Exp_{\sigma(D)}\Exp_p(a)<\Exp_\mu m_{p,D}$. 
%	
%	Observe that $\EU_{p\times\mu}\Exp_{\sigma(D)}a=\Exp_\mu\Exp_{\sigma(D)}\Exp_p(a)$ by interchange of expectations. This obtains our result. \todooldinfo{is this cleaner than RPs writeup?}
\end{proof}
\end{comment}
...
This is because $\Exp_\pb[\U(\sigma)]=\Exp_{\pb_\D}\Exp_{\pb(\cdot|D)}\U(\sigma)$, where $\pb_\D$ is the marginal. Each $\Exp_{\pb(\cdot|D)}\U(\sigma)$ is maximised when $\sigma_D(\EU_{\pb(\cdot|D)}(D))=1$, at least, whenever this is well-defined. So, similarly, $\Exp_\pb[\U(\sigma)]$  is maximised when $\pb_\D\{D\given \sigma_D(\EU_{\pb(\cdot|D)}(D))=1\}=1$.
\end{comment}

%Since, 
%		If $P$ is probability over $\Omega\times\D$, 
%$\Exp_P[\U(\sigma)]=\Exp_{P(\D)}\Exp_{P(\cdot|D)}\U(\sigma)$. This is a standard fact about iterated expectations. We have shown that $\Exp_{P(\cdot|D)}\U(\sigma)\leq t_D(p(\cdot|D))$, with equality iff $\sigma_D(\EU_{P(\cdot|D)}(D))=1$. Thus, $\Exp_P[\U(\sigma)]\leq\Exp_{P(\D)} t_D(p(\cdot|D))$ with equality only if $P(\{D\given\sigma_D(\EU_{P(\cdot|D)}(D))=1 \}=1$, i.e., only if $\sigma$ $P$-surely picks for $\EU_P$.)
...
\begin{itemize}
	...
	\item $\O$ is a set of ``options''. In our application they will be various picking functions, $\sigma$.
	\item $\Uwald:\O\times\Omega\to [l,u]$ is a bounded ``utility function''. In our application, it will be $\Uwald(\sigma,\omega)=\Exp_{\mu^*}\U(\sigma,\omega)=\Exp_{\mu^*}\Exp_{\sigma_D}\U(a)(\omega)$ \begin{itemize}
...
\item $\Uwald(\sigma)(\omega):=\Exp_{\mu^*}\U(\sigma)(\omega)=\Exp_{\sigma_{D}} \U(a)(\omega)$
\end{itemize}
...
\end{lemma}
\begin{proof}
	Observe that $\sigma$ is Bayes for $p$ iff it ${\mu^*}$-surely picks for $\EU_p$ using \cref{thm:EU appendix}. %\todooldinfo{where's the ref??} 
\end{proof}
...
\begin{lemma}\label{thm:cct-appln:cts}
	If ${\mu^*}$ requires almost everywhere decisiveness (and countably additive), then $\Uwald(\sigma)(\omega):=\Exp_{\mu^*}\U({\sigma_{D}})(\omega)=\Exp_{\mu^*}\Exp_{\sigma_D}\U(a)(\omega)$ is Bayes continuous. 
\end{lemma}
\begin{proof}
Suppose $p^*$ is a probability function over $\Omega$. And suppose $p_1,p_2\ldots$ is a sequence of probability functions over $\Omega$ that converges on $p^*$, that is, $p_1,p_2\ldots \longrightarrow p^*$. We will show that for any $\sigma^{p^*}$ which is Bayes for $p^*$ and $\sigma^{p_n}$ which are Bayes for $p_n$, then $\Uwald(\sigma^{p_n})(\omega)\longrightarrow\Uwald(\sigma^{p^*})(\omega)$. 

Now suppose that $D$ is such that: (i) $\EU_{p^*}(D)= \{a^*\}$, (ii) $\sigma^{p_n}_D(\EU_{p_n}(D))=1$, and (iii) $\sigma^{p^*}_D(\EU_{p^*}(D))=1$, so that $\sigma^{p^*}_D(\{a^*\}) = 1$. 
For every $a$,  $\Exp_{p_n}(a)\longrightarrow\Exp_{p^*}(a)$. If $D$ is finite, there must be some $N$ such that for all $n>N$, also $\EU_{p_n}(D)=\{a^*\}$. And then $\U(\sigma^{p_n})(\omega, D)=\Exp_{\sigma^{p_n}_D}a(\omega)=a^*(\omega)= \Exp_{\sigma^{p^*}_D}a(\omega) = \U(\sigma^{p^*})(\omega, D)$.  
...
%$\epsilon>0$, then there is an $N$ such that for all $a_n\in\EU_{p_n}(D)$ there is some $a^*\in\EU_{p^*}(D)$ with $\sum_\omega |a(\omega)-a^*(\omega)|<\epsilon $. Since there is only one such $a^*$ by assumption that $\EU_{p^*}(D)=\{a^*\}$, in fact, for all $\epsilon>0$, there is $N$ such that $\Exp_{\sigma_D^{p_n}} a(\omega)$ 
% so if $a\neq a^*$, 
% $$\U(\sigma^{p_n})(\omega, D)=\Exp_{\sigma^{p_n}_D}a(\omega)\longrightarrow \Exp_{\sigma^{p^*}_D}a(\omega) = a^*(\omega)=\U(\sigma^{p^*})(\omega, D).$$ 
...
$$\Exp_{\mu^*}\U(\sigma^{p_n})(\omega) \longrightarrow \Exp_{\mu^*} \U(\sigma^{p^*})(\omega)$$which is what we wish to prove.

We have supposed that $\sigma^{p^*}$ is Bayes for $p^*$ and $\sigma^{p_n}$ are Bayes for $p_n$. Thus, by \cref{thm:EU appendix}, $\sigma^{p^*}$ ${\mu^*}$-surely picks for $\EU_{p^*}$
and $\sigma^{p_n}$ ${\mu^*}$-surely pick for $\EU_{p_n}$. 
Let:
\begin{itemize}
	\item $\X^* = \Set{D \in \D \given \sigma^{p^*}_D(\EU_{p^*}(D)) = 1 }$
	\item $\X^n = \Set{D \in \D \given \sigma^{p_n}_D(\EU_{p^n}(D)) = 1 }$
...
%	If $\sigma$ does not $\mu^*$-surely pick for $p$, for any probability $p$, then there is some $\sigma'$
	
	If there is no probability $p$ over $\Omega$ for which $\sigma$ maximises $\Exp_{p}[\Exp_{\mu^*}[\U(\sigma)]]$; then there is some $\sigma'$ such that $\Exp_{\mu^*}\U(\sigma')(\omega)>\Exp_{\mu^*}\U(\sigma)(\omega)$ for all $\omega\in\Omega$. 
\end{corollary}
\begin{proof}
...
\begin{corollary}
	...
	If $\sigma$ does not $\mu^*$-surely pick for any probability $p$, then there is some $\sigma'$ such that for all probabilities $p$, $\Exp_{p\times\mu^*}[\U(\sigma')]>\Exp_{p\times\mu^*}[\U(\sigma)]$. 
\end{corollary}
\begin{proof}
	From \cref{thm:EU appendix}, such $\sigma$ thus does not maximise $\Exp_{p\times\mu^*}[\U(\sigma)]$ for any $p$. Observe, also that $\Exp_p[\Exp_{\mu^*}[\U(\sigma)]]=\Exp_{p\times\mu^*}[\U(\sigma)]$. So by \cref{thm:cct-applns:corr}, there is some $\sigma'$ with  $\Exp_{\mu^*}\U(\sigma')(\omega)>\Exp_{\mu^*}\U(\sigma)(\omega)$ for all $\omega$; and thus, $\Exp_{p}[\Exp_{\mu^*}[\U(\sigma')]]>\Exp_p[\Exp_{\mu^*}[\U(\sigma)]]$ for all probabilities $p$; which gives us the claim.
\end{proof}
...
%	 	If $\sigma$ does not $\mu^*$-surely pick for any probability $p$, then it is not a maximiser of $\Exp_p[\Uwald(\sigma)]=\Exp_p[\Exp_{\mu^*}\U(\sigma)]=\Exp_{p\times\mu^*}[\U(\sigma)]$ for any $p$; i.e., it is not Bayes.
%	 	
%	 	It is thus is dominated, i.e., there is $\sigma'$ with $\Uwald(\sigma')(\omega)>\Uwald(\sigma)(\omega)$ for all $\omega$; i.e., $\Exp_{\mu^*}\U(\sigma')(\omega)>\Exp_{\mu^*}\U(\sigma)(\omega)$ for all $\omega$; and thus, $\Exp_{p}\Exp_{\mu^*}[\U(\sigma')]>\Exp_p\Exp_{\mu^*}[\U(\sigma)]$ for all probabilities $p$. 
%	 \end{proof}
%}
