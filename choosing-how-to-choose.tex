...
\begin{definition}\label{def:cond}
	...
	That is, $\EU_{\pb(\cdot|-)}(D):=\{a\in D\given \text{for all $a'\in D$, }\Exp_{\pb(\cdot|D)}[\U(a)]\geq \Exp_{\pb(\cdot|D)}[\U(a')]\}$
%	 that is, $a\in\EU_{\pb(\cdot|-)}(D)$ iff $\Exp_{\pb(\cdot|D)}[\U(a)]

$\pb_\Decs$ is the marginal probability measure on $\Decs$ generated by $\pb$, that is $\pb_\D(E)=\pb(\Omega\times E)$ for $E\subseteq\Decs$. 
...
$\s$ \emph{$\pb_\Decs$-surely picks for $\EU_{\pb(\cdot|-)}$} iff \[\pb_\Decs\{D\given \s(D)\in EU_{\pb(\cdot|-)}(D)\}=1\]
\end{definition}
