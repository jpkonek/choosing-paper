...
%\end{proposition}
%}
The upshot is that E-Admissibility is not self-undermining in the same way that $\Gamma$-Maximin is self-undermining. So long as there is some $p\in\IP$ with some $p\times\mu\in\IB$,\todo{reread} then there are strategies that pick for it that it does not deem impermissible.
...
\begin{proposition}\label{thm:max-nu-suff}
		...
		More generally, if for some $\pb\in \IB$, $\prpickstrat$ $\pb$-surely picks for $\EU_{\pb(\cdot|-)}$, then $\prpickstrat\in\Maximality_\IB(\PrPickStrategies)$. 
\end{proposition}
...
\begin{proposition}\label{thm:max-nu-reg-nec}
	...
	Suppose that for every probability $p$, $\mu^*\{D\given \Maximality_\IP(D)\subseteq\EU_p(D)\}< 1$.\footnote{Equivalently, that for every $p$ there is a non-negligible set of decision problems $D$ for which there is some $a\in D$ which is not an $\EU_p$ act but which is Maximal: $\mu^*\{D\given \exists a\in D\left[(\exists b{\in} D \, \Exp_{p}(a)<\Exp_{p}(b))\text{ and }\forall b{\in} D\, \exists p'{\in}\IP \,\Exp_{p'}(b)\leq \Exp_{p'}(a)\right]\}>0$}\todo{this might actaully need to say every p rather than every p in P, no??? It's supposed to say, it's not an EU strategy, so that we can use the fact that it does not mu surely pick for any EUp???!!!}
...
Since $\Gamma$-Maximin is a more restrictive theory than Maximality, for a strategy to be $\Gamma$-Maximin there must be some $p\in\IP$ \todo{why in IP???}for which it $\mu^*$ surely picks for
...
\begin{colored}{violet}
	...
	Throughout the paper, we ignored the question of measurability. 
	
	$\Decs$ was just defined as finite subsets of an arbitrary non-empty set $\A$. 
	
	Fixing $\U:\A\times\Omega\to\Re$, we can instead treat $\A$ as $\Re^{|\Omega|}$. The measure space is given by the Lebesgue measurable sets. 
	Since each $D\in\Decs$ is a finite subset of $\A$, the measure space is given by ??
	
	All the results in this paper should be qualified to quantify only over measurable picking strategies $\s:\Decs\to\A$, or probabilistic picking strategies $\prpickstrat:\Decs\to\Delta(\A)$. 
...
When we have decision-state dependence, we made use of the idea of conditional probabilities. For $b$, a probability function over $\Omega\times\Decs$, we considered $b(\cdot | D)$, probability functions over $\Omega$ resulting from conditionalising $b$ on which decision problem is being faced with (\cref{sect:decdep}).

Let $b_\Decs$ be the marginal on $\Decs$. 
We need to check that we can define $b(\cdot|D)$ for $b_\Decs$-almost all $D$ and ensure that for bounded random variables $X:\Omega\times\Decs\to\Re$, (in particular, for our $\U(\s)$ or $\U(\prpickstrat)$), $\Exp_b[X]=\Exp_{b_\Decs}[\Exp_{b(\cdot|D)}[X]]$.

When $\Decs$ is supported on a finite subset of $\Decs$, we can just define this by the ratio formula for all $D$ with $b_\Decs(D)>0$. 

In the more general setting, we can do it due to the Regular Conditional Probability theorem. 

%\begin{proposition}
	We can define the regular conditional probability kernel, 	$b(\cdot|D)$ for $b_\Decs$ almost every $D$, with $\Exp_b[X]=\Exp_{b_\Decs}[\Exp_{b(\cdot|D)}[X]]$ for any integrable random variable $X:\Omega\times\Decs\to\Re$. 
%\end{proposition}
...
\begin{theorem}\label{thm:EU appendix} 
	\hspace*{1mm}
	\begin{enumerate}[label=\normalfont(\roman*), ref=\roman*]
		\item  If $\prpickstrat$ $\pb$-surely picks for $\EU_{\pb(\cdot|-)}$ then, for any $\prpickstrat'$, $\Exp_\pb[\U(\prpickstrat)]\geq\Exp_\pb[\U(\prpickstrat')]$
		\item  If $\prpickstrat$ $\pb$-surely picks for $\EU_{\pb(\cdot|-)}$ and $\prpickstrat'$ does not, then $\Exp_\pb[\U(\prpickstrat)]>\Exp_\pb[\U(\prpickstrat')]$.
	\end{enumerate}
\end{theorem}
\begin{proof}
...
We have $\Exp_{b}[\U(\prpickstrat)]=\Exp_{b_\Decs}[\Exp_{b(\cdot | D)}[\U(\prpickstrat)]]$, as $\U(\prpickstrat)$ is a measurable random variable on $\Omega\times\Decs$ given by $\U(\prpickstrat)(\omega,D)=\sum_{a\in D}\prpickstrat_D(a)\U(a)(\omega)$. 
	
Consider any $D^*\in\D$ for which $b(\cdot\given D^*)$ is well defined. 
...
%\end{align}	
\begin{align}
	\Exp_{b(\cdot | D^*)}[\U(\prpickstrat)(D^*)]&=	\sum_{\omega\in\Omega}b(\omega| D^*)\sum_{a\in D^*}\prpickstrat_{D^*}(a)\; \U(a)(\omega)&&\text{definition of $\U(\prpickstrat)$}\\
...
$
obtains its maximum at $\Exp_{b_\Decs}[t_b(D)]$ when $\prpickstrat_D(\EU_{b(\cdot|-)}(D))=1$ for $b_\D$-almost all $D$, i.e., when $\prpickstrat$ $b$-surely picks for $\EU_{b(\cdot|-)}$.
\end{proof}
...
\begin{theorem}\label{thm:EU appendix} 
	\begin{enumerate}[label=\normalfont(\roman*), ref=\roman*]
	\item  If $\nu$ $\pb$-surely picks for $\EU_{\pb(\cdot|-)}$ then, for any $\nu'$, $\Exp_\pb[\U(\nu)]\geq\Exp_\pb[\U(\nu')]$
	\item  If $\nu$ $\pb$-surely picks for $\EU_{\pb(\cdot|-)}$ and $\nu'$ does not, then $\Exp_\pb[\U(\nu)]>\Exp_\pb[\U(\nu')]$.
\end{enumerate}
\end{theorem}
\begin{proof}
