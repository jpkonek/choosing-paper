...
%	Put $\Sigma$ as the smallest $\sigma$-algebra containing each $\{D\in\Decs\given \#(D\cap A)=k\}$ for $A\in\mathcal{B}(\A)$ and $k\in\mathbb{N}\setminus\{0\}$.
%	Equivalently, it can be seen as the countable disjoint union of $k$-tuples in $\Re^n$, modulo permutations, with the obvious inherited $\sigma$-algebra.\todo{more care}
	...
	We then require any probability $p$$\mu$ on $\Decs$ to be a probability measure on $(\Decs,\Sigma)$; and $\pb$ to be a probability measure on $(\Omega\times\Decs,\mathcal{F}\otimes \Sigma)$
...
\begin{align}
	...
	&=\sum_\omega\int_\D X(\omega,D) b_\omega (\diff D)\\
		&=\sum_\omega\int_\D X(\omega,D) b(\omega |D) b_\D(\diff D)&&\text{integration by parts??}\\
