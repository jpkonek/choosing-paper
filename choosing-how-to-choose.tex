...
Then $b(\cdot|D)$ is defined and probabilistic for $b_\Decs$ almost every $D$ (and $D\mapsto b(\cdot|D)$ is measurable). Moreover, and importantly for us, the law of total expectation holds: for measurable integrable random variables $X:\Omega\times\Decs\to\Re$, $\Exp_b[X]=\Exp_{b_\Decs}[\Exp_{b(\cdot|D)}[X]]$. We will in particular be applying this to $\U(\s)$ and $\U(\nu)$. 
...
If $\U(\A)$ is assumed to be a Borel subset of $\Re^\Omega$, then our spaces are standard Borel so the regular conditional probability theorem can be used to find this  \citep[Theorem 3.4]{kallenberg1997foundations}.
As $\Omega$ is finite, though, it can also be constructed directly using Radon-Nikodym derivatives. 
...
\begin{sublemma}
	...
	For any $p\neq q$, both $\{g\in V\given \Exp_p[g]>0>\Exp_q[g]\}$ is non-empty and open.
\end{sublemma}
\begin{proof}
	{It is non-empty}:
	First, observe that there is some $g\in\Re^\Omega$ such that $\Exp_p[g]>0>\Exp_q[g]$. For example, with $A$ such that $p(A)\neq q(A)$, we can put $g=\mathbf{1}_A-\frac{p(A)-q(A)}{2}\mathbf{1}$; or the negative of this if required.\todo{cleaner eg??} 
...
Thus, for any $(p,\mu)\in\IB$, $\mu(\{\{a,0\}\given \U(a)\in V\text{ and }\Exp_p[\U(a)]>0>\Exp_q[\U(a)] \})>0$. Note also that for $D=\{a,0\}$ with $\Exp_p[\U(a)]>0>\Exp_q[\U(a)]$, then $\EU_{p}(D)=\{a\}$ and $\EU_q(D)=\{0\}$, so they are disjoint. 

We thus have that $\mu(\{\})$


We can find $V_1$ and $V_2$  disjoint non-empty subsets of $[l,h]^\Omega$ such that for each $i\in\{1,2\}$ and each $g\in \Re^\Omega$ there is some $\lambda>0$ such that $\lambda g\in V_i$.
For example, we could put  $V_1:=\{g\in[l,h]^n\given \|g\|<0.5\}$ and $V_2:=\{g\in[l,h]^n\given \|g\|>0.5\}$ (assuming that $l<-0.5<0.5<h$). 

Put $E_1:=\{\{a,0\}\given a\in V_1\}$ and $E_2:=\{\{a,0\}\given a\in V_2\}$.

Then for any $(p,\mu)\in \IB$ if $q$ is distinct from $p$, then $$\mu\{D\in E_i\given \EU_p(D)\cap\EU_q(D)=\emptyset\}>0$$ for both $E_1$ and $E_2$. 



Then take any distinct $q^*_1,q^*_2\in\IP$. Put $E_{q^*_1}=E_1$ and $E_{q^*_2}=E_2$ and $E_q=\emptyset$ for $q\notin\{q^*_1,q^*_2\}$ and observe that it satisfies the conditions of  \cref{thm:appendix:ead:existsimpermissibe:conditiontwoprobs[indep]}.
\end{proof}
