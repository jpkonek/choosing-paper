...
\begin{theorem}[CCM]\label{thm:ead-existsimpermissibel[indep]:CCM-1}
	...
	Suppose $\{p_i\given i\in I\}\subseteq \IP$ and $E_i\subseteq\Decs$ are disjoint  such that for all $(p,\mu)\in\IB$, $\exists i\in I$ with\begin{itemize}
		\item  $\mu(E_i)>0$ and 
		\item $\mu\{D\in E_i\given \EU_{p_i}(D)\cap \EU_p(D)\neq \emptyset\}=0$.
	\end{itemize}
...
\begin{proof}
	...
	Take any $p\times\mu\in\IB$. For at least one of $i\in I$, 
	$\mu\{D\in  E_i \given \EU_p(D)\cap\EU_{p_i}(D)\neq\emptyset\}=0$. 
	So $\mu\{D\in E_i\given \s(D)\notin\EU_p(D)\}\geq \mu\{D\given \s(D)\in\EU_{p_i}(D)\}\geq \mu(E_i)>0$. 
	Thus $\s$ does not $\mu$-surely pick for $\EU_p$.
\end{proof}

It can also be extended to allow for proper distributions by allowing each $\mu$ having some $J\subseteq I$ st: \begin{itemize}
	\item  $\mu(\bigcup_{j\in J} E_j)>0$ and 
	\item $\mu(\bigcup_{j\in J}\{D\in E_j\given \EU_{p_j}(D)\cap \EU_p(D)\neq \emptyset\})=0$.
\end{itemize}
...
\todo{JK version of \cref{thm:ead-existsimpermissibel[indep]:RP-1}}
\begin{proposition}\label{thm:ead-existsimpermissibel[indep]:JK-2}
