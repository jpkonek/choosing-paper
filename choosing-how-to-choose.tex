...
Let $\c$ be any choice function. Let $r:\Decs\rightarrow[0,1)$ be any measurable statistic, \textit{e.g.},  $r(D)=\Bigl(\sum_{x\in\c(D)}\|x\|^{2}\Bigr)\bmod 1$. 
...
\item[(A2)] \textbf{Menu richness\!.}\\[2pt]
      ...
      \[
        \mu\bigl(\,D\in B_{p}\given r(D)\in I \bigr)>0,
      \]
...
Under \textup{(A1)} and \textup{(A2)} there exists a measurable function
$
  s:\Decs\to\mathbb \A
$
such that
...
Define
\[
  k(D)=\min\bigl\{j:r(D)\in I_{j}(D)\bigr\},
  \qquad
  s(D)=x_{k(D)}(D).
\]
...
Define
$
  A_{p}:=\{D\in B_{p}\given r(D)\in I_{\beta(D)}(D)\}.
$
...
\section{CCM}
\begin{theorem}
	If $\mu^*$ is atomless and $\mu^*\{D\given \c(D)\subseteq\EU_p(D)\}<1$ then there is $\s$ which picks for $\c$ but for no $p$ does not $\mu^*$-surely pick for $\EU_p$. 
\end{theorem}
\begin{proof}
	Let $F_p(D):=\sup_{a \in D} q\cdot a-\min_{b\in \c(D)}q\cdot b$
	
	$F_p(D)\geq 0$ and $F_p(D)>0$ iff $\c(D)\not\subseteq\EU_p(D)$. 
	
	As $\mu^*\{D\given  F_p(D)>0\}>0$ by countable additivity there is some $m(q)\in\mathbb{N}$ such that $\mu^*\{D\given  F_p(D)>\nicefrac{1}{m(q)}\}>0$.
	
	Let $C_q:=\{D\given f_q(D)>\nicefrac{1}{m(q)}\}$. 
	
	
	Put $M=\max\{|l|,|h|\}$. 
	\begin{sublemma}
		If $\|p-q\|<\nicefrac{1}{2Mm}$ and $\max_{y \in D} q\cdot y-q\cdot x>\nicefrac{1}{m}$ then $\max_{y \in D} p\cdot y-p\cdot x>0$, so that $x\notin\EU_p(D)$.
	\end{sublemma}
	\begin{proof}
		Suppose $q\cdot y^*-q\cdot x>\nicefrac{1}{m}$. 
		\begin{align}
			p\cdot y^*-p\cdot x&=q\cdot y^*-q\cdot x+(p-p)\cdot(y^*-x)\geq \nicefrac{1}{m} -\|q-p\|\|y^*-x\|_\infty\geq  \nicefrac{1}{m} -\nicefrac{1}{2Mm}*M=\nicefrac{1}{m}>0
		\end{align}
	\end{proof}
	
	
	Let $U_q:=\{p\in\Prob\given \|p-q\|<\nicefrac{1}{2Mm}\}$.
	
	These are open, and cover $\Prob$ as $p\in U_p$. 
	
	So by compactness, there is y finite sub-cover, $U_{q_1},\ldots, U_{q_n}$. 
	
	Let $C_i:=\{D\given f_{p_i}(D)>\nicefrac{1}{m(p_i)}\}$
	
	\begin{sublemma}
		We can choose $E_i\subseteq C_i$ disjoint such that $\mu^*(E_i)>0$
	\end{sublemma}
	\begin{proof}
	...
	Then put $\s(D)\in\argmin_{x \in \c(D)}q_i\cdot x$ for $D\in E_i$, and $\s(D)\in \c(D)$ for $D\notin\bigcup_{i=1,\ldots,k} E_i$. 
	
	Observe that $\s(D)$ picks for $\c$ by definition. 
	
	And for any $p\in\Prob$, $p\in U_{q_i}$ for some $i$, and so by our sublemma,  $x\notin\EU_p(D)$.
\end{proof}
