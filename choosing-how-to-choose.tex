...
\begin{colored}{violet}
\section{Decision-State Dependence}
\newcommand{\Decs}{\mathcal{D}}

$b$ is a probability over $\Omega\times\Decs$. \todo{What is the measure space on $\Decs$???}
In this section, we spell out the marginal $b_\D$ and the conditional $b(\cdot |D)$ ensuring that $\Exp_b[X]=\Exp_{b_\D}[\Exp_{b(\cdot|D)}[X]]$

Let $b_\omega$ a measure on $\Decs$ given by $b_\omega(E)=b(\{\omega\}\times E)$ for $E\subseteq D$ measurable. Put $b_\D=\sum_{\omega\in\Omega} b_w$, is the marginal of $b$ on $\D$. 

As $b_\D$ is the finite sum of these $b_\omega$, they are absolutely continuous with respect to it; and thus by Radon–Nikodym theorem, for each $\omega$ there is a function $f_\omega$ on $\Decs$ such that for all measurable $E\subseteq\Decs$, $b_\omega(E)=\int_{E}f_\omega(D) b_\D(\diff D)$. 
Put $b(\omega |D):= f_\omega(D)$. 

Since $b_\D=\sum_{\omega\in\Omega}b_\omega$, one can check that for $b_\D$ almost all $D$, $b(\cdot |D)$ is a probability measure. 
...
\begin{align}
	\Exp_b[X]&=\int_b X(\omega, D) b(\diff \omega,\diff D)\\
