...
This is a \emph{prima facie} bad feature of her decision theory. Her decision theory makes recommendations both about \emph{picking strategies} and which \emph{actions} to choose in different decision problems. But these recommendations pull her in different directions. The picking strategy it recommends tells her to do one thing when faced with $D^{\mathrm{Allais}}_{\mathrm{local}}$, and the decision theory itself tells her to do another thing. There is a conflict in the theory's recommendations. The oddity of the situation is akin to one that David \citet[56]{lewis1971iim} identified in a different context:
\begin{quote}
	It is as if Consumer Bulletin were to advise you that Consumer Reports was a best
	buy whereas Consumer Bulletin itself was not acceptable; you could not possibly
	trust Consumer Bulletin completely thereafter.
...
%This is, \emph{prima facie}, a bad-making feature of a decision theory. 
It is thus a decision theory which is self-undermining, in the sense that given this particular precise probability distribution over the possible decision problems she might face---it's $11\%$ likely she'll face $D^{\mathrm{Allais}}_{\mathrm{local}}$, and $89\%$ likely she'll get \pounds 1m for sure---if she applies her decision theory to the question of which picking strategy to use, it rules out as impermissible its own recommended course of action. 
...
\begin{colored}{red}
Any failure of von Neumann-Morgenstern's Independence Principle will generate a case of undermining. 
Suppose your decision theory deems action $a$ preferable to action $b$.
But if a biased coin is to be tossed to determine whether 
and suppose further that it prefers the mixed option $b$-if-a-possibly-biased-coin-lands-heads-and-$c$-if-tails \todo{C: hard to read! I've actually rewritten}to the mixed option $a$-if-heads-and-$c$-if-tails. Then we can generate a way of being uncertain over which decision problem you'll be faced with so that the unique picking strategy which is compatible with the decision theory is itself impermissible according to the theory. 
\end{colored}
