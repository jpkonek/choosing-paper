...
\end{colored}
		Then if $\n$ is regular for $\Maximin_\IP$ then $\n\notin \Maximin_\IB(\Nu)$. \todo{double check thms... I removed ``in IP''!}\todo{This thm feels false}
	\end{proposition}
...
We will need to show that we have a so-called regular conditional probability kernel.

When $\Decs$ has finite support, we can simply define $b(\cdot|D)$ by the ratio formula on those $D$ with $b(\Decs)>0$. 
It is important for us that $\Exp_b[X]=\Exp_{b_\Decs}[\Exp_{b(\cdot|D)}[X]]$
...
\begin{corollary}\todo{why am I now using g? We were using X as rv, althoguh that was for rv on OmegaxDecs}
	Suppose that $\IB$ makes $\Omega$ and $\Decs$ completely independent. 
...
Suppose $V_1$ and $V_2$ are disjoint non-empty subsets of $[l,h]^\Omega$ such that for each $i\in\{1,2\}$ and each $g\in \Re^\Omega$ there is some $\lambda>0$ such that $\lambda g\in V_i$.
...
There is some $g\in\Re^\Omega$ such that $\Exp_p[g]>0>\Exp_{p^*_i}[g]$. For example, $g=\mathbf{1}_A-\frac{p(A)-p^*_i(A)}{2}\mathbf{1}$ for some $A\subseteq\Omega$ such that $p(A)\neq p^*_i(A)$; or take the negative of this if required. 
