...
%\end{proof}
	...
	\begin{theorem}
		...
		Suppose $\IP$ contains $p_1$ and $p_2$ such that for all $p\times\mu\in\IB$, for at least one of $q\in\{p_1,p_2\}$, 
		$\mu\{D\given \EU_p(D)\cap\EU_q(D)=\emptyset\}=1$. 
		
		Assume that there is some $E\subseteq\Decs$ such that for all $p\times\mu\in\IB$, $0<\mu(E)<1$. \\
		(For this, perhaps all $\mu$ are all absolutely continuous wrt a single $\mu^*$ which is non-atomic...)
	...
	\begin{proof}
		...
		Take any $p\times\mu\in\IB$. By assumption, for at least one of $q\in\{p_1,p_2\}$, 
		$\mu\{D\given \EU_p(D)\cap\EU_q(D)=\emptyset\}=1$. 
		So for $D\in E_i$, since $\s(D)\in \EU_{p_i}(D)$, then $\s(D)\notin\EU_p(D)$, at least $\mu$-surely. 
		Thus $\mu\{D\given \s(D)\notin\EU_p(D)\}\geq \mu(E_i)>0$. 
