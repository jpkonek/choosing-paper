...
\begin{colored}
	...
	\todo[inline]{Say what we'll do? Summary of results}
	
	
	What we'll show throughout this paper is that this idea results in a challenge for a host of theories that diverge from expected utility theory. 
	
	The strength of the results that we show vary in the different cases. 
	
	Decision theories that accommodate risk, by rationalising the Allais preferences, lead to cases where they deem their own recommendations as impermissible. Whilst this is a strong sense in which the theories undermine themselves, it is limited just to particular cases, i.e., particular ways of being uncertain over which decision problem you'll face. We can offer some strengthening of this to some natural way of being uncertain over a much wider range of decisions, however we do not have a general result for a whole host of uncertainty. It is thus still open to a defender of this theory to argue that they are (should?) not be uncertain over which decision will be faced with in such a way. 
	
	Some other decision theories which differ from expected utility theory are those which accommodate ambiguity or imprecision. For one of these, $\Gamma$-maximin, it is similarly undermining as it rationalises the Ellsberg preferences. Two other prominent decision theories for imprecision, however, are E-Admissibility and Maximality. 	
	These theories deem some actions as impermissible but may leave many possible actions incomparable, neither deeming one preferable to the other or treating them as indifferent. There are thus typically many different ways of acting which are compatible with the recommendations of the theory. 
	To apply the decision theory to judge which decision theory to use we are typically judging it by its fruits: the utility of what it recommends you pick. For these imprecise decision theories however, it does not offer univocal advice about what to pick. 
	
	Initially, then, we simply consider the theories judgements about picking strategies which specify what to pick in every possible decision. 
	For E-Admissibility, it deems impermissible any picking strategy which does not correspond to an expected utility picking strategy. Since picking in accordance with expected utility theory is also picking in accordance with E-Admissibility, we do not obtain the sort of strong undermining that we did for theories accommodating risk or $\Gamma$-maximin: it does not rule out all ways of picking which are compatible with itself. However, it does rule some out.
	 It is compatible with E-Admissibility to pick in accordance different probability functions from her credal set when faced with different decision problems. 
	 Such picking strategies, however, are deemed impermissible according to the theory.
	 There is thus a deep seated commitment to a modal coordination across possible decisions, even though it is not coordinating across time, it is coordination in her plans to what to pick if she were to face various decisions. 
	 
	 The theory also deems it impermissible to pick in accordance with some randomisation procedure over the actions which are not ruled out,\todo{something about the utility. }
	 as this does not look like picking in accordance with expected utility theory. 
	 The defender of these theories might then be asked how should one use such non-committal recommendations? It should not be by randomising amongst the options. Instead, one needs to select some probability from her credal set then use that to determine her plan for what to pick, whatever decision she is faced with. 
	 The question of how that differs from a permissivist approach to expected utility theory remains to be investigated. 
	 
	 One might wonder whether we could assign a utility value to the recommendations of the decision theory itself or explicitly consider the utility of adopting a decision theory, whatever that'll lead to, rather than looking at various picking strategies. What will you pick when you've adopted a decision theory which leaves various actions open. It seems natural to us to suggest that you're unsure how you'll pick amongst the options that are not rejected, and you happen to have a precise probability over the possible actions. But this is equivalent to a randomisation procedure. Similarly if we simply specify that the utility of a choice set leaving various options open is a mixture, this again is equivalent to a randomisation procedure. 
	 
	 A further option we consider is imprecise picking strategies, or having imprecise probabilities over how one might pick. We consider how to extend E-Admissibility to judge such imprecise ways of picking. We propose that each probability function judges the imprecise picking strategy as sub-optimal, preferring to adopt a precise expected utility picking strategy; and thus, in the spirit of E-Admissibility, we suggest that it deems this imprecise picking strategy as impermissible too, instead preferring you to select a probability function from the credal set and run with it. 
	 
 	Thus, whilst not strictly undermining in the way that the recommendations of the risk-aware decision theories are, it forces the defender of these theories to more carefully consider what it means to adopt such a theory and how to make use of its advice. It looks very much like permissive expected utility theory. 
	 
	 Maximality is an alternative prominent decision theory for imprecise probabilities which is less committal than E-Admissibility. Thus picking in accordance with expected utility theory according to one of the probabilities in the credal set is not rejected by the decision theory. 
	 We can again show that it deems impermissible any way of picking which does not amount to expected utility theory, at least when one's uncertainty over which decisions you'll face has some particular features. When your uncertainty over which decision you'll be faced with is precise and spread over a sufficiently broad range of decisions, then the only ways to pick which are not impermissible is to pick in accordance with expected utility theory. Again we see, then, that this rules out picking by randomisation, and it requires a modal coordination in how one plans to use the recommendations of the theory. 
...
\todo{The details of this are developed in \cref{sect:appendix:decdep}.}
...
\begin{definition}\label{def:cond}
	...
	That is, $\EU_{b(\cdot|-)}(D):=\{a\in D\given \text{for all $a'\in D$, }\Exp_{b(\cdot|D)}[\U(a)]\geq \Exp_{b(\cdot|D)}[\U(a')]\}$\todo{Should this ``that is'' go into footnote or be deleted?}
%	 that is, $a\in\EU_{b(\cdot|-)}(D)$ iff $\Exp_{b(\cdot|D)}[\U(a)]

$b_\Decs$ is the marginal probability measure on $\Decs$ generated by $b$, that is $b_\D(E)=b(\Omega\times E)$ for $E\subseteq\Decs$.\todo{measurable?}
...
Observe, then, that $\Exp_{\mu^*}\U(\s_{\text{1E,2F}})(\omega)=5$ for each $\omega$ in $\Set{\emph{Red}, \emph{Black}, \emph{Yellow}}$, but $\Exp_{\mu^*}\U(\s_{\text{1F,2E}})(\omega)=6$ for each $\omega$ in $\Set{\emph{Red}, \emph{Black}, \emph{Yellow}}$. So, for any $p$ with $p\times \mu^*\in \IB$,  $\EU_{p\times\mu^*}\U(\s_{\text{1E,2F}})=5$ and  $\EU_{p\times\mu^*}\U(\s_{\text{1F,2E}})=6$; so $\s_{\mathrm{Ellsberg}}=\s_{\text{1E,2F}}\notin\Maximin_{\IB}(\S)$. 
	...
	For instance, we might consider how the agent will choose in $D^{\mathrm{Ellsberg}}_1$ and $D^{\mathrm{Ellsberg}}_2$, individually, and then combine these choices and observe that the result is dominated---1F-and-2E dominates 1E-and-2F. One response is simply to reject the package principle. However, there is another version of the examples in which they are presented diachronically: first evaluate $D^{\mathrm{Ellsberg}}_1$, then $D^{\mathrm{Ellsberg}}_2$. And again we can note that 1F-then-2E dominates 1E-then-2F. And we can note that $\Gamma$-Maximin would still have you choose 2E when faced with $D^{\mathrm{Ellsberg}}_2$, even if you know you've already chosen 1F when faced with $D^{\mathrm{Ellsberg}}_1$. But some will deny that sure loss as a result of decisions at different times indicates irrationality.
		What this example highlights is the close connection between the analysis of this paper and existing challenges and discussions for these theories. Any such Dutch book or sequential choice challenge can be seen as a particular instance where one is unsure which decision problem you'll be faced with, taking them each as equally likely, and evaluating strategies. However, it is a slightly different philosophical question.\todo{...}
...
Do we obtain a converse to \cref{thm:ead-suff-indep}?  Are these the only E-Admissible strategies? 
...
\todo{minor rewrite this tilltle bit above. Reread}



There are always some strategies which pick for $\EAd_\IP$ but which are rejected by $\EAd_\IB(\Strategies)$, at least if $\EAd_\IP$ is not sufficiently precise, i.e., that it does not $\mu$-surely look like $\EU_p$, or a restriction thereof, for any $p\times\mu\in\IB$. 
...
{Suppose $\D$ and $\Omega$ are completely independent in $\IB$ (so that every $b\in\IB$ has the form $p\times\mu$).
	...
	Suppose further that for every $p\times \mu\in \IB$, $\mu\{D\given \EAd_\IP(D)\subseteq \EU_p(D)\}\neq 1$. That is, for all $p\times \mu\in \IB$, $\mu\{D\given \text{there is $p'\in\IP$ with }\EU_{p'}(D)\not\subseteq\EU_p(D)\}>0$. }

%Suppose every $b\in\IB$ has the form $p\times\mu$.
	
	%Suppose that for every $p\times \mu\in \IB$, $\mu\{D\given \EAd_\IP(D)\subseteq \EU_p(D)\}\neq 1$. That is, for all $p\times \mu\in \IB$, $\mu\{D\given \text{there is $p'\in\IP$ with }\EU_{p'}(D)\not\subseteq\EU_p(D)\}>0$. 
	
	Then there is some $\s$ which picks for $\EAd_\IP$ but which is not in $\EAd_\IB(\Strategies)$.\todo{struggling to prove it...!}
\end{proposition}
...
\begin{definition}
	...
	\end{proposition}
\end{colored}
	Whilst some of its picking strategies are not ruled out, some will be, at least if $\EAd_{\IB(\cdot|-)}$ is not a restriction of some precise choice function, $\EU_{b(\cdot|-)}$.
...
Suppose that for every $b\in\IB$, $b_\D\{D\given \EAd_{\IB(\cdot|-)}(D)\subseteq\EU_{b(\cdot|-)}\}<1$. That is, for all $b\in \IB$, $b_\D\{D\given \text{there is $b'\in\IB$ with }\EU_{b'(\cdot|-)}(D)\not\subseteq\EU_{b(\cdot|-)}(D)\}>0$. 
	...
	Then there is some $\s$ which picks for $\EAd_{\IB(\cdot|-)}$ that is rejected by $\EAd_\IB(\Strategies)$.\todo{I'm struggling to prove it... }
...
then $\PrPickStrategies$ will have a particular $\n^\c$ for each $\c$, where $\n^\c$ picks for $\c$. 
...
\begin{definition}\label{def:EU-complete}
	A set of probabilistic picking strategies, $\PrPickStrategies$, is \emph{$\EU$-complete} if, for every probability $p$ over $\Omega$, there is some $\n$ in $\PrPickStrategies$ such that $\n$ picks for $\EU_p$.
	%	
...
$$\n \in \EU_{p\times \mu}(\PrPickStrategies) \Leftrightarrow \n \text{ $\mu$-surely picks for $\EU_p$.}$$
and, more generally, 
$$\n \in \EU_{b}(\PrPickStrategies) \Leftrightarrow \n \text{ $b_\Decs$-surely picks for $\EU_{b(\cdot|-)}$.}$$
\end{proposition}
...
\begin{proposition}\label{thm:max-nu}Suppose $\PrPickStrategies$ is $\EU$-complete. Suppose that 
	...
	Then if $\n \in \Maximality_\IB(\PrPickStrategies)$, then there is some probability $p$ such that $\n$ $\mu^*$-surely picks for $\EU_p$.\todo{did we know that $p$ in $\IP$??}
...
Suppose that $\IB$ has the form $\{p\times \mu^*\given p\in \IP\}$ and $\mu^*$ requires almost everywhere decisiveness.
	...
	Then, if $\n$ is regular picking strategy for $\Maximality$ then $\n\notin \Maximality_\IB(\PrPickStrategies)$.\todo{do we also state that there is a strategy ruled out? Is it affected by our issues with EAd?}
\end{proposition}
...
$\EU_p$, although such strategies may nonetheless be impermissible according to $\Gamma$-Maximin, as we saw, for example, in the Ellsberg case (\cref{sect:gamma}) where the only strategy compatible with $\Gamma$-Maximin was judged impermissible by the theory itself. 
	\begin{proposition}\label{thm:gamma-nu}Suppose $\PrPickStrategies$ is $\EU$-complete. Suppose that 
		$\IB$ has the form $\{p\times \mu^*\given p\in \IP\}$ and $\mu^*$ requires almost everywhere decisiveness. Then if $\n \in \Maximin_\IB(\PrPickStrategies)$, there is some probability $p$ where $\n$ $\mu^*$-surely picks for $\EU_p$.
		
		If also for every probability $p$, $\mu^*\{D\given \Maximin_\IP(D)\subseteq \EU_p(D)\}<1$, then there is some $\n$ which picks for $\Maximin_\IP$ and which is not in $\Maximin_\IB(\PrPickStrategies)$ (in fact, any $\n$ which picks for $\EU_p$ with $p\in\IP$ will do).  And if $\n$ is regular for $\Maximin_\IP$ then $\n\notin \Maximin_\IB(\PrPickStrategies)$. \todo{double check thms... I removed ``in IP''!}
	\end{proposition}
...
%bad: For many $\mu$, every regular $\n$ which picks for $\c$ is not in $\c(\PrPickStrategies)$\newline
%good: 
...
%\end{tabularx}
	...
	Throughout the paper, we have ignored the question of measurability. We have considered various probability measures: $p$ a probability on $\Omega$, $\mu$ a probability on $\Decs$, and $b$ a probability on $\Omega\times\Decs$. To make this precise, we should fix the $\sigma$-algebras on each of these spaces.
	...
	One can then construct a $\sigma$-algebra on $\Decs$ from $\mathcal{B}(\A)$. \todo{I think these are all equivalent.}
	\begin{itemize}
...
%	Equivalently, it can be seen as the countable disjoint union of $k$-tuples in $\Re^n$, modulo permutations, with the obvious inherited $\sigma$-algebra.\todo{more care}
	...
	All the results in this paper are restricted to measurable picking strategies $\s:\Decs\to\A$, or measurable probabilistic picking strategies $\n:\Decs\to\Delta(\A)$ so that the induced payoffs, $\U(\s),\U(\n):\Omega\times\Decs\to\Re$, are measurable random variables with respect to $(\Omega\times\Decs,\mathcal{F}\otimes \Sigma)$, recalling that $\U(\n)(\omega,D):=\Exp_{\n_D}[\U(\cdot)(\omega)]$. 
...
For $b$, a probability measure on $(\Omega\times\Decs,\mathcal{F}\otimes \Sigma)$, we assumed we had appropriate conditional probability functions, $b(\cdot | D)$, on $\Omega$, defined for $b_\Decs$-almost every $D$. The important feature that our results rely on is that for bounded measurable random variables $X:\Omega\times\Decs\to\Re$, the law of total expectation holds: $\Exp_b[X]=\Exp_{b_\Decs}[\Exp_{b(\cdot|D)}[X]]$. We will in particular apply this to our $\U(\s)$ or $\U(\n)$, which are bounded, by assumption that $\U$ is bounded.\todo{We could probably instead remove it as a *background* assumption?? Just as we restrict to measurable $\s$ or $\n$, we could restrict to bounded or integrable strategies?? (though ``integrable'' is relative to a measure...) Anyway, I don't think we shoudl change this, but just to note it. }

When $\Decs$ is supported on a finite subset of $\Decs$, we can just define $b(\cdot | D)$ by the ratio formula for any $D$ with $b_\Decs(D)>0$ and verify that the law of total expectation holds. 

%One might instead simply stipulate that $b$ is constructed 

In the more general setting, we need to make use of further machinery to find this regular probability kernel. As $\Omega$ is finite, this can just be done from Radon-Nikodym derivatives. 


Let $\Omega_b:=\Set{\omega\given b(\{\omega\}\times\Decs)>0}$, and note that $b(\Omega_b)=1$. For any $\omega\in \Omega_b$, let $\beta_\omega$ be the measure on $\Decs$ given by $\beta_\omega(E)=b(\{\omega\}\times E)$ for $E\subseteq \Decs$ measurable. 
Then $b_\Decs=\sum_{\omega\in\Omega_b}\beta_\omega$, and so each $\beta_\omega$ is absolutely continuous wrt $b_\Decs$, and thus by the Radon-Nikodym theorem, there are $b_\Decs$-measurable functions $\frac{\diff \beta_\omega}{\diff b_\Decs}$. 
By the non-negativity and linearity of Radon-Nikodym derivatives, $\frac{\diff \beta_\omega}{\diff b_\Decs}\geq 0$ and $\sum_{\omega\in\Omega_b}\frac{\diff \beta_\omega}{\diff b_\Decs}=1$. 
Let $b(\cdot|D)$ be a measure on $\Omega$ given by $b(A|D)=\sum_{\omega\in A} \frac{\diff \beta_\omega}{\diff b_\Decs}$, which is thus a probability function on $\Omega$.  
...
Under our imposed $\sigma$-algeRbra, $(\Decs,\Sigma)$ is a standard Borel space, and so the Regular Conditional Probability theorem, or disintegration theorem \citep[Theorem 3.4]{kallenberg1997foundations} guarantees the existence of $b(\cdot|D)$ for $b_\Decs$ almost every $D$ such that for every bounded measurable random variable $X:\Omega\times \Decs\to\Re$, the law of total expectation holds: $\Exp_b[X]=\Exp_{b_\Decs}[\Exp_{b(\cdot|D)}[X]]$.\todo{check ref carefully or find beter one?}
...
\section{Alternative direct proof...}\todo{doesn't make use of standard Borel!}
...
This will then imply the results about E-Admissible strategies due to the following, which is immediate from the definition of E-Admissibility.
\begin{lemma}\label{thm:ead to eu appendix}
	$a\in\EAd_\IP(D)$ iff there is some $p\in\IP$ such that $a\in\EU_p(D)$.  
	Thus, any $\s$ which picks for $\EU_p$ for some $p\in\IP$ also picks for $\EAd_\IP$; and similarly for $\n$.

More generally, 
	$a\in\EAd_\IB(\cdot|-)(D)$ iff there is some $b\in\IB$ such that $a\in\EU_b(D)$.  
Thus, any $\s$ which picks for $\EU_{b(\cdot|-)}$ for some $b\in\IB$ also picks for $\EAd_\IB(\cdot|-)$; and similarly for $\n$. 

Also, 
	$\s\in\EAd_\IB(\S)$ iff there is some $b\in\IB$ such that $\s\in\EU_b(\S)$. 
	and 
		$\n\in\EAd_\IB(\PrPickStrategies)$ iff there is some $b\in\IB$ such that $\n\in\EU_b(\PrPickStrategies)$ 
\end{lemma}

%, as what is judged as E-Admissible is just those options which are maximisers of expected utility for one of the members of the credal set, and thus are just the expected utility picking strategies. 

\Cref{thm:ead-equiv[indep],thm:ead-existence[dep],thm:ead-existsimpermissibel[indep],thm:ead-nu-nec-suff,thm:ead-nu-reg-nec,thm:ead-suff-indep,thm:ead-equiv[dep]} thus follow from \cref{thm:ead to eu appendix,thm:EU appendix}.
...
\end{lemma}
\begin{proof}
	By \cref{thm:EU appendix}, $\n$ ${\mu^*}$-surely picks for $\EU_p$ iff it is a maximiser of  \cref{thm:EU appendix}. %\todooldinfo{where's the ref??} 
\end{proof}
...
% so if $a\neq a^*$, 
% $$\U(\n^{p_n})(\omega, D)=\Exp_{\n^{p_n}_D}a(\omega)\longrightarrow \Exp_{\n^{p^*}_D}a(\omega) = a^*(\omega)=\U(\n^{p^*})(\omega, D).$$ 
...
\begin{theorem}[CCM]\label{thm:ead-existsimpermissibel[indep]:CCM-1}
...
\begin{proof}
	...
	This gives us the assumptions of \cref{thm:ead-existsimpermissibel[indep]:CCM-1} clearly follow. 
\end{proof}


We can show this is satisfied given some other assumptions:

\begin{corollary}[CCM version of \cref{thm:ead-existsimpermissibel[indep]:RP-1}]\label{thm:ead-existsimpermissible[indep]:CCM-1:nondegentakeleave}
	Completely independent. 
	
	Assume there are distinct $l,u\in\Re$ such that for all $g\in[l,u]^\Omega$, there is some $a\in\A$ such that $\U(a)=g$. 
	
	Suppose $\IP$ contains distinct $p_1,p_2$ and for every $(p,\mu)\in\IB$, for every $U$ open non-empty $\subseteq\A$, $\mu$ assigns strictly positive measure to $\{\{a,0\}\given a\in U\}$.
	
	Then there is $\s$ which picks for $\EAd_\IP$ but there is no $p\times\mu\in \IB$ such that $\s$ $\mu$-surely pick for $\EU_p$.
\end{corollary}
\begin{proof}
	\todo[inline]{I've tried to be careful now about existence of A but it's made things confused...?}
	...
	By our assumptions on $\A$, then for any $g\in V_i$ there is some $a\in \A$ such that $\U(a)=g$. 
	
	Put $E_1:=\{\{a,0\}\given \U(a)\in V_1\}$ and $E_2:=\{\{a,0\}\given \U(a) \in V_2\}$. 
	
	
	Let $\mathrm{Disagree}(p,q):=\{a\in \A\given \Exp_p[\U(a)]>0\text{ and }\Exp_q[\U(a)]<0\}\cup\{a\in \A\given \Exp_p[\U(a)]<0\text{ and }\Exp_q[\U(a)]>0\}$. Note that it is open. 
	
	Also, if $a\in \mathrm{Disagree}(p,q)$ and $\lambda a\in \A$ then $\lambda a\in\mathrm{Disagree}(p,q)$.	
	
	Also observe that $\EU_{q}(\{a,0\})\cap \EU_{p}(\{a,0\})=\emptyset$ iff $a\in \mathrm{Disagree}(p,q)$.
	
	Now take any $p_1,p_2$ distinct in $\IP$. 
	
	Suppose $q\neq p_1$.
	Take $\omega^*$ such that $q(\omega^*)\neq p_1(\omega^*)$. Let $r=\frac{q(\omega^*)+p_i(\omega^*)}{2}>0$ and consider the Dutch-book-style bet $g^*(\omega)=\begin{cases}
		1-r&\omega=\omega^*\\
		-r&\omega\neq \omega^*
	\end{cases}$
	
	We can find some $\lambda>0$ so that $\|\lambda g^*\|<0.5$, thus that $\lambda g^*\in V_1$. Then there is some $a^*\in\A$ with $\U(a^*)=\lambda g^*$, and $a^*\in \mathrm{Disagree}(p_1,q)$, and thus $V_1\cap \mathrm{Disagree}(p_1,q)\neq\emptyset$. It is thus a non-empty open subset of $\A$ and so \begin{equation}
		\mu\{\{a,0\}\given a\in V_1\cap \mathrm{Disagree}(p_1,q)\}>0
	\end{equation}
	If $a\in V_1\cap \mathrm{Disagree}(p_1,q)$, then $\{a,0\}\in\{D\in E_1\given \EU_{p_1}(D)\cap \EU_q(D)=\emptyset\}$. Thus, $\mu \{D\in E_1\given \EU_{p_1}(D)\cap \EU_p(D)= \emptyset\}>0$, as required. 
	
	For $q=p_1$, it is distinct from $p_2$, so using an analogous argument 
	we can find $a^*\in V_1\cap \mathrm{Disagree}(p_2,q)$. 
	So it is a non-empty open subset of $\A$. 
	Then observe that $\mu \{D\in E_2\given \EU_{p_2}(D)\cap \EU_p(D)= \emptyset\}>0$, as required. 
	
	We can then apply \cref{thm:ead-existsimpermissibel[indep]:CCM-1}.
\end{proof}
...
\begin{corollary}Completely independent. 
		...
		Assume that for all  $g\in[-1,1]^\Omega$, there is some $a\in\A$ such that $\U(a)=g$. 
...
\end{corollary}
\begin{proof}
	Let $V_1,V_2$ be disjoint open subsets of $B_1(0)\subseteq [-1,1]^n$. Let $E_i:=\{D\in\D\given D\subseteq V_i\}$. This is open. 
	
	
	
	\begin{sublemma}
	Let $E$ be open non-empty and $E\subseteq B_1(\{0\})$. 
...
\textbf{It is nonempty:}
	...
	Take any $D_0\in E$. 
	If $\EU_{q_1}(D_0)\cap \EU_{q_2}(D_0)=\emptyset$ this suffices. 
	
	Otherwise there is some $a_0\in \EU_{q_1}(D_0)\cap \EU_{q_2}(D_0)$. 
	
	As $E$ is open, there is some $\epsilon>0$ such that 
	
	As in \cref{thm:ead-existsimpermissible[indep]:CCM-1:nondegentakeleave}, there is some $a^*\in\mathrm{Disagree}(q_1,q_n)$. Moreover, we can choose $a^*$ so that $\|a^*\|<1-\|a_0\|$ and $\|a^*\|<\epsilon$, so then $\|a_0+a^*\|<1$, so by our assumption on $\A$, $a_0+a^*\in\A$. 
	
	WLOG assume that $\Exp_{q_1}[\U(a^*)]>0$ and $\Exp_{q_1}[\U(a^*)]<0$. Note then that $\Exp_{q_1}[\U(a_0+a^*)]>\Exp_{q_1}[\U(a_0)]$ and $\Exp_{q_1}[\U(a_0+a^*)]<\Exp_{q_1}[\U(a_0)]$.
	
	Consider $D^*=D_0\cup\{a_0+a^*\}$ and observe that $d(D^*,D_0)<d(a_0+a^*,a_0)<\epsilon$. So $D^*\in E$. 
	
	Now, $\EU_{q_1}(D^*)=\{a_0+a^*\}$ and $\EU_{q_2}(D^*)=\{a_0\}$, so we have found our $D^*\in E$ with $\EU_{q_1}(D^*)\cap \EU_{q_2}(D^*)=\emptyset$. 
...
%		
%		
%		 and $a_0\in D_0$. As $E$ is open under the Hausdorff metric, there is some $\epsilon>0$ such that $d(D_0,D)<\epsilon\implies D\in E$, where $$d(D,D'):=\max\{\sup_{a\in D}\inf_{a'\in D'} d(a,a'), \sup_{a'\in D'}\inf_{a\in D} d(a,a')\}$$ with the Euclidean distance on $\A\cong\Re^n$. 
%		
%		Consider $a^*\in\A$ such that  $\Exp_{q_1}[\U(a^*)]>0>\Exp_{q_2}[\U(a^*)]$ and $\|a^*\|<\epsilon$. \todo{this is assumptions about $\A$! Needs more than the assumptions so far...! E.g., so far, we could just have $\A$ is utilities in $\{1,2,3\}$. }
%		Consider $D^*=\{a_0+a^*,a_0-a^*\}$. Observe that $d(D^*,\{a_0\})<\epsilon$. Also $d(\{a_0\},D_0)=0$ as $a_0\in D_0$. So $d(D^*,D_0)\leq d(D^*,\{a_0\})+d(D_0,\{a_0\})<\epsilon$. But, since $\Exp_{q_1}[\U(a^*)]>0>\Exp_{q_2}[\U(a^*)]$, $\EU_{q_1}(D^*)=\{a_0+a^*\}$ and $\EU_{q_2}(D^*)=\{a_0-a^*\}$; thus $D^*\in \{D\given \EU_{q_1}(D)\cap \EU_{q_2}(D)=\emptyset\}$. This has shown that $\{D\in E\given \EU_{q_1}(D)\cap \EU_{q_2}(D)=\emptyset\}$ is non-empty.
		 \end{proof}
	Take any $p_1,p_2$ disjoint in $\IP$ and $E_1,E_2$ disjoint open subsets of $\Decs$ and will then satisfy the conditions of \cref{thm:ead-existsimpermissibel[indep]:CCM-1}.	
\end{proof}
...
\begin{proposition}[JK]\label{thm:ead-existsimpermissibel[indep]:JK-1}
...
\end{itemize}
\todo[inline]{CCM: this example can't work... every strategy is rationalised by $t=2/3$. }
\section{RP's version}

\begin{proposition}[RP]\label{thm:ead-existsimpermissibel[indep]:RP-1}
...
\begin{proposition}[{JK version of RP \cref{thm:ead-existsimpermissibel[indep]:RP-1}}]\label{thm:ead-existsimpermissibel[indep]:JK-2}
...
\begin{corollary}\label{thm:ead-existsimpermissibel[indep]:JK-2-corollary}
