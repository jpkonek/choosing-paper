...
\emph{Uncertainty } The agent's uncertain beliefs about the world are represented by a single probability function $p$ over $\Omega$. The set of all such probabilities is $\Prob$.
...
\begin{proposition}\label{thm:max-nu}Suppose $\Nu$ is $\EU$-complete. Suppose that 
	...
	Then if $\n \in \Maximality_\IB(\Nu)$, then there is some probability $p\in\Prob$ such that $\n$ $\mu^*$-surely picks for $\EU_p$.%\todo{did we know that $p$ in $\IP$??}
...
The set $\S$ of all deterministic picking strategies discussed in the earlier part of the paper is EU-complete, and so we obtain the result that we hinted at in \cref{sect:Max}, namely, that if we have $\IB$ that satisfies the conditions of \Cref{thm:max-nu}, then it is only $\EU_p$ strategies that are in $\Maximality_\IB(\S)$. \begin{colored}{blue} Now consider the following corollary of \Cref{thm:ead-existsimpermissible[indep]}:



\begin{corollary}\label{thm:ead-existsimpermissible[indep]-corr}

 Suppose that $\IB$ has the form $\{p\times \mu^*\given p\in \IP\}$.

Suppose there is a selection of pairwise disjoint events $E_q\subseteq\Decs$, one for each $q\in\IP$ (some of which may be empty), such that for all $p\in\Prob$, 
\[
\mu^*\left(\bigcup_{q\in\IP}\{D\in E_q\given \EU_p(D)\cap\EU_q(D)=\emptyset\}\right)>0
\]

Then there is $\s$ which picks for $\EAd_\IP$ but which does not $\mu^*$-surely pick for $\EU_p$ for any $p\in\Prob$.

\end{corollary}

If we have $\IB$ that satisfies the conditions of \Cref{thm:max-nu}, we can appeal to this corollary and \Cref{thm:max-nu}, together with the fact that E-Admissibility is at least as permissive as Maximality, to give sufficient conditions under which there is a deterministic strategy $\s$ that picks for $\Maximality_{\IP}$ but that does not belong to $\Maximality_\IB(\S)$.
...
We can also apply the same trick to \cref{thm:seed-suff-impermissible-strat}. Consider the following corollary:


\begin{corollary}\label{thm:seed-suff-impermissible-strat-corr}
	Suppose that for any $p\in\Prob$, $\mu^*(U_p)>0$ where 
\[
U_p:=\bigl\{D\in\Decs\given \c(D)\not \subseteq\EU_p(D)\bigr\}.
\] 
Suppose further that there is some measurable statistic $r:\Decs\rightarrow[0,1)$ such that for any $p\in\Prob$ and any open interval $I\subset[0,1)$, 
\[
\mu^*\bigl(\,D\in U_p\given r(\c(D))\in I \bigr)>0.
\]
Then there is $\s$ which picks for $\c$ but which does not $\mu^*$-surely pick for $\EU_p$ for any $p\in\Prob$.
\end{corollary}

This corollary and \Cref{thm:max-nu} give us:
...
There are various other natural richness conditions we might consider, \textit{e.g.},


\begin{proposition}\label{thm:atomless-existsimpermissible}
	For any choice function $\c$, if $\mu^*$ is atomless and for all $p\in\Prob$, $\mu^*\{D\given \c(D)\subseteq\EU_p(D)\}<1$, then there is $\s$ which picks for $\c$ but for no $p\in\Prob$ does it $\mu^*$-surely pick for $\EU_p$. 
\end{proposition}
...
\begin{corollary}\label{thm:atomless-max-existsimpermissible}
	...
	If $\mu^*$ is atomless and for all $p\in\Prob$, $\mu^*\{D\given \Maximality_\IP(D)\subseteq\EU_p(D)\}<1$, then there is $\s$ which picks for $\Maximality_\IP$ but which is not in $\Maximality_\IB(\S)$. 
	\end{corollary}
...
Since $\Gamma$-Maximin is a more restrictive theory than Maximality, for a strategy to be $\Gamma$-Maximin there must be some probability function $p\in\Prob$ % \todo{why in IP???}
...
Suppose $\Nu$ is $\EU$-complete. Suppose that 
		$\IB$ has the form $\{p\times \mu^*\given p\in \IP\}$ and $\mu^*$ requires almost everywhere decisiveness. Then if $\n \in \Maximin_\IB(\Nu)$, there is some probability $p\in\Prob$ where $\n$ $\mu^*$-surely picks for $\EU_p$.
		
		Suppose further that for all $p\in\Prob$, $\mu^*\bigl\{D\in\Decs\given \Gamma_\IP(D)\not \subseteq\EU_{p}(D)\bigr\}>0$. Then if $\n$ is regular for $\Maximin_\IP$ then $\n\notin \Maximin_\IB(\Nu)$.
...
We can also use \cref{thm:seed-suff-impermissible-strat-corr} and \cref{thm:atomless-existsimpermissible} to show that there are deterministic strategies $\s$ that pick for $\Gamma_{\IP}$ but do not belong to $\Gamma_\IB(\S)$.
...
\begin{corollary}\label{thm:atomless-gamma-existsimpermissible}
	...
	If $\mu^*$ is atomless and for all $p\in\Prob$, $\mu^*\{D\given \Gamma_\IP(D)\subseteq\EU_p(D)\}<1$, then there is $\s$ which picks for $\Gamma_\IP$ but which is not in $\Gamma_\IB(\S)$. 
	\end{corollary}
...
%\end{tabularx}
	...
	The $\sigma$-algebra we generate will be defined relative to a utility function $\U:\A\to\Re^\Omega\cong\Re^n$. Recall that $\U$ is bounded, so in fact $\U:\A\to[l,h]^\Omega\cong[l,h]^n$.	
...
If $r(\c(D))\in I_{\beta(D)}(D)$ then $s(D)=x_{\beta(D)}(D)\notin\EU_{b(\cdot|-)}(D)$.
...
\cref{thm:max-nu,thm:max-existsimpermissible[dep],thm:max-nu-reg-nec,thm:gamma-nu,thm:atomless-existsimpermissible} require a different approach.
%\todo{add ref to the new undermining thm we're adding. }


%\end{colored}
\section{Maximality \\(\cref{thm:max-nu,thm:max-existsimpermissible[dep],thm:max-nu-reg-nec,thm:gamma-nu,thm:atomless-existsimpermissible})}\label{sect:wald:appendix}
To prove \cref{thm:max-nu,thm:max-existsimpermissible[dep],thm:max-nu-reg-nec,thm:gamma-nu,thm:atomless-existsimpermissible} we turn to a version of Abraham Wald's (\citeyear{wald1947cct}) Complete Class Theorem, which we prove as \Cref{thm:cct}. % \todo{what are the two versions referred to?}
...
\cref{thm:max-nu} follows from this. \cref{thm:max-existsimpermissible[dep]} follows from this together with \cref{thm:ead-existsimpermissible[indep]-corr}, since Maximality is more permissive than E-Admissibility.



We now prove \cref{thm:atomless-existsimpermissible}. Let $\c$ be any choice function. 
...
\begin{proof}
	...
	Let $G(q,D,\epsilon):=\{b\in D\given t_q(D)>\Exp_q[\U(b)]+\epsilon\}$. 
	
	  \begin{sublemma}
		There is some $\epsilon_q>0$ s.t. $\mu^*\{D\given G(q,D,\epsilon_q)\cap\c(D)\neq \emptyset\}>0$. 
	\end{sublemma}
	\begin{proof}
		We have assumed that $\mu^*\{D\given \c(D)\not \subseteq\EU_q(D)\}>0$, for all $q\in\Prob$.
		 
		Observe
		\begin{align}
			\{D\given \c(D)\not\subseteq\EU_q(D)\}&=\{D\given\exists b\in\c(D)\text{ and }t_q(D)>\Exp_q[\U(b)] \}\\
			&=\bigcup_{k\in\mathbb{N}}\{D\given\exists b\in \c(D)\text{ and }t_q(D)>\Exp_q[\U(b)]+\nicefrac{1}{k} \}\\
	...
	We have assumed that utility is bounded. Put $M>0$ such that $\|\U(a)\|_\infty=\sup\{|\U(a)(\omega)|\given \omega\in\Omega\}\leq M$ for all $a\in\A$, that is $M\geq\max\{|l|,|h|\}$ if $\U:\A\to[l,h]^\Omega$. 	Let $$U_q:=\{p\in\Prob\given \|q-p\|_1<\frac{\epsilon_q}{4M}\}.$$
	...
	Then put $\s(D)\in G(q_i,D,\epsilon_{q_i})\cap\c(D)$ for $D\in E_i\subseteq C_i$, and $\s(D)\in \c(D)$ for $D\notin\bigcup_{i=1,\ldots,k} E_i$. 
	...
	\end{sublemma}
	\begin{proof} As $b\in G(q,D,\epsilon_q)$,  $t_q(D)>\Exp_q[\U(b)]+\epsilon_q$, where $t_q(D)=\sup_{a \in D}\Exp_q[\U(a)]$. Take $a^*\in \EU_q(D)$ so that $\Exp_q[\U(a^*)]=t_q(D)$ which implies $\Exp_q[\U(a^*)]-\Exp_q[\U(b)]\geq \epsilon_q$. 
		...
		\begin{align}
			...
			&\geq \epsilon_q -\|p-q\|_1\|\U(a^*)-\U(b)\|_\infty \ \text{(H\"older's inequality)}\\
...
Suppose $\Nu$ is $\EU$-complete. Suppose that 
		...
		Suppose further that for every probability $p\in\Prob$, $\mu^*\{D\given \Maximin_\IP(D)\subseteq\EU_p(D)\}<1$. 
		Then if $\n$ is regular for $\Maximin_\IP$ then $\n\notin \Maximin_\IB(\Nu)$.
...
\begin{proof}
	Suppose that $\n \in \Maximin_\IB(\Nu)\subseteq\Maximality_\IB(\Nu)$. By  \cref{thm:max-nu}, there is some probability $p\in\Prob$ where $\n$ $\mu^*$-surely picks for $\EU_p$.
	...
	$\mu^*\{D\given \Maximin_\IP(D)\not\subseteq\EU_p(D)\}>0$, we have $\mu^*\{D\given \nu(\EU_p(D))<1\}>0$.
	 So $\nu$ does not $\mu^*$-surely pick for $\EU_p$. Since this holds for any $p \in \Prob$, we must have $\n \not\in \Maximin_\IB(\Nu)$.
