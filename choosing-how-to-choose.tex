...
\end{colored}
		Then if $\n$ is regular for $\Maximin_\IP$ then $\n\notin \Maximin_\IB(\Nu)$. \todo{JK looking at this}%double check thms... I removed ``in IP''!}\todo{This thm feels false}
	\end{proposition}
...
\subsection{Underminingness of E-Admissibility \\(\cref{thm:ead-existsimpermissible[indep],thm:ead-existsimpermissible[dep],thm:ead-nu-reg-nec,thm:max-existsimpermissible[dep]})}\label{sect:appendix:eadundermining}

We now move to \cref{thm:ead-existsimpermissible[indep],thm:ead-existence[dep],thm:ead-nu-reg-nec}. For these results, we need to give conditions under which there exists a strategy that picks for E-Admissibility and does not look like an expected utility strategy from the point of view of our measure $\mu$ over the decision problems. 
...
\begin{proof}
	...
	\begin{proof}
		...
		As in \cref{thm:ead-existsimpermissible[indep]:CCM-1:nondegentakeleave}, we can find some $X^*$ such that $\Exp_{p}[X^*]>0>\Exp_{q}[X^*]$. And, moreover, we can choose a scalar $\lambda$ small enough so that $\U(a_0)+\lambda X^*\in[l,h]^n$ and thus that $a_0+\lambda X^*\in\U(\A)$.% (abusing notation). 
...
To prove \cref{thm:max-nu,thm:max-nu-reg-nec}, we turn to two versions of Abraham Wald's (\citeyear{wald1947cct}) Complete Class Theorem, which we combine in \Cref{thm:wald-cct}. % \todo{what are the two versions referred to?}
We state them in a general setting and then explain how they apply to our case.
...
\begin{itemize}
	...
	\item $\Uwald:\O\times\Omega\to [l,u]$ is a bounded ``utility function''. In our application, it will be $\Uwald(\n,\omega)=\Exp_{\mu^*}\U(\n,\omega)=\Exp_{\mu^*}\Exp_{\n_D}\U(a)(\omega)$.\footnote{We have assumed it is bounded; however all that is actually needed for the proof is that it is bounded above, i.e., we could allow $\Uwald:\O\times\Omega\to (-\infty,0]$.}
...
%For example, if $\Uwald$ is a measure of accuracy which is continuous and strictly proper, then these will be satisfied. They are, however, \emph{much} weaker than that.
	...
	\begin{lemma}\label{thm:cct:bdry are the Bayes optimal}
		Suppose $\Uwald$ is Bayes-continuous on $\O$ and $\O$ is Bayes-existing relative to $\Uwald$. Then, if $Z\in\cl(\Conv(\Uwald(\O)))$ is not weakly dominated in $\Conv(\Uwald(\O))$, then $Z\in \Uwald(\O)$ and $Z$ is Bayes  in $\Uwald(\O)$. 
%		
%		The proof also shows t
	...
	\begin{proof}
		...
		Since $Z$ is not weakly dominated, it is also not strictly dominated. So, by \cref{thm:cct:admiss in conv are Bayes in conv}, there is some probability function $p^*$ on $\Omega$ with $p^*(Z)\geq p^*(\Uwald(o))$, for all $o$. Since $\O$ is Bayes-existing relative to $\Uwald$, there is some $o_{p^*}$ that is Bayes for $p^*$ in $\Uwald(\O)$. 
...
%	This applies to any $X$, whether or not it is in $\Uwald(\O)$ itself, but of course we actually want the result for $o$. 
	\begin{theorem}\label{thm:cct}
		Suppose $\O$ is Bayes-existing relative to $\Uwald$ and $\Uwald$ is Bayes-continuous on $\O$. Then, if $o$ is not Bayes then there is $o'$ that strictly dominates it; moreover, it is strictly dominated by an option that is itself Bayes and not even weakly dominated. 
	\end{theorem}
\begin{proof}
...
\todo{remove this from a thm env}
\begin{theorem}[Main theorem: a version of Wald's Complete Class Theorem]\label{thm:wald-cct}
	Suppose either: {\normalfont(i)} $\Uwald(\O)$ is closed and convex, or {\normalfont(ii)} that $\Uwald$ is Bayes-continuous over $\O$ and $\O$ is Bayes-existing relative to $\Uwald$. Then $o$ is Bayes iff $o$ is not strictly dominated.
\end{theorem}
...
\begin{lemma}\label{thm:cct-appln:existing}
	If $\Nu$ is EU-complete, then $\Nu$ is Bayes-existing relative to $\Uwald$. 
\end{lemma}
\begin{proof}
	If $\Nu$ is EU-complete, then for each $p$, there is $\n$ in $\Nu$ that picks for $\EU_p$. And, by \cref{thm:EU appendix}, since $\n$ picks for $\EU_p$, $\n$ is Bayes for $p$. %\todooldinfo{where's the ref??} 
\end{proof}
...
\begin{corollary}\label{thm:cct-applns:corr}
	Suppose that $\Nu$ is EU-complete, and suppose $\mu^*$ requires almost everywhere decisiveness and is countably additive. 
...
\end{proof}
\begin{corollary}
		Suppose that $\Nu$ is EU-complete, $\mu^*$ requires almost everywhere decisiveness and is countably additive. %\todo{why are we explicitly stating ctbly add here. Surely for the existence of the conditionals we need the ctbly add. i was assuming that we had assumed somewhere in general that it was ctbly add.....!!!!!}
...
\]
Because $p\mapsto \Exp_{p}(x)$ is continuous and $K$ is compact, $\underline{\Exp}_K(x)$ is finite and continuous. %; indeed it is $1$-Lipschitz with respect to $\|\cdot\|_\infty$, since $|p\cdot v - p\cdot w| \le \|v-w\|_\infty$ for all $p\in P$. 
...
%For any $p,q\in\IP$, let $S_p=\{D\given \EU_p(D)\textrm{ is a singleton}\}$ and $A_{p,q}=\{D\in S_p\cap S_q\given \EU_p(D)=\EU_q(D)\}$. Let $x_p=\{q\given \mu^*(A_{p,q})=1\}$. Since $p\sim q$ iff $\mu^*(A_{p,q})=1$ is an equivalence relation, $\X=\{x_p\given p\in\IP\}$ partitions $\IP$. Let $B_{x_p}=\cap_{q\in x_p} A_{p,q}$.
