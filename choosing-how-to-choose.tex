...
\begin{colored}{violet}
	...
	$\Decs$ was just defined as finite subsets of an arbitrary non-empty set $\A$ and we gave $\mu$ a probability measure on $\Decs$ (or $\pb$ a probability measure on $\Omega\times\Decs$). We should, however, specify the measure space. 
	
	Fixing $\U:\A\times\Omega\to\Re$ we can treat $\A$ as a subset of $\Re^{|\Omega|}$. We will consider the measure space given by the Lebesgue measurable sets. 
	Since each $D\in\Decs$ is a finite subset of $\A$, the measure space on $\Decs$ can be defined from it. 
...
When $b=p\times\mu$, we have $b_\Decs$ is $\mu$ and $b(\cdot|D)=p$ for all $D$.
...
We have $\Exp_{b}[\U(\prpickstrat)]=\Exp_{b_\Decs}[\Exp_{b(\cdot | D)}[\U(\prpickstrat)]]$, as $\U(\prpickstrat)$ is a measurable random variable on $\Omega\times\Decs$.
...
Thus, $\Exp_{b}[\U(\prpickstrat)]=\Exp_{b_\Decs}[\Exp_{b(\cdot | D)}[\U(\prpickstrat)]]$ is maximised just when for $\pb_\Decs$ almost every $D$,  $\prpickstrat_{D^*}(\EU_{b(\cdot|-)}(D))=1$, which is to say that $\prpickstrat$ $\pb$-surely picks for $\EU_{\pb(\cdot|-)}$. 
%
%
