...
Take $\nu^*\in\Nu$ Bayes for $p^*$ and $\nu^1,\nu^2,\ldots$ in $\Nu$ with $\nu^k$ Bayes for $p_k$. This is possible by \cref{thm:cct-appln:existing}, and also by this, $\nu^*$ $\mu^*$-surely picks for $\EU_{p^*}$ and each $\nu^k$ $\mu^*$-surely picks for $\EU_{p_k}$. 
That is, $\mu^*(\Set{D \given \n^*_D(\EU_{p^*}(D)) = 1})=1$, and $\mu^*(\Set{D  \given \n^k_D(\EU_{p_k}(D)) = 1})=1$ for each $k$.

Let $$E:=\Set{D\given \n^{p^*}_D(\EU_{p^*}(D)) = 1\text{ and for all $k$,  $\n^{p_k}_D(\EU_{p_k}(D)) = 1$}}.$$
