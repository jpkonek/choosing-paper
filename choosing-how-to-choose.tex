...
\begin{theorem}[CCM]\label{thm:ead-existsimpermissibel[indep]:CCM-1}
	...
	Suppose $\IP$ contains $p_1$ and $p_2$ such that for all $p\times\mu\in\IB$, for at least one of $i\in\{1,2\}$, 
	$\mu\{D\given \EU_p(D)\cap\EU_{p_i}(D)=\emptyset\}=1$. 
...
\begin{proof}
	...
	Put $\s(D)\in\EU_{p_1}(D)$ for $D\in E$ and  $\s(D)\in\EU_{p_2}(D)$ for $D\notin E$. So it picks for $\EAd_\IP$. 
	
	Denote $E_1=E$ and $E_2=\Decs\setminus E$, just so we can make the argument once:
	
	Take any $p\times\mu\in\IB$. For at least one of $i\in\{1,2\}$, 
	$\mu\{D\given \EU_p(D)\cap\EU_{p_i}(D)=\emptyset\}=1$. 
...
\todo{RP attempt at proof of something like Propn 7}
