...
%\end{proposition}
	...
	\begin{proposition}\label{thm:ead-nu-reg-nec}
		Suppose that $\PrPickStrategies$ is $\EU$-complete. \todo{I don't think we need EU-complete!}
...
This will then imply many of the results about E-Admissible strategies due to the following, which is immediate from the definition of E-Admissibility.
\begin{proposition}\label{thm:ead to eu appendix}
	$a\in\EAd_\IP(D)$ iff there is some $p\in\IP$ such that $a\in\EU_p(D)$.  
...
\Cref{thm:ead-equiv[indep],thm:ead-nu-nec-suff,thm:ead-suff-indep} thus follow from \cref{thm:ead to eu appendix,thm:EU appendix}.


For \cref{thm:ead-existsimpermissibel[indep],thm:ead-existence[dep],thm:ead-nu-reg-nec}, we need to moreover give conditions under which no strategy looks like an expected utility strategy. For regular picking strategies this is simple: it has to not look like an $\EU_p$ strategy. 

	\begin{proposition}
	Suppose that for every $b\in\IB$, $b_\D\{D\given \EAd_{\IB(\cdot|-)}(D)\subseteq\EU_{b(\cdot|-)}\}<1$. 
	That is, for all $b\in \IB$, $b_\D\{D\given \text{there is $b'\in\IB$ with }\EU_{b'(\cdot|-)}(D)\not\subseteq\EU_{b(\cdot|-)}(D)\}>0$. \todo{is this second codnition really equivalent??}
	
	Then, if $\n$ is a regular picking strategy for $\EAd_{\IB(\cdot|-)}$, then $\n\notin \EAd_\IB(\PrPickStrategies)$.
\end{proposition}

\begin{proof}
	Suppose $\nu$ is a regular picking strategy for $\EAd_\IP$. Take $b\in\IB$
	
	If $\EAd_{\IB(\cdot|-)}(D)\subseteq\EU_{b(\cdot|-)}$, then $\nu(\EU_{b(\cdot|-)}(D))\neq 1$, by regularity. As 
	$b_\D\{D\given \EAd_{\IB(\cdot|-)}(D)\not\subseteq\EU_{b(\cdot|-)}\}>0$, then $\mu\{D\given \nu(\EU_{b(\cdot|-)}(D))<1\}>0$. So $\nu$ does not $\mu$-surely pick for $\EU_p$.  
\end{proof}

For showing the existence of deterministic picking strategies which are not $\EU_p$ strategies, this assumption will not suffice. 
\begin{example}
	If $\IP=\{p_1,p_2\}$ and $p_1$ expects $a_1$ better than $a_2$ and $p_2$ expects $a_2$ better than $a_1$. Suppose $\mu\{a_1,a_2\}=1$. Then picking strategies are just determined by their selection on this single decision, picking either $a_1$ or $a_2$. So any strategy is $\mu$-surely an $\EU_p$ strategy, even though $\mu\{D\given \EAd_\IP(D)\subseteq\EU_p(D)\}\neq 1$. 
\end{example}
...
In these cases, we have distinct decisions each of which have positive measure but where every probability 

\begin{proposition}
	If $q_1,q_2\in\IB$ and $E_1,E_2\subseteq\Decs$ such that for all $(p,\mu)\in\IB$ there is at least one of $q_1$ or $q_2$ such that
	\[
	\mu\{D\in E_i\given \EU_p(D)\cap\EU_{q_i}(D)=\emptyset\}>0
	\]
...
\begin{proposition}
	If $b_1,b_2\in\IB$ and $E_1,E_2\subseteq\Decs$ such that for all $b\in\IB$ there is at least one of $b_1$ or $b_2$ such that
	 \[
		\mu\{D\in E_i\given \EU_{b(\cdot|D)}(D)\cap \EU_{b_i(\cdot|D)}=\emptyset\}>0
	\]
\end{proposition}
...
\begin{proof}
	...
	Now take any $p_1,p_2$ distinct in $\IP$. We will show that for any $q\in\IP$, for at least one of $p_1$ or $p_2$, $\mu \{D\in E_i\given \EU_{p_i}(D)\cap \EU_p(D)= \emptyset\}>0$, as required for \cref{thm:ead-existsimpermissibel[indep]:CCM-1}. 
