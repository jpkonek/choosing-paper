...
Let's suppose the agent thinks the odds of this company going bust are $1:10$. Perhaps her decision theory prefers Risky to Safe. 
...
Any strict failure of von Neumann-Morgenstern's Independence Principle will also generate a case of undermining. Suppose your decision theory deems option $a$ preferable to option $b$; and suppose further that it prefers the mixed option $b$-if-a-possibly-biased-coin-lands-heads-and-$c$-if-tails to the mixed option $a$-if-heads-and-$c$-if-tails. Then we can generate a way of being uncertain over which decision problem you'll be faced with so that the unique picking strategy which is compatible with the decision theory is itself impermissible according to the theory. 




These cases are formally related to cases of sequential incoherence for such decision theories \citep{hammond1988cfeu,machina1989}. 
\begin{colored}{violet}
The formal motivation for our investigation was instead \citet{levinstein2017pgeu} who, following \citet{schervish2009psr}, also considers evaluations based on uncertainty over which decision problem is being faced, although his analysis is to evaluate credences according to their expected guidance value in each state of the world. 
\end{colored}
...
If it is self-undermining, then one of its verdicts is that picking in accordance with itself is impermissible. Could such a decision theory be correct? 
That is, could its evaluations match with what is actually impermissible for the agent. 
Since it judges some alternative picking strategy to be better than its own, it is thus impermissible to pick in accordance with it.

The decision theory evaluates some alternative picking strategy to be preferable to itself, but to conclude that it is impermissible to use its own picking strategy, the alternative one needs to be available to the agent. 
...
Similarly, if in attempting to use a given decision theory, it may misfire and result in even worse decisions, it may be permissible, or even required, to adopt the simpler decision theory. 
However, we are assuming that we are not in this sort of case, and that it is within the agent's control to choose any picking strategy and that there is no misfiring. 
...
If we again suppose that the self-undermining decision theory is correct, it leads to a rational dilemma, where one is rationally required to adopt a picking strategy which picks in an impermissible way. 
...
There is another way the defender of these theories could argue against our claim of self-underminingness: they can note that we have essentially assumed Bayesian conditionalization and argue against conditionalization. As pointed out in \citet[16]{campbellmoore2020arae}, several standard arguments for conditionalization assume that rational agents maximise expected utility.

The picking strategy which the decision theory deems optimal is compatible with that very decision theory when we allow her credences to change when she learns which decision problem she is faced with. Instead of holding fixed her credences (by assuming probabilistic independence and conditionalization), the defender of the decision theory might instead argue for an alternative credal update strategy: the one which results in the optimal picking strategy when coupled with risk-weighted expected utility theory \citep[see also][]{campbellmoore2022aurs,brown1976ceu}.
...
\begin{colored}{red}
%\todoinfo{ORIGINAL}
There is another way the defender of these theories could argue against our claim of self-underminingness: they can argue that we have essentially assumed Bayesian conditionalization and argue against conditionalization. 
