...
It can also be extended to allow for proper distributions by allowing each $\mu$ having some $J\subseteq I$ st:  $\mu(\bigcup_{j\in J}\{D\in E_j\given \EU_{p_j}(D)\cap \EU_p(D)= \emptyset\})>0$.
...
\end{proof}
Weakening the zero separations but instead requiring it to assign positive probability to every non-degenerate subset of $\Decs$.
\begin{lemma}Suppose $E$ non-empty open $\subseteq\Decs$. For $q_1$ and ${q_2}$ disjoint, 
	$\{D\in E\given \EU_{q_1}(D)\cap \EU_{q_2}(D)=\emptyset\}$ is open and non-empty $\subseteq\Decs$.
\end{lemma}
\begin{proof}

It is open: $E$ is open. We just need to show that $\{D\given \EU_{q_1}(D)\cap \EU_{q_2}(D)=\emptyset\}$ is open. 
Since $q_1\neq {q_2}$ there are $a_1,a_2\in\A$ such that $\Exp_{q_1}[\U(a_1)]>\Exp_{q_1}[\U(a_2)]$ and $\Exp_{q_2}[\U(a_1)]<\Exp_{q_2}[\U(a_2)]$. So we can find open $U_1\ni a_1$ and $U_2\ni a_2$ with the same inequality of expectation. So $\{D\given D\cap U_1=\emptyset\text{ and }D\cap U_2=\emptyset\}$ is open and $\{D\given \EU_{q_1}(D)\cap \EU_{q_2}(D)=\emptyset\}$.

It is nonempty:

Take any $D_0\in E$ and $a_0\in D_0$. As $E$ is open under the Hausdorff metric, there is some $\epsilon>0$ such that $d(D_0,D)<\epsilon\implies D\in E$, where $$d(D,D'):=\max\{\sup_{a\in D}\inf_{a'\in D'} d(a,a'), \sup_{a'\in D'}\inf_{a\in D} d(a,a')\}$$ with the Euclidean distance on $\A\cong\Re^n$. 
...
\todo{CCM version of \cref{thm:ead-existsimpermissibel[indep]:JK-1}}
\begin{proposition}\label{thm:ead-existsimpermissibel[indep]:CCM-2}
	content...
\end{proposition}
...
\begin{proposition}[RP]\label{thm:ead-existsimpermissibel[indep]:RP-1}
	...
	If $\mu$ is non-degenerate and $\IP$ is not precise, there is some $\s$ that picks for $\EAd_\IP$ but which is not in $\EAd_\IB(\Strategies)$.
\end{proposition}
\begin{proof}
...
\begin{proposition}
	Suppose that for every $(p,\mu)\in\IB$ and every measurable 
	
	Suppose $\IP$ is imprecise. 
\end{proposition}
...
\todo[inline]{CCM: this can't be true. measure 1 on decs where they all agree... }
