...
\begin{proof}
	...
	Let $G(q,D,\epsilon):=\{b\in\c(D)\given t_q(D)>\Exp_q[b]+\epsilon\}$. 
	  ...
	  \begin{sublemma}
		There is some $\epsilon>0$ such that $\mu^*\{D\given G(q,D,\epsilon)\neq \emptyset\}$. 
	\end{sublemma}
	\begin{proof}
	...
	Now, for any $p\in\Prob$, there is some $i$ such that $p\in U_{q_i}$ and so $\s(D)\notin\EU_p(D)$ by this sublemma:
		\begin{sublemma}
	...
	\begin{proof}
		...
		\begin{align}
			...
			&\geq \epsilon -\|q-p\|\|\U(a^*)-\U(b)\|_\infty\\
			&\geq  \epsilon -\nicefrac{\epsilon}{2M}\times M=\nicefrac{\epsilon}{2}>0\qedhere
		\end{align}
	\end{proof}
\end{proof}
