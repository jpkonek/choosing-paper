...
Then, there is some $\s$ that picks for $\EAd_\IP$ but which is not in $\EAd_\IB(\Strategies)$ iff there is some  $E\subseteq\Decs$ such that (1) $(\forall p\times\mu_p\in\IB)\mu_p(E)>0$, and (2) $(\forall X\in E)(\exists f_X\in\EAd_\IP(X))(\exists g_X\not\in\EAd_\IP(X))(\forall E^\prime\subseteq E)$
\[
\posi(\left\{f_X-g_X\given X\in E^\prime\right\})\cap \mathbb{R}^n_{<0}=\varnothing\;\Rightarrow\;(\forall p\times\mu_p\in\IB)\mu_p(E^\prime)=0
\]
...
\begin{proof}
	...
	Let $\s(x)=f_X$ for $X\in E$. For $X\notin E$, let $\s(X)$ be anything in $\EAd_\IP(X)$.  So $\s$ picks for $\EAd_\IP$. 


	Choose any $p\times\mu_p\in\IB$. Suppose for contradiction that $\s$ $\mu_p$-surely pick for $\EU_p$. Then $f_X\in\EU_p(X)$ $\mu_p$-a.e. on $E$, or equivalently, $f_X\in\EU_p(X)$ on $E^\prime$ for some $E^\prime\subseteq E$ with $\mu_p(E)=\mu_p(E^\prime)>0$. In that case,
\[
\posi(\left\{f_X-h\given X\in E^\prime, h\in X\right\})\cap \mathbb{R}^n_{<0}=\varnothing
\]
since	 $p(f_X-h)\geq0$ for all $X\in E^\prime, h\in X$. But $\left\{f_X-g_X\given X\in E^\prime\right\}\subseteq\left\{f_X-h\given X\in E^\prime, h\in X\right\}$, so we must have 
\[
